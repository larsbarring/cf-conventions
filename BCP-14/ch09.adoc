...
When using these new conventions, __the features contained within a collection [red aqua-background]#must# always be of the same type; and all the collections in a CF file [red aqua-background]#must# be of the same feature type__.
(Future versions of CF [red aqua-background]#may# allow mixing of multiple feature types within a file.)
...
Other space-time coordinates [red aqua-background]#may# be included which are not mandatory.

In Table 9.1 the spatial coordinates x and y typically refer to longitude and latitude but other horizontal coordinates [black aqua-background]#could# also be used (see sections 4 and 5.6).  
...
However a featureType [red aqua-background]#may# also include other space-time coordinates which are not mandatory (notably the z coordinate, and for instance a **`forecast_reference_time`** coordinate in addition to a mandatory time coordinate).
...
As of CF version 1.6 such data [black aqua-background]#can# be stored using the representations that are documented here by two means: 1) by utilizing the orthogonal multidimensional array representation and omitting the featureType attribute; or 2) by assigning arbitrary coordinates to the mandatory dimensions. 
For example a globally-averaged latitude position (90s to 90n) [black aqua-background]#could# be represented arbitrarily (and poorly) as a latitude position at the equator.
...
Each feature [black aqua-background]#can# have its own set of element subscripts o and p.
For instance, in a collection of timeSeries features, each individual timeSeries [black aqua-background]#can# have its own set of times.
...
Other data sources, such as the output of numerical models, [red aqua-background]#may# commonly generate features of identical size.
...
The dimensions [red aqua-background]#may# be given in any order. 
If there is a need for either the instance or an element dimension to be the netCDF unlimited dimension (so that more features or more elements [black aqua-background]#can# be appended), then that dimension [red aqua-background]#must# be the outer dimension of the data variable i.e. the leading dimension in CDL.
...
If the sample dimension is the netCDF unlimited dimension, new data [black aqua-background]#can# be appended to the file.  

In all representations, the instance dimension (which is also the sample dimension in ragged representations) [red aqua-background]#may# be set initially to a size that is arbitrarily larger than what is [red aqua-background]#required# for the features which are available at the time that the file is created.  
Allocating unused array space in this way (pre-filled with missing values -- see also section 9.6, __Missing data__), [black aqua-background]#can# be useful as a means to reserve space that will be available to add features at a later time.

==== Orthogonal multidimensional array representation

The **orthogonal multidimensional array representation**, the simplest representation, [black aqua-background]#can# be used if each feature instance in the collection has identical coordinates along the element axis of the features. 
...
The **incomplete multidimensional array representation** [black aqua-background]#can# used if the features within a collection do not all have the same number of elements, but sufficient storage space is available to allocate the number of elements [red aqua-background]#required# by the longest feature to all features. 
That is, features that are shorter than the longest feature [red aqua-background]#must# be padded with missing values to bring all instances to the same storage size.
...
The **contiguous ragged array representation** [black aqua-background]#can# be used only if the size of each feature is known at the time that it is created. 
...
In this representation, the file contains a **count variable**, which [red aqua-background]#must# be an integer type and
...
For indices that correspond to features, whose data have not yet been written, the count variable [red aqua-background]#should#  have a value of zero or a missing value.
...
The canonical use case for this representation is the storage of real-time data streams that contain reports from many sources; the data [black aqua-background]#can# be written as it arrives.
...
In this representation, the file contains an **index variable**, which [red aqua-background]#must# be an integer type, and [red aqua-background]#must# have the sample dimension as its single dimension. 
...
For those indices of the sample dimension, into which data have not yet been written, the index variable [red aqua-background]#should# be pre-filled with missing values.

[[featureType, Section 9.4, "The featureType  attribute"]]
=== The featureType  attribute

A global attribute, **featureType**, is [red aqua-background]#required# for all Discrete Geometry representations except the orthogonal multidimensional array representation, for which it is highly recommended.
The exception is allowed for backwards compatibility, as discussed in 9.3.1.
A Discrete Geometry file [red aqua-background]#may# include arbitrary numbers of data variables, but (as of CF v1.6) all of the data variables contained in a single file [red aqua-background]#must# be of the single feature type indicated by the global **`featureType`** attribute, if it is present.1   The value assigned to the **`featureType`** attribute is case-insensitive;  it [red aqua-background]#must# be one of the string values listed in the left column of Table 9.1.

[[coordinates-metadata, Section 9.5, "Coordinates and metadata"]]
=== Coordinates and metadata

Every feature within a Discrete Geometry CF file [red aqua-background]#must# be unambiguously associated with an extensible collection of instance variables that identify the feature and provide other metadata as needed to describe it.
Every element of every feature [red aqua-background]#must# be unambiguously associated with its space and time coordinates and with the feature that contains it.
The **`coordinates`** attribute [red aqua-background]#must# be attached to every data variable to indicate the spatiotemporal coordinate variables that are needed to geo-locate the data.

Where feasible, one of the coordinate or auxiliary coordinate variables of a discrete sampling geometry [red aqua-background]#should# have an attribute named **`cf_role`**.
...
The variable carrying the **`cf_role`** attribute [red aqua-background]#may# have any data type.
When a variable is assigned this attribute, it [red aqua-background]#must# provide a unique identifier for each feature instance.  
CF files that contain timeSeries, profile or trajectory featureTypes, [red aqua-background]#should# include only a single occurrence of a **`cf_role`** attribute;  CF files that contain timeSeriesProfile or trajectoryProfile [red aqua-background]#may# contain two occurrences, corresponding to the two levels of structure in these feature types.
...
Only the set of coordinates which are regarded as the nominal (default or preferred) positions [red aqua-background]#should# be indicated by the attribute **`axis`**, which [red aqua-background]#should# be assigned string values to indicate the orientations of the axes (**`X`**, **`Y`**, **`Z`**, or **`T`**).
See <<example-h.5>> (a single timeseries with time-varying deviations from a nominal point spatial location): 
Auxiliary coordinate variables containing the nominal and the precise positions [red aqua-background]#should# be listed in the relevant **`coordinates`** attributes of data variables.
In orthogonal representations the nominal positions [black aqua-background]#could# be  coordinate variables, which do not need to be listed in the **`coordinates`** attribute, rather than auxiliary coordinate variables.

Coordinate bounds [red aqua-background]#may# optionally be associated with coordinate variables and auxiliary coordinate variables using the bounds attribute, following the conventions described in section 7.1.
...
If there is a vertical coordinate variable or auxiliary coordinate variable, it [red aqua-background]#must# be identified by the means specified in section 4.3.  
The use of the attribute **`axis=Z`** is [red aqua-background]#recommended# for clarity.
...
In data for discrete sampling geometries written according to the rules of this section, wherever there are unused elements in data storage, the data variable and all its auxiliary coordinate variables (spatial and time) [red aqua-background]#must# contain missing values.
This situation [red aqua-background]#may# arise for the incomplete multidimensional array representation, and in any representation if the instance dimension is set to a larger size than the number of features currently stored.
Data variables [red aqua-background]#should# (as usual) also contain missing values to indicate when there is no valid data available for the element, although the coordinates are valid.

Similarly, for indices where the instance variable identified by **`cf_role`** contains a missing value indicator, all other instance variables [red aqua-background]#should# also contain missing values.
