...
For instance, if a data variable contains time series of observational data from a number of observing stations, it [red]#may# be convenient to provide the names of the stations as labels for the elements of the station dimension (<<time-series-data>>).
...
When data is representative of geographic regions which can be identified by names but which have complex boundaries that cannot practically be specified using longitude and latitude boundary coordinates, a labeled axis [red]#should# be used to identify the regions.
...
To indicate that the label values are standardized the variable that contains the labels [red]#must# be given the **`standard_name`** attribute with the value `region`.
...
The plain-language name of the taxon [red]#must# be contained in a variable with **`standard_name`** of **`biological_taxon_name`**.
A Life Science Identifier (LSID) [red]#may# be contained in a variable with **`standard_name`** of **`biological_taxon_lsid`**.
...
It is strongly [red]#recommended# in CF that the authority chosen is World Register of Marine Species (WoRMS) for oceanographic data and Integrated Taxonomic Information System (ITIS) for freshwater and terrestrial data.
...
This [red]#may# be converted to a URL by adding prefixes such as ​https://www.lsid.info/.
...
If both are present then each **`biological_taxon_name`** coordinate [red]#must# exactly match the name resolved from the **`biological_taxon_lsid`** coordinate.
If LSIDs are available for some taxa in a dataset then the **`biological_taxon_lsid**` auxiliary coordinate variable [red]#should# be included and missing data given for those taxa that do not have an identifier.
...
In some situations a dimension [red]#may# have alternative sets of coordinates values.
...
For such alternative coordinate variables, there are no mandatory attributes, but they [red]#may# have any of the attributes allowed for coordinate variables.
...
Levels on a vertical axis [red]#may# be described by both the physical coordinate and the ordinal model level number.
----
