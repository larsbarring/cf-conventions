...
One byte numeric data [red aqua-background]#should# be stored using the **`byte`** or **`unsigned byte`** data types.
...
In many situations, any integer type [red aqua-background]#may# be used.
When the phrase "integer type" is used in this document, it [red aqua-background]#should# be understood to mean **`byte`**, **`unsigned byte`**, **`short`**, **`unsigned short`**, **`int`**, **`unsigned int`**, **`int64`**, or **`unsigned int64`**.

Strings in variables [red aqua-background]#may# be represented one of two ways - as atomic strings or as character arrays.
An n-dimensional array of strings [red aqua-background]#may# be implemented as a variable of type **`string`** with n dimensions, or as a variable of type **`char`** with n+1 dimensions where the last (most rapidly varying) dimension is large enough to contain the longest string in the variable.
...
The other strings, such as "May", [red aqua-background]#should# be padded with trailing NULL or space characters so that every array element is filled.
If the atomic string option is chosen, each element of the variable [black aqua-background]#can# be assigned a string with a different length.
...
Variable, dimension, attribute and group names [red aqua-background]#should# begin with a letter and be composed of letters, digits, and underscores.
...
Case is significant in netCDF names, but it is [red aqua-background]#recommended# that names [red aqua-background]#should not# be distinguished purely by case, i.e., if case is disregarded, no two names [red aqua-background]#should# be the same.
It is also [red aqua-background]#recommended# that names [red aqua-background]#should# be obviously meaningful, if possible, as this renders the file more effectively self-describing.

This convention does not standardize any variable or dimension names.
Attribute names and their contents, where standardized, are given in English in this document and [red aqua-background]#should# appear in English in conforming netCDF files for the sake of portability.
Languages other than English are permitted for variables, dimensions, and non-standardized attributes.
The content of some standardized attributes are string values that are not standardized, and thus are not [red aqua-background]#required# to be in English.
For example, a description of what a variable represents [red aqua-background]#may# be given in a non-English language using the **`long_name`** attribute (see <<long-name>>) whose contents are not standardized, but a description given by the **`standard_name`** attribute (see <<standard-name>>) [red aqua-background]#must# be taken from the standard name table which is in English.

[[dimensions]]
=== Dimensions

A variable [red aqua-background]#may# have any number of dimensions, including zero, and the dimensions [red aqua-background]#must# all have different names.
__COARDS strongly [black aqua-background]#recommends# limiting the number of dimensions to four, but we wish to allow greater flexibility__.
The dimensions of the variable define the axes of the quantity it contains.
Dimensions other than those of space and time [red aqua-background]#may# be included.
Several examples [black aqua-background]#can# be found in this document.
Under certain circumstances, one [red aqua-background]#may# need more than one dimension in a particular quantity.
...
Dimensions [red aqua-background]#may# be of any size, including unity.
When a single value of some coordinate applies to all the values in a variable, the [red aqua-background]#recommended# means of attaching this information to the variable is by use of a dimension of size unity with a one-element coordinate variable.
...
The advantage of using either a coordinate variable or an auxiliary coordinate variable is that all its attributes [black aqua-background]#can# be used to describe the single-valued quantity, including boundaries.
...
Generic applications [red aqua-background]#should# treat the data as missing where any auxiliary coordinate variables have missing values; special-purpose applications might be able to make use of the data.
...
Since version 2.4 the NUG defines missing data as all values outside of the **`valid_range`**, and specifies how the **`valid_range`** [red aqua-background]#should# be defined from the **`_FillValue`** (which has library specified default values) if it hasn't been explicitly specified.
If only one missing value is needed for a variable then we [black aqua-background]#recommend#  that this value be specified using the **`_FillValue`** attribute.
...
The **`_FillValue`** [red aqua-background]#should# be outside the range specified by **`valid_range`** (if used) for a variable.
...
Applications that process variables that have attributes to indicate both a transformation (via a scale and/or offset) and missing values [red aqua-background]#should# first check that a data value is valid, and then apply the transformation.
Note that values that are identified as missing [red aqua-background]#should not# be transformed.
Since the missing value is outside the valid range it is possible that applying a transformation to it [black aqua-background]#could# result in an invalid operation.
...
If the variable is packed using the **`scale_factor`** and **`add_offset`** attributes (see <<packed-data>>), the elements of the **`actual_range`** [red aqua-background]#should# have the type intended for the unpacked data.
The elements of **`actual_range`** [red aqua-background]#must# be exactly equal to the minimum and the maximum data values which occur in the variable (when unpacked if packing is used), and both [red aqua-background]#must# be within the **`valid_range`** if specified.
If the data is all missing or invalid, the **`actual_range`** attribute cannot be used.

=== Attributes

This standard describes many attributes (some mandatory, others optional), but a file [red aqua-background]#may# also contain non-standard attributes.
Such attributes do not represent a violation of this standard.
Application programs [red aqua-background]#should# ignore attributes that they do not recognise or which are irrelevant for their purposes.
Conventional attribute names [red aqua-background]#should# be used wherever applicable.
Non-standard names [red aqua-background]#should# be as meaningful as possible.
Before introducing an attribute, consideration [red aqua-background]#should# be given to whether the information would be better represented as a variable.
In general, if a proposed attribute requires ancillary data to describe it, is multidimensional, requires any of the defined netCDF dimensions to index its values, or requires a significant amount of storage, a variable [red aqua-background]#should# be used instead.
When this standard defines string attributes that [red aqua-background]#may# take various prescribed values, the possible values are generally given in lower case.
However, applications programs [red aqua-background]#should not# be sensitive to case in these attributes.
...
The list [red aqua-background]#may# begin and end with any number of spaces.
See <<attribute-appendix>> for a list of attributes described by this standard.

[[identification-of-conventions]]
==== Identification of Conventions

Files that follow this version of the CF Conventions [red aqua-background]#must# indicate this by setting the NUG defined global attribute **`Conventions`** to a string value that contains "**`CF-{current-version-as-attribute}`**".
The Conventions version number contained in that string [black aqua-background]#can# be used to find the web based versions of this document are from the link:$$https://cfconventions.org/$$[netCDF Conventions web page].
...
In this case, the value of the Conventions attribute [red aqua-background]#may# be a single text string containing a list of the convention names separated by blank space (recommended) or commas (if a convention name contains blanks).
This is the Unidata [red aqua-background]#recommended# syntax from NetCDF Users Guide, Appendix A.
...
For readability in ncdump outputs it is [red aqua-background]#recommended# to embed newline characters into long strings to break them into lines.
...
If it was model-generated, **`source`** [red aqua-background]#should# name the model and its version, as specifically as [black aqua-background]#could# be useful.
If it is observational, **`source`** [red aqua-background]#should# characterize it (e.g., "**`surface observation`**" or "**`radiosonde`**").
...
We [black aqua-background]#recommend# that each line begin with a timestamp indicating the date and time of day that the program was executed.
...
It [red aqua-background]#may# be of benefit to name groups in such a way that human readers [black aqua-background]#can# interpret them.
However, files that conform to this standard [red aqua-background]#shall not# require software to interpret or decode information from group names.
References to out-of-group variable and dimensions [red aqua-background]#shall# be found by applying the scoping rules outlined below.
...
For example, you [black aqua-background]#can# define a dimension in the root group, and use its dimension id when defining a variable in a sub-group."

Any variable or dimension [black aqua-background]#can# be referred to, as long as it [black aqua-background]#can# be found with one of the following search strategies:
...
Because coordinate variables [red aqua-background]#must# share dimensions with the variables that reference them, the ancestor search is executed only until the local apex group is reached.
...
The lateral search algorithm [red aqua-background]#may# only be used for NUG coordinate variables; it [red aqua-background]#shall not# be used for auxiliary coordinate variables.

[NOTE]
====
This use of the lateral search strategy to find them is discouraged.
They are allowed mainly for backwards-compatibility with existing datasets, and [red aqua-background]#may# be [black aqua-background]#deprecated# in future versions of the standard.
====

==== Application of attributes

The following attributes are [red aqua-background]#optional# for non-root groups.
...
If these attributes are present, they [red aqua-background]#may# be applied additively to the parent attributes of the same name.
...
The following attributes [red aqua-background]#may# only be used in the root group and [red aqua-background]#shall not# be duplicated or overridden in child groups:

* `Conventions`
* `external_variables`

Furthermore, per-variable attributes [red aqua-background]#must# be attached to the variables to which they refer.
They [red aqua-background]#may# not be attached to a group, even if all variables within that group use the same attribute and value.
