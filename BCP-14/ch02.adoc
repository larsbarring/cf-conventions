...
NetCDF files [red]#should# have the file name extension "**`.nc`**".



=== Data Types

// TODO: Check, [red]#should# this be a bullet list?
Data variables [red]#must# be one of the following data types: **`string`**, **`char`**, **`byte`**, **`unsigned byte`**, **`short`**, **`unsigned short`**, **`int`**, **`unsigned int`**, **`int64`**, **`unsigned int64`**, **`float`** or **`real`**, and **`double`** (which are all the [netCDF external data types](https://docs.unidata.ucar.edu/nug/current/md_types.html) supported by netCDF-4).
...
One byte numeric data [red]#should# be stored using the **`byte`** or **`unsigned byte`** data types.
...
In many situations, any integer type [red]#may# be used.
When the phrase "integer type" is used in this document, it [red]#should# be understood to mean **`byte`**, **`unsigned byte`**, **`short`**, **`unsigned short`**, **`int`**, **`unsigned int`**, **`int64`**, or **`unsigned int64`**.

Strings in variables [red]#may# be represented one of two ways - as atomic strings or as character arrays.
An n-dimensional array of strings [red]#may# be implemented as a variable of type **`string`** with n dimensions, or as a variable of type **`char`** with n+1 dimensions where the last (most rapidly varying) dimension is large enough to contain the longest string in the variable.
...
The other strings, such as "May", [red]#should# be padded with trailing NULL or space characters so that every array element is filled.
...
Variable, dimension, attribute and group names [red]#should# begin with a letter and be composed of letters, digits, and underscores.
...
Case is significant in netCDF names, but it is [red]#recommended# that names [red]#should not# be distinguished purely by case, i.e., if case is disregarded, no two names [red]#should# be the same.
It is also [red]#recommended# that names [red]#should# be obviously meaningful, if possible, as this renders the file more effectively self-describing.

This convention does not standardize any variable or dimension names.
Attribute names and their contents, where standardized, are given in English in this document and [red]#should# appear in English in conforming netCDF files for the sake of portability.
Languages other than English are permitted for variables, dimensions, and non-standardized attributes.
The content of some standardized attributes are string values that are not standardized, and thus are not [red]#required# to be in English.
For example, a description of what a variable represents [red]#may# be given in a non-English language using the **`long_name`** attribute (see <<long-name>>) whose contents are not standardized, but a description given by the **`standard_name`** attribute (see <<standard-name>>) [red]#must# be taken from the standard name table which is in English.
...
A variable [red]#may# have any number of dimensions, including zero, and the dimensions [red]#must# all have different names.
...
Dimensions other than those of space and time [red]#may# be included.
Several examples can be found in this document.
Under certain circumstances, one [red]#may# need more than one dimension in a particular quantity.
...
Dimensions [red]#may# be of any size, including unity.
When a single value of some coordinate applies to all the values in a variable, the [red]#recommended# means of attaching this information to the variable is by use of a dimension of size unity with a one-element coordinate variable.
...
Generic applications [red]#should# treat the data as missing where any auxiliary coordinate variables have missing values; special-purpose applications might be able to make use of the data.
...
Since version 2.4 the NUG defines missing data as all values outside of the **`valid_range`**, and specifies how the **`valid_range`** [red]#should# be defined from the **`_FillValue`** (which has library specified default values) if it hasn't been explicitly specified.
...
The **`_FillValue`** [red]#should# be outside the range specified by **`valid_range`** (if used) for a variable.
...
Applications that process variables that have attributes to indicate both a transformation (via a scale and/or offset) and missing values [red]#should# first check that a data value is valid, and then apply the transformation.
Note that values that are identified as missing [red]#should not# be transformed.
...
If the variable is packed using the **`scale_factor`** and **`add_offset`** attributes (see <<packed-data>>), the elements of the **`actual_range`** [red]#should# have the type intended for the unpacked data.
The elements of **`actual_range`** [red]#must# be exactly equal to the minimum and the maximum data values which occur in the variable (when unpacked if packing is used), and both [red]#must# be within the **`valid_range`** if specified.
If the data is all missing or invalid, the **`actual_range`** attribute cannot be used.

=== Attributes

This standard describes many attributes (some mandatory, others optional), but a file [red]#may# also contain non-standard attributes.
Such attributes do not represent a violation of this standard.
Application programs [red]#should# ignore attributes that they do not recognise or which are irrelevant for their purposes.
Conventional attribute names [red]#should# be used wherever applicable.
Non-standard names [red]#should# be as meaningful as possible.
Before introducing an attribute, consideration [red]#should# be given to whether the information would be better represented as a variable.
In general, if a proposed attribute requires ancillary data to describe it, is multidimensional, requires any of the defined netCDF dimensions to index its values, or requires a significant amount of storage, a variable [red]#should# be used instead.
When this standard defines string attributes that [red]#may# take various prescribed values, the possible values are generally given in lower case.
However, applications programs [red]#should not# be sensitive to case in these attributes.
...
The list [red]#may# begin and end with any number of spaces.
...
Files that follow this version of the CF Conventions [red]#must# indicate this by setting the NUG defined global attribute **`Conventions`** to a string value that contains "**`CF-{current-version-as-attribute}`**".
...
In this case, the value of the Conventions attribute [red]#may# be a single text string containing a list of the convention names separated by blank space (recommended) or commas (if a convention name contains blanks).
This is the Unidata [red]#recommended# syntax from NetCDF Users Guide, Appendix A.
...
For readability in ncdump outputs it is [red]#recommended# to embed newline characters into long strings to break them into lines.
...
If it was model-generated, **`source`** [red]#should# name the model and its version, as specifically as could be useful.
If it is observational, **`source`** [red]#should# characterize it (e.g., "**`surface observation`**" or "**`radiosonde`**").
...
It [red]#may# be of benefit to name groups in such a way that human readers can interpret them.
However, files that conform to this standard [red]#shall not# require software to interpret or decode information from group names.
References to out-of-group variable and dimensions [red]#shall# be found by applying the scoping rules outlined below.
...
Because coordinate variables [red]#must# share dimensions with the variables that reference them, the ancestor search is executed only until the local apex group is reached.
...
The lateral search algorithm [red]#may# only be used for NUG coordinate variables; it [red]#shall not# be used for auxiliary coordinate variables.

[NOTE]
====
This use of the lateral search strategy to find them is discouraged.
They are allowed mainly for backwards-compatibility with existing datasets, and [red]#may# be deprecated in future versions of the standard.
====

==== Application of attributes

The following attributes are [red]#optional# for non-root groups.
...
If these attributes are present, they [red]#may# be applied additively to the parent attributes of the same name.
...
The following attributes [red]#may# only be used in the root group and [red]#shall not# be duplicated or overridden in child groups:

* `Conventions`
* `external_variables`

Furthermore, per-variable attributes [red]#must# be attached to the variables to which they refer.
They [red]#may# not be attached to a group, even if all variables within that group use the same attribute and value.
