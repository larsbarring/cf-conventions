...
followed by a sequence of **`entry`** elements which [red]#may# optionally be followed by a sequence of **`alias`** elements.
...
`Entry` elements [red]#may# optionally also contain the following elements:
...
It is possible that new tags [red]#may# be added in the future.
Any applications that parse the standard table [red]#should# be written so that unrecognized tags are gracefully ignored.
