...
* {issues}590[Issue #590]: Clarify that grid_mapping [lime-background]#can# also be used for converting spatial bounds
...
* {issues}584[Issue #584]: Allow `Z` as time-zone offset, with a couple of examples, and [lime-background]#allow# time-zone offset with date alone, both being consistent with UDUNITS syntax.
...
* {issues}166[Issue #166]: Clarify that time coordinate variables [aqua-background]#must# have **`units`** containing **`since`** and a reference time; distinguish between canonical units of time with and without **`since`**.
* {issues}141[Issue #141]: Clarification that text [aqua-background]#may# be stored in variables and attributes as either vlen strings or char arrays, and [aqua-background]#must# be represented in Unicode Normalization Form C and encoded according to UTF-8.
...
* {issues}174[Issue #174]: A one-dimensional string-valued variable [aqua-background]#must not# have the same name as its dimension, in order to avoid its being mistaken for a coordinate variable.
...
* {issues}509[Issue #509]: In exceptional cases [lime-background]#allow# a standard name to be aliased into two alternatives
* {issues}501[Issue #501]: Clarify that data variables and variables containing coordinate data are highly [aqua-background]#recommended# to have **`long_name`** or **`standard_name`** attributes, that **`cf_role`** is used only for discrete sampling geometries and UGRID mesh topologies, and that CF does not prohibit CF attributes from being used in ways that are not defined by CF but that in such cases their meaning is not defined by CF.
...
* {issues}468[Issue #468]: Update section 2.3 to clarify [aqua-background]#recommended# character set
...
* {issues}179[Issue #179]: Don't [lime-background]#require# longitude and Latitude for projected coordinates.
...
* {issues}155[Issue #155], {issues}156[Issue #156]: Allow alternate grid mappings for geometry containers. When node_count attribute is missing, [lime-background]#require# the dimension of the node coordinate variables to be one of the dimensions of the data variable.
...
* {tickets}164.html[Ticket #164]: If coordinates attribute is carried by geometry container, [lime-background]#require# coordinate variables which correspond to node coordinate variables to have the corresponding axis attribute.
