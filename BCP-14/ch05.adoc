...
The dimensions of an auxiliary coordinate variable [red]#must# be a subset of the dimensions of the variable with which the coordinate is associated, with three exceptions.
...
We recommend that the name of a multidimensional coordinate variable [red]#should not# match the name of any of its dimensions because that precludes supplying a coordinate variable for the dimension.
...
An application that is trying to find the latitude coordinate of a variable [red]#should# always look first to see if any of the variable's dimensions correspond to a latitude coordinate variable.
If the latitude coordinate is not found this way, then the auxiliary coordinate variables listed by the **`coordinates`** attribute [red]#should# be checked.
...
If the longitude, latitude, vertical or time coordinate is single-valued, it [red]#may# be stored either as a coordinate variable with a dimension of size one, or as a scalar coordinate variable (<<scalar-coordinate-variables>>).
...
However, it is not permissible for a data variable to have both a coordinate variable and an auxiliary coordinate variable, or more than one of either type of variable, having an **`axis`** attribute with any given value e.g. there [red]#must# be no more than one **`axis`** attribute for **`X`** for any data variable.
Note that if the **`axis`** attribute is not specified for an auxiliary coordinate variable, it [red]#may# still be possible to determine if it is a spatiotemporal dimension from its own units or standard_name, or from the units and standard_name of the coordinate variable corresponding to its dimensions (see <<coordinate-types>>).
...
To geo-reference data horizontally with respect to the Earth, a __grid mapping variable__ [red]#may# be provided by the data variable, using the **`grid_mapping`** attribute.
If the coordinate variables for a horizontal grid are not longitude and latitude, then a grid_mapping variable provides the information [red]#required# to derive longitude and latitude values for each grid location.
If no __grid mapping variable__ is referenced by a data variable, then longitude and latitude coordinate values [red]#shall# be supplied __in addition__ to the [red]#required# coordinates.
For example, the Cartesian coordinates of a map projection [red]#may# be supplied as coordinate variables and, in addition, two-dimensional latitude and longitude variables [red]#may# be supplied via the **`coordinates`** attribute on a data variable.
The use of the **`axis`** attribute with values **`X`** and **`Y`** is [red]#recommended# for the coordinate variables (see <<coordinate-types>>).
...
A __grid mapping variable__ [red]#may# be referenced by a data variable  in order to explicitly declare the coordinate reference system (CRS) used for the horizontal spatial coordinate values.
...
When the horizontal spatial coordinate variables are not longitude and latitude, it is [red]#required# that further information is provided to geo-locate the horizontal position.
A __grid mapping variable__ provides this information.

If no __grid mapping variable__ is provided and the coordinate variables for a horizontal grid are not longitude and latitude, then it is [red]#required# that the latitude and longitude coordinates are supplied via the **`coordinates`** attribute.
Such coordinates [red]#may# be provided in addition to the provision of a __grid mapping variable__, but that is not required.
...
The one attribute that all grid mapping variables [red]#must# have is grid_mapping_name, which takes a string value that contains the mapping's name.
...
This attribute is attached to data variables so that variables with different mappings [red]#may# be present in a single file.
...
Explicit 'axis order' is important when the grid_mapping_variable contains an attribute crs_wkt as it is mandated by the OGC CRS-WKT standard that coordinate tuples with correct axis order are provided as part of the reference to a Coordinate Reference System.
...
The `grid_mapping` variable [red]#may# identify datums (such as the reference ellipsoid, the geoid or the prime meridian) for horizontal or vertical coordinates.
Therefore a grid mapping variable [red]#may# be needed when the coordinate variables for a horizontal grid are longitude and latitude.
The `grid_mapping_name` of `latitude_longitude` [red]#should# be used in this case.

The expanded form of the **`grid_mapping attribute`** is [red]#required# if one wants to store coordinate information for more than one coordinate reference system.
...
This syntax [red]#may# be used to explicitly link coordinates and grid mapping variables where only one coordinate reference system is used.
...
All coordinate names listed in the **`grid_mapping`** attribute [red]#must# be coordinate variables or auxiliary coordinates of the data variable.
...
This [red]#should# prevent a COARDS compliant application from mistaking the variables **`rlon`** and **`rlat`** to be actual longitude and latitude coordinates.
...
An [red]#optional# grid mapping attribute called **`crs_wkt`** [red]#may# be used to specify multiple coordinate system properties in so-called __well-known text__ format (usually abbreviated to CRS WKT or OGC WKT).
...
The **`crs_wkt`** attribute [red]#should# comprise a text string that conforms to the WKT syntax as specified in reference <<OGC_WKT-CRS>>.
If desired the text string [red]#may# contain embedded newline characters to aid human readability.
...
The grid_mapping attribute is thus given a value crs:lat lon to define that where coordinate pairs are required, these [red]#shall# be ordered (lat, lon), to be consistent with the provided crs_wkt string (and not order inverted).
...
Therefore the CRS [red]#should# be described as thoroughly as possible with the single-property grid mapping attributes as well as by **`crs_wkt`**.

In cases where CRS property values can be represented by both a single-property grid mapping attribute and the **`crs_wkt`** attribute, the grid mapping [red]#should# be provided, and if both are provided, the onus is on data producers to ensure that their property values are consistent.
Therefore information from either one (or both) [red]#may# be read in by the user without needing to check both.
However, if the two values of a given property are different, the CRS information cannot be interpreted accurately and users [red]#should# inform the provider so the issue can be addressed.
...
Likewise, in those cases where the value of a CRS WKT element [red]#should# be used consistently across the CF-netCDF community (names of projections and projection parameters, for example) then, the values shown in
https://github.com/cf-convention/cf-conventions/wiki/Mapping-from-CF-Grid-Mapping-Attributes-to-CRS-WKT-Elements
...
Example 5.12 illustrates how certain WKT elements - all of which are [red]#optional# - can be used to specify CRS properties not covered by existing CF grid mapping attributes, including:
...
The preceding two example (5.11 and 5.12) [red]#may# be combined, if the data provider desires to provide explicit latitude and longitude coordinates as well as projection coordinates and to provide CRS WKT referencing for both sets of coordinates.
...
When a variable has an associated coordinate which is single-valued, that coordinate [red]#may# be represented as a scalar variable (i.e. a data variable which has no netCDF dimensions).
Since there is no associated dimension these scalar coordinate variables [red]#should# be attached to a data variable via the **`coordinates`** attribute.
...
If two or more single-valued coordinates are not independent, but have related values (this might be the case, for instance, for time and forecast period, or vertical coordinate and model level number, <<alternative-coordinates>>), they [red]#should# be stored as coordinate or auxiliary coordinate variables of the same size one dimension, not as scalar coordinate variables.
...
The variable [red]#should# be a scalar (i.e. it has no dimensions) of arbitrary type, and the value of its single element is immaterial.
...
The dimensions of the domain [red]#must# be stored with the **`dimensions`** attribute, and the presence of a **`dimensions`** attribute will identify the variable as a domain variable.
Therefore the **`dimensions`** attribute [red]#must not# be present on any variables that are to be interpreted as data variables.
It is necessary to list these dimensions, rather than inferring them from the contents of the other attributes, as it can not be guaranteed that the referenced variables span all of the [red]#required# dimensions (as could be the case for a discrete axis, for instance).
...
If a domain has no named dimensions then the value of the **`dimensions`** attribute [red]#must# be an empty string, as could be the case if the dimensions of the domain are all defined implicitly by scalar coordinate variables.

The dimensions listed by the **`dimensions`** attribute constrain the dimensions that [red]#may# be spanned by variables referenced from any of the other attributes, in the same way that the array dimensions perform that role for a data variable.
For instance, all variables named by the **`cell_measures`** attribute (<<cell-measures>>) of a domain variable [red]#must# span a subset of zero or more of the dimensions given by the **`dimensions`** attribute.

It is [red]#optional# for coordinate variables to be listed by a domain variable's **`coordinates`** attribute.
...
It is [red]#recommended# that a domain variable has a **`long_name`** attribute to describe its contents.

It is [red]#recommended# that a domain variable does not have any of the attributes marked in <<attribute-appendix>> as applicable to data variables except those which are also marked as applicable to domain variables.

Multiple domain variables [red]#may# exist in a file with, or without, data variables.
...
A mesh topology variable [red]#may# provide the topology for one or more domains, defined at the nodes, edges, or faces of the mesh.
...
A data or domain variable [red]#may# use one of a mesh topology variable's domains by referencing the mesh topology variable with the **`mesh`** attribute; along with the identity of [red]#required# domain provided by the **`location`** attribute (see example <<example-mesh-topology-variable>>).

The variables containing the coordinate values for cells indexed by the mesh topology are defined by the mesh topology variable but are equivalent to one-dimensional auxiliary coordinate variables, and so [red]#may# also be provided by the data or domain variable's **`coordinates`** attribute.
...
A two-dimensional UGRID mesh topology variable for the mesh depicted in <<figure-mesh-example>>, with data variables defined at face, edge and node elements of the mesh. All [red]#optional# attributes have been omitted.
