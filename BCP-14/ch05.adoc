...
The dimensions of an auxiliary coordinate variable [aqua-background]#must# be a subset of the dimensions of the variable with which the coordinate is associated, with three exceptions.
...
It is [aqua-background]#recommended# that the name of a multidimensional coordinate variable [aqua-background]#should not# match the name of any of its dimensions because that precludes supplying a coordinate variable for the dimension.
...
If the longitude, latitude, vertical or time coordinate is multi-valued, varies in only one dimension, and varies independently of other spatiotemporal coordinates, it is [lime-background]#not permitted# to store it as an auxiliary coordinate variable.
...
An application that is trying to find the latitude coordinate of a variable [aqua-background]#should# always look first to see if any of the variable's dimensions correspond to a latitude coordinate variable.
If the latitude coordinate is not found this way, then the auxiliary coordinate variables listed by the `coordinates` attribute [aqua-background]#should# be checked.
Note that it is permissible, but [aqua-background]#optional#, to list coordinate variables as well as auxiliary coordinate variables in the `coordinates` attribute.
If the longitude, latitude, vertical or time coordinate is single-valued, it [aqua-background]#may# be stored either as a coordinate variable with a dimension of size one, or as a scalar coordinate variable (<<scalar-coordinate-variables>>).

If an `axis` attribute is attached to an auxiliary coordinate variable, it [lime-background]#can# be used by applications in the same way the `axis` attribute attached to a coordinate variable is used.
However, it is not permissible for a data variable to have both a coordinate variable and an auxiliary coordinate variable, or more than one of either type of variable, having an `axis` attribute with any given value e.g. there [aqua-background]#must# be no more than one `axis` attribute for `X` for any data variable.
Note that if the `axis` attribute is not specified for an auxiliary coordinate variable, it [aqua-background]#may# still be possible to determine if it is a spatiotemporal dimension from its own units or `standard_name`, or from the units and `standard_name` of the coordinate variable corresponding to its dimensions (see <<coordinate-types>>).
For instance, auxiliary coordinate variables which lie on the horizontal surface [lime-background]#can# be identified as such by their dimensions being horizontal.
...
To geo-reference data horizontally with respect to the Earth, a __grid mapping variable__ [aqua-background]#may# be provided by the data variable, using the `grid_mapping` attribute.
If the coordinate variables for a horizontal grid are not longitude and latitude, then a grid_mapping variable provides the information [aqua-background]#required# to derive longitude and latitude values for each grid location.
If no __grid mapping variable__ is referenced by a data variable, then longitude and latitude coordinate values [aqua-background]#shall# be supplied __in addition__ to the [aqua-background]#required# coordinates.
For example, the Cartesian coordinates of a map projection [aqua-background]#may# be supplied as coordinate variables and, in addition, two-dimensional latitude and longitude variables [aqua-background]#may# be supplied via the `coordinates` attribute on a data variable.
The use of the `axis` attribute with values `X` and `Y` is [aqua-background]#recommended# for the coordinate variables (see <<coordinate-types>>).
...
The latitude and longitude coordinates of a horizontal grid that was not defined as a Cartesian product of latitude and longitude axes, [lime-background]#can# sometimes be represented using two-dimensional coordinate variables.
...
It is [aqua-background]#recommended# that this type of gridded data be stored using the compression scheme described in <<compression-by-gathering>>.
Compression by gathering preserves structure by storing a set of indices that [lime-background]#allows# an application to easily scatter the compressed data back to two-dimensional arrays.
...
Notice that even if an application does not recognize the `compress` attribute, the grids stored in this format [lime-background]#can# still be handled, by an application that recognizes the `coordinates` attribute.
...
A __grid mapping variable__ [aqua-background]#may# be referenced by a data variable  in order to explicitly declare the coordinate reference system (CRS) used for the horizontal spatial coordinate values.
For example, if the horizontal spatial coordinates are latitude and longitude, the grid mapping variable [lime-background]#can# be used to declare the figure of the earth (WGS84 ellipsoid, sphere, etc.) they are based on.
...
When the horizontal spatial coordinate variables are not longitude and latitude, it is [aqua-background]#required# that further information is provided to geo-locate the horizontal position.
A __grid mapping variable__ provides this information.

If no __grid mapping variable__ is provided and the coordinate variables for a horizontal grid are not longitude and latitude, then it is [aqua-background]#required# that the latitude and longitude coordinates are supplied via the `coordinates` attribute.
Such coordinates [aqua-background]#may# be provided in addition to the provision of a __grid mapping variable__, but that is not [aqua-background]#required#.

When a data variable is representative of cells of non-zero size, and the coordinate variables are not longitude and latitude, __bounds__ variables [aqua-background]#should# be provided for vertices of the cell boundaries in the horizontal coordinates of the grid (see  <<cell-boundaries>>).
The __grid mapping variable__ provides then the information to convert bounds in latitude and longitude coordinates, and it is [aqua-background]#optional# for the dataset to provide these.
If no __grid mapping variable__ is provided, then the cell extents in latitude and longitude coordinate system [aqua-background]#should# be provided (see <<cell-boundaries>> and especially Example 7.2 for the common case of four-sided cells).
...
The one attribute that all grid mapping variables [aqua-background]#must# have is `grid_mapping_name`, which takes a string value that contains the mapping's name.
...
This attribute is attached to data variables so that variables with different mappings [aqua-background]#may# be present in a single file.
...
The `grid_mapping` variable [aqua-background]#may# identify datums (such as the reference ellipsoid, the geoid or the prime meridian) for horizontal or vertical coordinates.
Therefore a grid mapping variable [aqua-background]#may# be needed when the coordinate variables for a horizontal grid are longitude and latitude.
The `grid_mapping_name` of `latitude_longitude` [aqua-background]#should# be used in this case.

The expanded form of the `grid_mapping` attribute is [aqua-background]#required# if one wants to store coordinate information for more than one coordinate reference system.
...
This syntax [aqua-background]#may# be used to explicitly link coordinates and grid mapping variables where only one coordinate reference system is used.
...
All coordinate names listed in the `grid_mapping` attribute [aqua-background]#must# be coordinate variables or auxiliary coordinates of the data variable.
...
A CF compliant application [lime-background]#can# determine that rlon and rlat are longitude and latitude values in the rotated grid by recognizing the standard names `grid_longitude` and `grid_latitude`.
Note that the units of the rotated longitude and latitude axes are given as `degrees`.
This [aqua-background]#should# prevent a COARDS compliant application from mistaking the variables `rlon` and `rlat` to be actual longitude and latitude coordinates.
...
An application [lime-background]#can# determine that `x` and `y` are the projection coordinates by recognizing the standard names `projection_x_coordinate` and `projection_y_coordinate`.
...
An [aqua-background]#optional# grid mapping attribute called `crs_wkt` [aqua-background]#may# be used to specify multiple coordinate system properties in so-called __well-known text__ format (usually abbreviated to CRS WKT or OGC WKT).
...
The `crs_wkt` attribute [aqua-background]#should# comprise a text string that conforms to the WKT syntax as specified in reference <<OGC_WKT-CRS>>.
If desired the text string [aqua-background]#may# contain embedded newline characters to aid human readability.
...
The `grid_mapping` attribute is thus given a value `crs:lat lon` to define that where coordinate pairs are [aqua-background]#required#, these [aqua-background]#shall# be ordered (lat, lon), to be consistent with the provided `crs_wkt` string (and not order inverted).
A 2-D array of (lat, lon) tuples [lime-background]#can# then be explicitly derived from the combination of the lat and lon variables.
...
Therefore the CRS [aqua-background]#should# be described as thoroughly as possible with the single-property grid mapping attributes as well as by `crs_wkt`.

In cases where CRS property values [lime-background]#can# be represented by both a single-property grid mapping attribute and the `crs_wkt` attribute, the grid mapping [aqua-background]#should# be provided, and if both are provided, the onus is on data producers to ensure that their property values are consistent.
Therefore information from either one (or both) [aqua-background]#may# be read in by the user without needing to check both.
However, if the two values of a given property are different, the CRS information cannot be interpreted accurately and users [aqua-background]#should# inform the provider so the issue [lime-background]#can# be addressed.
...
Likewise, in those cases where the value of a CRS WKT element [aqua-background]#should# be used consistently across the CF-netCDF community (names of projections and projection parameters, for example) then, the values shown in
https://github.com/cf-convention/cf-conventions/wiki/Mapping-from-CF-Grid-Mapping-Attributes-to-CRS-WKT-Elements
...
Examples 5.11 illustrates how the coordinate system properties specified via the `crs` grid mapping variable in Example 5.9 [lime-background]#might# be expressed using a `crs_wkt` attribute.
...
Example 5.12 illustrates how certain WKT elements - all of which are [aqua-background]#optional# - [lime-background]#can# be used to specify CRS properties not covered by existing CF grid mapping attributes, including:
...
The preceding two example (5.11 and 5.12) [aqua-background]#may# be combined, if the data provider desires to provide explicit latitude and longitude coordinates as well as projection coordinates and to provide CRS WKT referencing for both sets of coordinates.
...
When a variable has an associated coordinate which is single-valued, that coordinate [aqua-background]#may# be represented as a scalar variable (i.e. a data variable which has no netCDF dimensions).
Since there is no associated dimension these scalar coordinate variables [aqua-background]#should# be attached to a data variable via the `coordinates` attribute.

The use of scalar coordinate variables is a convenience feature which avoids adding size one dimensions to variables.
A numeric scalar coordinate variable has the same information content and [lime-background]#can# be used in the same contexts as a size one numeric coordinate variable.
...
Once a name is used for a scalar coordinate variable it [lime-background]#can not# be used for a 1D coordinate variable.
For this reason it is strongly [aqua-background]#recommended# against using a name for a scalar coordinate variable that matches the name of any dimension in the file.
...
If two or more single-valued coordinates are not independent, but have related values (this [lime-background]#might# be the case, for instance, for time and forecast period, or vertical coordinate and model level number, <<alternative-coordinates>>), they [aqua-background]#should# be stored as coordinate or auxiliary coordinate variables of the same size one dimension, not as scalar coordinate variables.
...
The domain variable supports the same attributes as are [lime-background]#allowed# on a data variable for describing a domain, with exactly the same meanings and syntaxes, as described in <<attribute-appendix>>.
...
The dimensions of the domain [aqua-background]#must# be stored with the `dimensions` attribute, and the presence of a `dimensions` attribute will identify the variable as a domain variable.
Therefore the `dimensions` attribute [aqua-background]#must not# be present on any variables that are to be interpreted as data variables.
It is necessary to list these dimensions, rather than inferring them from the contents of the other attributes, as it [lime-background]#can not# be guaranteed that the referenced variables span all of the [aqua-background]#required# dimensions (as [lime-background]#could# be the case for a discrete axis, for instance).
...
If a domain has no named dimensions then the value of the `dimensions` attribute [aqua-background]#must# be an empty string, as [lime-background]#could# be the case if the dimensions of the domain are all defined implicitly by scalar coordinate variables.

The dimensions listed by the `dimensions` attribute constrain the dimensions that [aqua-background]#may# be spanned by variables referenced from any of the other attributes, in the same way that the array dimensions perform that role for a data variable.
For instance, all variables named by the `cell_measures` attribute (<<cell-measures>>) of a domain variable [aqua-background]#must# span a subset of zero or more of the dimensions given by the `dimensions` attribute.

It is [aqua-background]#optional# for coordinate variables to be listed by a domain variable's `coordinates` attribute.
...
It is [aqua-background]#recommended# that a domain variable has a `long_name` attribute to describe its contents.

It is [aqua-background]#recommended# that a domain variable does not have any of the attributes marked in <<attribute-appendix>> as applicable to data variables except those which are also marked as applicable to domain variables.

Multiple domain variables [aqua-background]#may# exist in a file with, or without, data variables.
Note that the data variable attributes describing its domain [lime-background]#can not# be replaced by a reference to a domain variable.
...
A mesh topology variable [aqua-background]#may# provide the topology for one or more domains, defined at the nodes, edges, or faces of the mesh.
...
The canonical definitions of mesh topology variables and location index set variables are given externally by the UGRID conventions <<UGRID>>, but their standardized attributes, many of which are [aqua-background]#optional#, are listed in <<appendix-mesh-topology-attributes>> and <<attribute-appendix>>.
...
A data or domain variable [aqua-background]#may# use one of a mesh topology variable's domains by referencing the mesh topology variable with the **`mesh`** attribute; along with the identity of [aqua-background]#required# domain provided by the **`location`** attribute (see example <<example-mesh-topology-variable>>).

The variables containing the coordinate values for cells indexed by the mesh topology are defined by the mesh topology variable but are equivalent to one-dimensional auxiliary coordinate variables, and so [aqua-background]#may# also be provided by the data or domain variable's **`coordinates`** attribute.
Note that the mesh topology variable [lime-background]#allows# cell bounds to be provided without any cell coordinate values, via its __**`node_coordinates`**__ attribute.
...
A two-dimensional UGRID mesh topology variable for the mesh depicted in <<figure-mesh-example>>, with data variables defined at face, edge and node elements of the mesh. All [aqua-background]#optional# attributes have been omitted.
