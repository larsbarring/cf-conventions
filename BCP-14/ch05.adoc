...
The dimensions of an auxiliary coordinate variable [red aqua-background]#must# be a subset of the dimensions of the variable with which the coordinate is associated, with three exceptions.
...
We [black aqua-background]#recommend# that the name of a multidimensional coordinate variable [red aqua-background]#should not# match the name of any of its dimensions because that precludes supplying a coordinate variable for the dimension.
...
An application that is trying to find the latitude coordinate of a variable [red aqua-background]#should# always look first to see if any of the variable's dimensions correspond to a latitude coordinate variable.
If the latitude coordinate is not found this way, then the auxiliary coordinate variables listed by the `coordinates` attribute [red aqua-background]#should# be checked.
...
If the longitude, latitude, vertical or time coordinate is single-valued, it [red aqua-background]#may# be stored either as a coordinate variable with a dimension of size one, or as a scalar coordinate variable (<<scalar-coordinate-variables>>).

If an `axis` attribute is attached to an auxiliary coordinate variable, it [black aqua-background]#can# be used by applications in the same way the `axis` attribute attached to a coordinate variable is used.
However, it is not permissible for a data variable to have both a coordinate variable and an auxiliary coordinate variable, or more than one of either type of variable, having an `axis` attribute with any given value e.g. there [red aqua-background]#must# be no more than one `axis` attribute for `X` for any data variable.
Note that if the `axis` attribute is not specified for an auxiliary coordinate variable, it [red aqua-background]#may# still be possible to determine if it is a spatiotemporal dimension from its own units or `standard_name`, or from the units and `standard_name` of the coordinate variable corresponding to its dimensions (see <<coordinate-types>>).
For instance, auxiliary coordinate variables which lie on the horizontal surface [black aqua-background]#can# be identified as such by their dimensions being horizontal.
...
To geo-reference data horizontally with respect to the Earth, a __grid mapping variable__ [red aqua-background]#may# be provided by the data variable, using the `grid_mapping` attribute.
If the coordinate variables for a horizontal grid are not longitude and latitude, then a grid_mapping variable provides the information [red aqua-background]#required# to derive longitude and latitude values for each grid location.
If no __grid mapping variable__ is referenced by a data variable, then longitude and latitude coordinate values [red aqua-background]#shall# be supplied __in addition__ to the [red aqua-background]#required# coordinates. 
For example, the Cartesian coordinates of a map projection [red aqua-background]#may# be supplied as coordinate variables and, in addition, two-dimensional latitude and longitude variables [red aqua-background]#may# be supplied via the `coordinates` attribute on a data variable. 
The use of the `axis` attribute with values `X` and `Y` is [red aqua-background]#recommended# for the coordinate variables (see <<coordinate-types>>).
...
The latitude and longitude coordinates of a horizontal grid that was not defined as a Cartesian product of latitude and longitude axes, [black aqua-background]#can# sometimes be represented using two-dimensional coordinate variables.
...
We [black aqua-background]#recommend# that this type of gridded data be stored using the compression scheme described in <<compression-by-gathering>>.
...
Notice that even if an application does not recognize the `compress` attribute, the grids stored in this format [black aqua-background]#can# still be handled, by an application that recognizes the `coordinates` attribute.
...
A __grid mapping variable__ [red aqua-background]#may# be referenced by a data variable  in order to explicitly declare the coordinate reference system (CRS) used for the horizontal spatial coordinate values.
For example, if the horizontal spatial coordinates are latitude and longitude, the grid mapping variable [black aqua-background]#can# be used to declare the figure of the earth (WGS84 ellipsoid, sphere, etc.) they are based on.
...
When the horizontal spatial coordinate variables are not longitude and latitude, it is [red aqua-background]#required# that further information is provided to geo-locate the horizontal position.
A __grid mapping variable__ provides this information.

If no __grid mapping variable__ is provided and the coordinate variables for a horizontal grid are not longitude and latitude, then it is [red aqua-background]#required# that the latitude and longitude coordinates are supplied via the `coordinates` attribute. 
Such coordinates [red aqua-background]#may# be provided in addition to the provision of a __grid mapping variable__, but that is not required.
...
The one attribute that all grid mapping variables [red aqua-background]#must# have is `grid_mapping_name`, which takes a string value that contains the mapping's name.
...
This attribute is attached to data variables so that variables with different mappings [red aqua-background]#may# be present in a single file.
...
The `grid_mapping` variable [red aqua-background]#may# identify datums (such as the reference ellipsoid, the geoid or the prime meridian) for horizontal or vertical coordinates.
Therefore a grid mapping variable [red aqua-background]#may# be needed when the coordinate variables for a horizontal grid are longitude and latitude.
The `grid_mapping_name` of `latitude_longitude` [red aqua-background]#should# be used in this case.

The expanded form of the `grid_mapping` attribute is [red aqua-background]#required# if one wants to store coordinate information for more than one coordinate reference system.
...
This syntax [red aqua-background]#may# be used to explicitly link coordinates and grid mapping variables where only one coordinate reference system is used.
...
All coordinate names listed in the `grid_mapping` attribute [red aqua-background]#must# be coordinate variables or auxiliary coordinates of the data variable.
...
A CF compliant application [black aqua-background]#can# determine that rlon and rlat are longitude and latitude values in the rotated grid by recognizing the standard names `grid_longitude` and `grid_latitude`.
Note that the units of the rotated longitude and latitude axes are given as `degrees`.
This [red aqua-background]#should# prevent a COARDS compliant application from mistaking the variables `rlon` and `rlat` to be actual longitude and latitude coordinates.
...
An application [black aqua-background]#can# determine that `x` and `y` are the projection coordinates by recognizing the standard names `projection_x_coordinate` and `projection_y_coordinate`.
...
An [red aqua-background]#optional# grid mapping attribute called `crs_wkt` [red aqua-background]#may# be used to specify multiple coordinate system properties in so-called __well-known text__ format (usually abbreviated to CRS WKT or OGC WKT).
...
The `crs_wkt` attribute [red aqua-background]#should# comprise a text string that conforms to the WKT syntax as specified in reference <<OGC_WKT-CRS>>.
If desired the text string [red aqua-background]#may# contain embedded newline characters to aid human readability.
...
The `grid_mapping` attribute is thus given a value `crs:lat lon` to define that where coordinate pairs are required, these [red aqua-background]#shall# be ordered (lat, lon), to be consistent with the provided `crs_wkt` string (and not order inverted). 
A 2-D array of (lat, lon) tuples [black aqua-background]#can# then be explicitly derived from the combination of the lat and lon variables.
...
Therefore the CRS [red aqua-background]#should# be described as thoroughly as possible with the single-property grid mapping attributes as well as by `crs_wkt`.

In cases where CRS property values [black aqua-background]#can# be represented by both a single-property grid mapping attribute and the `crs_wkt` attribute, the grid mapping [red aqua-background]#should# be provided, and if both are provided, the onus is on data producers to ensure that their property values are consistent.
Therefore information from either one (or both) [red aqua-background]#may# be read in by the user without needing to check both.
However, if the two values of a given property are different, the CRS information cannot be interpreted accurately and users [red aqua-background]#should# inform the provider so the issue [black aqua-background]#can# be addressed.
...
Likewise, in those cases where the value of a CRS WKT element [red aqua-background]#should# be used consistently across the CF-netCDF community (names of projections and projection parameters, for example) then, the values shown in
https://github.com/cf-convention/cf-conventions/wiki/Mapping-from-CF-Grid-Mapping-Attributes-to-CRS-WKT-Elements
...
Example 5.12 illustrates how certain WKT elements - all of which are [red aqua-background]#optional# - [black aqua-background]#can# be used to specify CRS properties not covered by existing CF grid mapping attributes, including:
...
The preceding two example (5.11 and 5.12) [red aqua-background]#may# be combined, if the data provider desires to provide explicit latitude and longitude coordinates as well as projection coordinates and to provide CRS WKT referencing for both sets of coordinates.
...
When a variable has an associated coordinate which is single-valued, that coordinate [red aqua-background]#may# be represented as a scalar variable (i.e. a data variable which has no netCDF dimensions).
Since there is no associated dimension these scalar coordinate variables [red aqua-background]#should# be attached to a data variable via the `coordinates` attribute.

The use of scalar coordinate variables is a convenience feature which avoids adding size one dimensions to variables.
A numeric scalar coordinate variable has the same information content and [black aqua-background]#can# be used in the same contexts as a size one numeric coordinate variable.
...
Once a name is used for a scalar coordinate variable it [black aqua-background]#can not# be used for a 1D coordinate variable.
For this reason we strongly [black aqua-background]#recommend# against using a name for a scalar coordinate variable that matches the name of any dimension in the file.
...
If two or more single-valued coordinates are not independent, but have related values (this might be the case, for instance, for time and forecast period, or vertical coordinate and model level number, <<alternative-coordinates>>), they [red aqua-background]#should# be stored as coordinate or auxiliary coordinate variables of the same size one dimension, not as scalar coordinate variables.
...
The dimensions of the domain [red aqua-background]#must# be stored with the `dimensions` attribute, and the presence of a `dimensions` attribute will identify the variable as a domain variable.
Therefore the `dimensions` attribute [red aqua-background]#must not# be present on any variables that are to be interpreted as data variables.
It is necessary to list these dimensions, rather than inferring them from the contents of the other attributes, as it [black aqua-background]#can not# be guaranteed that the referenced variables span all of the [red aqua-background]#required# dimensions (as [black aqua-background]#could# be the case for a discrete axis, for instance).
...
If a domain has no named dimensions then the value of the `dimensions` attribute [red aqua-background]#must# be an empty string, as [black aqua-background]#could# be the case if the dimensions of the domain are all defined implicitly by scalar coordinate variables.

The dimensions listed by the `dimensions` attribute constrain the dimensions that [red aqua-background]#may# be spanned by variables referenced from any of the other attributes, in the same way that the array dimensions perform that role for a data variable.
For instance, all variables named by the `cell_measures` attribute (<<cell-measures>>) of a domain variable [red aqua-background]#must# span a subset of zero or more of the dimensions given by the `dimensions` attribute.

It is [red aqua-background]#optional# for coordinate variables to be listed by a domain variable's `coordinates` attribute.
...
It is [red aqua-background]#recommended# that a domain variable has a `long_name` attribute to describe its contents.

It is [red aqua-background]#recommended# that a domain variable does not have any of the attributes marked in <<attribute-appendix>> as applicable to data variables except those which are also marked as applicable to domain variables.

Multiple domain variables [red aqua-background]#may# exist in a file with, or without, data variables.
Note that the data variable attributes describing its domain [black aqua-background]#can not# be replaced by a reference to a domain variable.
...
A mesh topology variable [red aqua-background]#may# provide the topology for one or more domains, defined at the nodes, edges, or faces of the mesh.
...
A data or domain variable [red aqua-background]#may# use one of a mesh topology variable's domains by referencing the mesh topology variable with the **`mesh`** attribute; along with the identity of [red aqua-background]#required# domain provided by the **`location`** attribute (see example <<example-mesh-topology-variable>>).

The variables containing the coordinate values for cells indexed by the mesh topology are defined by the mesh topology variable but are equivalent to one-dimensional auxiliary coordinate variables, and so [red aqua-background]#may# also be provided by the data or domain variable's **`coordinates`** attribute.
...
A two-dimensional UGRID mesh topology variable for the mesh depicted in <<figure-mesh-example>>, with data variables defined at face, edge and node elements of the mesh. All [red aqua-background]#optional# attributes have been omitted.
