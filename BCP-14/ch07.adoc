...
When gridded data does not represent the point values of a field but instead represents some characteristic of the field within cells of finite "volume," a complete description of the variable [red]#should# include metadata that describes the domain or extent of each cell, and the characteristic of the field that the cell values represent.
...
__A boundary variable will have one more dimension than its associated coordinate or auxiliary coordinate variable.__ The additional dimension [red]#should# be the most rapidly varying one, and its size is the maximum number of cell vertices.
...
It is [red]#recommended# that BI attributes not be included on a boundary variable.
If a BI attribute is included, it [red]#must# also be present in the parent variable, and it [red]#must# exactly match the parent attribute's data type and value.
A boundary variable can only have inheritable attributes if they are also present on its parent coordinate variable.
A bounds variable [red]#may# have any of the attributes marked "BO" for ("own") in the "Use" column of <<attribute-appendix>>.
...
If a parametric coordinate variable with a **`formula_terms`** attribute (section 4.3.2) also has a **`bounds`** attribute, its boundary variable [red]#must# have a **`formula_terms`** attribute too.
...
Whenever a **`formula_terms`** attribute is attached to a boundary variable, the formula terms [red]#may# additionally be identified using a second method: variables appearing in the vertical coordinates' **`formula_terms`** [red]#may# be declared to be coordinate, scalar coordinate or auxiliary coordinate variables, and those coordinates [red]#may# have **`bounds`** attributes that identify their boundary variables.
In that case, the **`bounds`** attribute of a formula terms variable [red]#must# be consistent with the **`formula_terms`** attribute of the boundary variable.
Software digesting legacy datasets (constructed prior to version 1.7 of this standard) [red]#may# have to rely in some cases on the first method of identifying the formula term variables and in other cases, on the second.
...
float eta(eta) ;
 ...
 float A(eta) ;
   ...
   A:units = "Pa" ;
   A:bounds = "A_bnds" ; // This attribute is included for the [red]#optional# second method
 float B(eta) ;
   B:long_name = "'b' coefficient for vertical coordinate at full levels" ;
   B:units = "1" ;
   B:bounds = "B_bnds" ; // This attribute is included for the [red]#optional# second method
 float A_bnds(eta, 2) ;
 float B_bnds(eta, 2) ;
 float PS(lat, lon) ;
   PS:units = "Pa" ;
 ...
 float temp(eta, lat, lon) ;
   ...
   temp:units = "K";
   temp:coordinates = "A B" ; // This attribute is included for the [red]#optional# second method
----
====
...
Bounds for 1-D coordinate variables:: For a coordinate variable such as **`lat(lat)`** with associated boundary variable **`latbnd(x,2)`**, the interval endpoints [red]#must# be ordered consistently with the associated coordinate, e.g., for an increasing coordinate, **`lat(1)`** &gt; **`lat(0)`** implies **`latbnd(i,1)`** &gt;= **`latbnd(i,0)`** for all **`i`** (<<img-bnd_1d_coords>>).
+
If adjacent intervals are contiguous, the shared endpoint [red]#must# be represented indentically in each instance where it occurs in the boundary variable.
...
Bounds for multi-dimensional coordinate variables with p-sided cells:: In all other cases, the bounds [red]#should# be dimensioned **`(...,n,p)`**, where **`(...,n)`** are the dimensions of the auxiliary coordinate variables, and **`p`** the number of vertices of the cells.
The vertices [red]#must# be traversed anticlockwise in the lon-lat plane as viewed from above.
...
The gridpoint location, `lat(i)`, [red]#should# be contained within this interval.
====
...
The gridpoint location, `(lat(j,i),lon(j,i))`, [red]#should# be contained within this region.
====
...
Area weights [red]#may# also be needed to map data from one grid to another in such a way as to preserve the area mean of the field.
The preservation of area-mean values while regridding [red]#may# be essential, for example, when calculating surface heat fluxes in an atmospheric model with a grid that differs from the ocean model grid to which it is coupled.
...
This is why it [red]#may# be necessary to store the grid-cell areas in addition to the cell vertices.
...
In this case, rather than (or in addition to) indicating grid cell area, it [red]#may# be necessary to indicate volume.

To indicate extra information about the spatial properties of a variable's grid cells, a **`cell_measures`** attribute [red]#may# be defined for a variable.
This is a string attribute comprising a list of blank-separated pairs of words of the form "**`measure: name`**".
For the moment, "**`area`**" and "**`volume`**" are the only defined measures, but others [red]#may# be supported in future.
The "name" is the name of the variable containing the measure values, which we refer to as a "measure variable".
The dimensions of a measure variable [red]#must# be the same as or a subset of the dimensions of the variable to which it is related, but their order is not restricted, and with one exception:
...
The variable [red]#must# have a **`units`** attribute and [red]#may# have other attributes such as a **`standard_name`**.
...
In this case supplying grid-cell areas via the **`cell_measures`** attribute is unnecessary because it [red]#may# be assumed that applications can perform this calculation, using their own value for the radius of the Earth.

A variable referenced by **`cell_measures`** is not [red]#required# to be present in the file containing the data variable.
If the **`cell_measures`** variable is located in another file (an "external file"), rather than in the file where it is referenced, it [red]#must# be listed in the **`external_variables`** attribute of the referencing file (Section 2.6.3).
...
The values of __method__ [red]#should# be selected from the list in <<appendix-cell-methods>>, which includes `point`, `sum`, `mean`, among others.
...
It [red]#must# be remembered that the method applies only to the axis designated in **`cell_methods`** by __name__, and different methods [red]#may# apply to other axes.
...
Furthermore, it [red]#should# be noted that if any __method__ other than "**`point`**" is specified for a given axis, then **`bounds`** [red]#should# also be provided for that axis (except for the relatively rare exceptions described in <<cell-methods-no-coordinates>>).
...
More often, however, cell values for intensive quantities are means, and this [red]#should# be indicated explicitly by setting the cell method to **`mean`** and specifying the cell bounds.

Because the default interpretation for an intensive quantity differs from that of an extensive quantity and because this distinction [red]#may# not be understood by some users of the data, it is [red]#recommended# that every data variable include for each of its dimensions and each of its scalar coordinate variables the **`cell_methods`** information of interest (unless this information would not be meaningful).
It is especially [red]#recommended# that **`cell_methods`** be explicitly specified for each spatio-temporal dimension and each spatio-temporal scalar coordinate variable.
...
An application [red]#may# simply obtain the midpoint values by making use of the boundary data in `time_bnds`.
====
...
If more than one cell method is to be indicated, they [red]#should# be arranged in the order they were applied.
...
If the methods could have been applied in any order without affecting the outcome, they [red]#may# be put in any order in the **`cell_methods`** attribute.

If a data value is representative of variation over a combination of axes, a single method [red]#should# be prefixed by the names of all the dimensions involved (listed in any order, since in this case the order [red]#must# be immaterial).
Dimensions [red]#should# be grouped in this way only if there is an essential difference from treating the dimensions individually.
...
To indicate variation over horizontal area, it is [red]#recommended# that instead of specifying the combination of horizontal dimensions, the special string "**`area`**" be used.
...
To indicate more precisely how the cell method was applied, extra information [red]#may# be included in parentheses () after the identification of the method.
...
If the cell method applies to a combination of axes, they [red]#may# have a common original interval e.g. **`cell_methods="lat: lon: standard_deviation (interval: 10 km)"`**.
Alternatively, they [red]#may# have separate intervals, which are matched to the names of axes by position e.g. **`cell_methods="lat: lon: standard_deviation (interval: 0.1 degree_N interval: 0.2 degree_E)"`**, in which 0.1 degree applies to latitude and 0.2 degree to longitude.
...
If there is no standardized information, the keyword **`comment:`** [red]#should# be omitted.
...
A dimension of size one [red]#may# be the result of "collapsing" an axis by some statistical operation, for instance by calculating a variance from time series data.
...
To indicate this, one of two conventions [red]#may# be used.

The first convention is a method that can be used for the common case of a single area-type.
In this case, the **`cell_methods`** attribute [red]#may# include a string of the form "__name: method__  **`where`**  __type__".
Here __name__ could, for example, be **`area`** and __type__ [red]#may# be any of the strings permitted for a variable with a **`standard_name`** of **`area_type`**.
...
If the data writer expects __type__ to be interpreted as one of the standard **`area_type`** strings, then none of the variables in the netCDF file [red]#should# be given a name identical to that of the string (because the second convention, described in the next paragraph, takes precedence).
...
Nevertheless, useful information can be conveyed by a **`cell_methods`** entry of "**`time: mean`**" (where **`time`**, it [red]#should# be noted, is a valid **`standard_name`**).
(As [red]#required# by this convention, it is assumed here that for the data referred to by this **`cell_methods`** attribute, "time" is not a dimension or coordinate variable.)
...
We recommend that whenever possible, cell bounds [red]#should# be supplied by giving the variable a dimension of size one and attaching bounds to the associated coordinate variable.
...
Climatological statistics [red]#may# be derived from corresponding portions of the annual cycle in a set of years, e.g., the average January temperatures in the climatology of 1961-1990, where the values are derived by averaging the 30 Januarys from the separate years.
...
The time coordinates [red]#should# be values that are representative of the climatological time intervals, such that an application which does not recognise climatological time will nonetheless be able to make a reasonable interpretation.

For compatibility with the COARDS standard, a climatological time coordinate in the default **`standard`** and **`julian`** calendars [red]#may# be indicated by setting the date/time reference string in the time coordinate's **`units`** attribute to midnight on 1 January in year 0 (i.e., **`since 0-1-1`**).
This convention is deprecated because it does not provide any information about the intervals used to compute the climatology, and there [red]#may# be inconsistencies among software packages in the interpretation of the time coordinates with a reference time of year 0.
Use of year 0 for this purpose is impossible in all other calendars, because year 0 is a valid year.

A climatological axis [red]#may# use different statistical methods to represent variation among years, within years and within days.
...
As usual, the statistical operations are recorded in the **`cell_methods`** attribute, which [red]#may# have two or three entries for the climatological time dimension.

Valid values of the **`cell_methods`** attribute [red]#must# be in one of the forms from the following list.
...
(The date and time specifications [red]#must# be calculated from the time coordinates expressed in units of "time interval since reference date and time".)
...
The methods which can be specified are those listed in <<appendix-cell-methods>> and each entry in the **`cell_methods`** attribute [red]#may# also, as usual, contain non-standardised information in parentheses after the method.
...
Polygonal cells with an arbitrary number of vertices can be described using <<cell-boundaries>>, but in that case every cell [red]#must# have the same number of vertices and [red]#must# be a single polygon ring.
In contrast, each geometry [red]#may# have a different number of nodes, the geometries [red]#may# be lines (as alternatives to points and polygons), and they [red]#may# be __multipart__, i.e., include several disjoint parts.
...
One of the dimensions of the data variable [red]#must# be the number of geometries to which the data applies.
...
The geometry node coordinate variables [red]#must# each have an **`axis`** attribute whose allowable values are __X__, __Y__, and __Z__.
...
If a different grid mapping is used, then the provided coordinates [red]#must not# have the **`nodes`** attribute.

Whether linked to normal CF space-time coordinates with a **`nodes`** attribute or not, inclusion of such coordinates is [red]#recommended# to maintain backward compatibility with software that has not implemented geometry capabilities.
...
The nodes [red]#must# be stored consecutively for each geometry and in the order of the geometries, and within each multipart geometry the nodes [red]#must# be stored consecutively for each part and in the order of the parts.
Polygon exterior rings [red]#must# be stored before any interior rings they [red]#may# contain.
Nodes for polygon exterior rings [red]#must# be ordered using the right-hand rule, e.g., anticlockwise in the lon-lat plane as viewed from above.
Polygon interior rings [red]#must# be in clockwise order.
...
When more than one geometry instance is present, the geometry container variable [red]#must# have a **`node_count`** attribute that contains the name of a variable indicating the count of nodes per geometry.
...
However in that case, the dimension of the node coordinate variables [red]#must# be one of the dimensions of the data variable (because it serves also as the instance dimension for geometries).

For multipart __lines__, multipart __polygons__, and __polygons__ with holes, the geometry container variable [red]#must# have a **`part_node_count`** attribute that indicates a variable of the count of nodes per geometry part.
Note that because multipoint geometries always have a single node per part, the **`part_node_count`** is not [red]#required# for __point__ geometry types.
The single dimension of the part node count variable [red]#must# equal the total number of parts in all the geometries.

For __polygon__ geometries with holes, the geometry container variable [red]#must# have an **`interior_ring`** attribute that contains the name of a variable that indicates if the polygon parts are interior rings (i.e., holes) or not.
This interior ring variable [red]#must# contain the value 0 to indicate an exterior ring polygon and 1 to indicate an interior ring polygon.
...
[cols="4"]
|===============
| geometry_type | Dimensionality | Description of Geometry Instance | Additional [red]#required# attributes on geometry container variable
...
**Table 7.1.** Dimensionality, description, and additional [red]#required# attributes for geometry_types.
