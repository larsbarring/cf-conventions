...
The CF attributes listed here [aqua-background]#may# be used to define mesh topologies (<<mesh-topology-variables>>).
This list is intended as a summary of the attributes that have been standardized via the UGRID conventions <<UGRID>>, which [aqua-background]#should# be consulted for further details.
...
| S
| MT
| Specifies the auxiliary coordinate variables representing the node locations (latitude, longitude, or other spatial coordinates, and [aqua-background]#optional# elevation or other coordinates).
