...
Each section contains: the valid name that is used with the **`grid_mapping_name`** attribute; a list of the specific attributes that [aqua-background]#may# be used to assign values to the mapping's parameters; the standard names used to identify the coordinate variables that contain the mapping's independent variables; and references to the mapping's definition or other information that [aqua-background]#may# help in using the mapping.
Since the attributes used to set a mapping's parameters [aqua-background]#may# be shared among several mappings, their definitions are contained in a table in the final section.
The attributes which describe the ellipsoid and prime meridian [aqua-background]#may# be included, when applicable, with any grid mapping.
These are:
...
__Map parameters:__::
* **`standard_parallel`** - There [aqua-background]#may# be 1 or 2 values.
* **`longitude_of_central_meridian`**
* **`latitude_of_projection_origin`**
* **`false_easting`** - This parameter is [aqua-background]#optional# (default is 0)
* **`false_northing`** - This parameter is [aqua-background]#optional# (default is 0)
...
__Notes:__:: Notes on using the **`PROJ`** software package for computing the mapping [aqua-background]#may# be found at
...
+
The algorithm for computing the mapping [aqua-background]#may# be found at link:$$https://www.cgms-info.org/documents/pdf_cgms_03.pdf$$[https://www.cgms-info.org/documents/pdf_cgms_03.pdf].
This document assumes the point of observation is directly over the equator, and that the **`sweep_angle_axis`** is y.

+
Explanatory diagrams for the projection [aqua-background]#may# be found link:$$https://proj.org/operations/projections/geos.html$$[on the PROJ website], as well as notes on using the PROJ software for computing the mapping.
...
Only one of those the attributes **`fixed_angle_axis`** or **`sweep_angle_axis`** is mandatory, as they [lime-background]#can# be used to infer each other.
...
The initial definition of this projection used these standard names to identify the projection coordinates even though their canonical units (meters) do not match those [aqua-background]#required# for this projection (radians).
...
* **`refinement_level`** - The HEALPix refinement level: Mandatory for indexing scheme **`nested`** or **`ring`**, but [aqua-background]#must# be omitted for indexing scheme **`nuniq`** or **`zuniq`**
...
The HEALPix index coordinates [aqua-background]#may# be stored in a coordinate, auxiliary, or scalar coordinate variable. Indexing schemes are optimized to [lime-background]#allow# applications efficient data access when the HEALPix indices are ordered strictly monotonically.
Therefore it is [aqua-background]#recommended# to store strictly monotonic HEALPix indices in a coordinate variable, which [lime-background]#allows# an application to assess their monotonicity without expensive checking.
...
The lowest [lime-background]#allowed# refinement level is `0`, giving 12 cells globally.
...
If the **`healpix`** grid mapping is used on an ellipsoid, the latitudes returned by the <<GHB05>> algorithm [aqua-background]#must# be interpreted as authalic latitude, instead of geodetic latitude (for a sphere, the two latitudes are the same).
...
+
The **`nuniq`** and **`zuniq`** indexing schemes [lime-background]#allow# pixels from multiple refinement levels to be combined: **`nuniq`** effectively concatenates **`nested`** indices from lower to the higher refinement levels, resulting in spatial proximity for consecutive pixels within a refinement level but not across different refinement levels; and **`zuniq`** indexing sorts pixels of any refinement level along the Z-order curve, resulting in spatial proximity for consecutive pixels, regardless of their refinement level.
+
An index according to the **`nuniq`** indexing scheme (`index_nuniq`) [lime-background]#can# be computed from a refinement level and an index according to the **`nested`** indexing scheme (`index_nested`) with the formula `index_nuniq = index_nested + 4 ^ (1 + refinement_level)` <<RH15>>.
+
An index according to the **`zuniq`** indexing scheme (`index_zuniq`) [lime-background]#can# be computed from a refinement level and an index according to the  **`nested`** indexing scheme (`index_nested`) with the formula `index_zuniq = (2 * index_nested + 1) * 4 ^ (29 - refinement_level)` <<K24>>.
+
The  **`refinement_level`** parameter [aqua-background]#must# be omitted for the **`nuniq`** and **`zuniq`** schemes, for which the resolution of a cell is implied by the index alone.
+
A refinement level, either given by the  **`refinement_level`** parameter or implied by **`nuniq`** and **`zuniq`** indices, [aqua-background]#must# be no greater than `29`, which is the highest refinement level for which all indices of any indexing scheme [lime-background]#can# be represented by 64-bit integers.
...
__Notes:__:: Notes on using the **`PROJ`** software package for computing the mapping [aqua-background]#may# be found at link:$$https://proj.org/operations/projections/lcc.html$$[https://proj.org/operations/projections/lcc.html].
and
...
__Notes:__:: Notes on using the **`PROJ`** software packages for computing the mapping [aqua-background]#may# be found at
...
Detailed formulas [lime-background]#can# be found in <<Snyder>> pages 76-85.
...
It is included so that the figure of the Earth [lime-background]#can# be described.
...
As PROJ [lime-background]#requires# the standard parallel, <<Snyder>> formula 21-7 [lime-background]#can# be used to compute it from the scale factor if needed.
...
* **`grid_subtype`** - Reduced Gaussian grid type.
The [lime-background]#permitted# values (case-insensitive) are **`normal`** and **`octahedral`**.
* **`latitude_dimension`** - The name of both a one-dimensional variable, which contains the latitude coordinates of the latitude lines, and of its single dimension, which specifies the number of latitude lines _N_, where _N_ [aqua-background]#must# be even because these grids do not have points at the poles and equator.
* Either **`points_per_latitude`** or **`accumulated_points_per_latitude`** - Whichever is given, it contains the name of a one-dimensional variable, whose single dimension is identified by the **`latitude_dimension`** attribute.
The **`points_per_latitude`** and **`accumulated_points_per_latitude`** variables are equivalent; the latter is the cumulative sum of the former, and [aqua-background]#may# ease user computations of latitudes and longitudes.
* **`longitude_of_first_meridian`** - This parameter is [aqua-background]#optional# (default is 0)

__Map coordinates:__::

In addition to the map parameters, the coordinate system is defined by a one-dimensional variable whose single dimension is identified by the **`latitude_dimension`** attribute, and whose name is the same as the name of its dimension.
It [aqua-background]#must# have a **`units`** attribute and [aqua-background]#may# have other attributes to identify it as containing latitude coordinates (see Section 4.2).
The latitudes correspond to the zeroes of the Legendre polynomial of order _N_.
+
The data variable [aqua-background]#must# have an integer-valued one-dimensional coordinate variable with **`standard_name`** attribute value **`reduced_gaussian_index`**.
This is a discrete index with a unique value to identify each gridpoint.
The **`reduced_gaussian_index`**, together with the numbers of longitude points for each latitude line and the **`latitude`** values, [lime-background]#can# be used to reconstruct the latitude and longitude of each of the gridpoints.
+
The **`reduced_gaussian_index`** is strictly monotonic and non‑negative.
It is a serial index for the points of the grid, ordered east to west and north to south, starting from 0 on the closest latitude line to the North Pole, proceeding eastward along each latitude line for the number of points specified for that latitude, and continuing to the last point on the latitude line closest to the South Pole.
The last index is _M_-1, where _M_ is number of points in the global reduced Gaussian grid.
Along each latitude line, the first point is at the longitude given by the **`longitude_of_first_meridian`** (by default 0 degrees East), and the points are evenly spaced.
+
The **`latitude`** and **`longitude`** coordinates for each cell [lime-background]#can# be optionally included as 1D auxiliary coordinate variables.

__Notes:__::

The reduced Gaussian grid is customarily identified by a nominal grid resolution.
This number is half the number of latitudes _N_/2, equal to the number of latitudes in each hemisphere between pole and equator e.g. grid resolution of 1280 means that the size of the dimension named by the **`latitude_dimension`** attribute is _N_=2560.
+
The **`grid_subtype`** (e.g., Octahedral, Normal) and the number of latitudes from pole to equator (the nominal resolution) together identify the grid.
For more information on the grid type, see `<<RGG>>` and `<<RGG-O>>`.
+
For a non-global domain (regional or selective e.g. land only), the number of reduced Gaussian gridpoints is smaller than _M_ for the global grid.
The **`reduced_gaussian_index`** coordinate variable locates the points by their reduced Gaussian index on the global grid.
The map parameters, the latitude variable, and the variable specifying the numbers of longitudes always refer to the global grid.
...
variables:
   char reduced_gaussian ;
        ...
        reduced_gaussian:latitude_dimension = "lat" ;
   float lat(lat) ;
   ...
   int reduced_gaussian_index(reduced_gaussian_index) ;
        reduced_gaussian_index:standard_name = "reduced_gaussian_index"
...
* **`north_pole_grid_longitude`** - This parameter is [aqua-background]#optional# (default is 0).
...
Detailed formulas [lime-background]#can# be found in <<Snyder>>, pages 243-248.
...
__Notes:__:: Formulas for the mapping and its inverse along with notes on using the **`PROJ`** software package for doing the calcuations [aqua-background]#may# be found at
link:$$https://proj.org/operations/projections/stere.html$$[https://proj.org/operations/projections/stere.html]
and
...
This [aqua-background]#should not# be confused with the PROJ geos projection.
...
| **`crs_wkt`** | S
| This [aqua-background]#optional# attribute [aqua-background]#may# be used to specify multiple coordinate system properties in well-known text (WKT) format.
The syntax [aqua-background]#must# conform to the WKT format as specified in reference <<OGC_WKT-CRS>>.
...
This attribute [aqua-background]#should# be specified for those projected coordinate reference systems in which the X-Y cartesian coordinates have been derived using a spherical Earth approximation.
If the cartesian coordinates were derived using an ellipsoid, this attribute [aqua-background]#should not# be defined.
...
It corresponds to the inner-gimbal axis of the gimbal view model, whose axis of rotation moves about the outer-gimbal axis.
This value [lime-background]#can# adopt two values, "x" or "y", corresponding with the Earth's E-W or N-S axis, respectively.
The counterpart to this attribute is `sweep_angle_axis`.
If set to "x", `sweep_angle_axis` is "y", and vice versa.
If one of the attributes `fixed_angle_axis` or `sweep_angle_axis` is provided, the other is not mandatory, as they [lime-background]#can# be used to infer each other.
...
| **`grid_subtype`** | N
| Reduced Gaussian grid type (case-insensitive). 
Normal or octahedral.
...
This is to ensure that named datums [lime-background]#can# be correctly identified for precise datum transformations (see <<CF-WKT>> for more details).
...
In the case of a spherical Earth this attribute [aqua-background]#should# be omitted or set to zero.
...
If all three attributes are specified then the supplied values [aqua-background]#must# be consistent with the aforementioned formula.)

| **`latitude_dimension`** | N
| The number of latitude lines from pole to equator (N) in a reduced Gaussian grid. 
The full grid therefore contains 2N latitude lines. 
N [aqua-background]#must# be an even number.
...
| Specifies the longitude, with respect to Greenwich, of the first meridian used to define the grid mapping indexing in a reduced Gaussian grid. 
It is 0 degrees by default.
...
Used by perspective-type map projections, for example the Vertical Perspective Projection, which [aqua-background]#may# be used to simulate the view from a Meteosat satellite.
...
In the case of a spherical Earth approximation this attribute [aqua-background]#should# be omitted (the preferred option) or else set equal to the value of the semi_major_axis attribute.
See also the inverse_flattening attribute.
...
| **`towgs84`** | N
|   This indicates a list of up to 7 Bursa Wolf transformation parameters., which [lime-background]#can# be used to approximate a transformation from the horizontal datum to the WGS84 datum.
More precise datum transformations [lime-background]#can# be done with datum shift grids.
...
. The various **`*_name`** attributes are [aqua-background]#optional# but [aqua-background]#recommended# when known as they [lime-background]#allow# for a better description and interoperability with WKT definitions.
. **`reference_ellipsoid_name`**, **`prime_meridian_name`**, **`horizontal_datum_name`** and **`geographic_crs_name`** [aqua-background]#must# be all defined if any one is defined, and if **`projected_crs_name`** is defined then **`geographic_crs_name`** [aqua-background]#must# be also.
