...
CF specified the standard names **`projection_x_coordinate`** and **`projection_y_coordinate`** for these coordinates prior to version 1.9, but that use is [lime-background]#deprecated#.
...
+
The use of `projection_x_coordinate` and `projection_y_coordinate` was [lime-background]#deprecated# in version 1.9 of the CF Conventions.
...
Detailed formulas [lime-background]#can# be found in <<bibliography.adoc#Snyder>> pages 76-85.
...
This is to ensure that named datums [lime-background]#can# be correctly identified for precise datum transformations  (see https://github.com/cf-convention/cf-conventions/wiki/OGC-WKT-Coordinate-System-Issues for more details).
...
. The various **`*_name`** attributes are [aqua-background]#optional# but [aqua-background]#recommended# when known as they allow for a better description and interoperability with WKT definitions.
