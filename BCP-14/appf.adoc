...
Each section contains: the valid name that is used with the **`grid_mapping_name`** attribute; a list of the specific attributes that [red]#may# be used to assign values to the mapping's parameters; the standard names used to identify the coordinate variables that contain the mapping's independent variables; and references to the mapping's definition or other information that [red]#may# help in using the mapping.
Since the attributes used to set a mapping's parameters [red]#may# be shared among several mappings, their definitions are contained in a table in the final section.
The attributes which describe the ellipsoid and prime meridian [red]#may# be included, when applicable, with any grid mapping.
These are:
...
__Map parameters:__::
* **`standard_parallel`** - There [red]#may# be 1 or 2 values.
* **`longitude_of_central_meridian`**
* **`latitude_of_projection_origin`**
* **`false_easting`** - This parameter is [red]#optional# (default is 0)
* **`false_northing`** - This parameter is [red]#optional# (default is 0)
...
__Notes:__:: Notes on using the **`PROJ`** software package for computing the mapping [red]#may# be found at
...
+
The algorithm for computing the mapping [red]#may# be found at link:$$https://www.cgms-info.org/documents/pdf_cgms_03.pdf$$[https://www.cgms-info.org/documents/pdf_cgms_03.pdf].
This document assumes the point of observation is directly over the equator, and that the **`sweep_angle_axis`** is y.

+
Explanatory diagrams for the projection [red]#may# be found link:$$https://proj.org/operations/projections/geos.html$$[on the PROJ website], as well as notes on using the PROJ software for computing the mapping.
...
The initial definition of this projection used these standard names to identify the projection coordinates even though their canonical units (meters) do not match those [red]#required# for this projection (radians).
...
__Notes:__:: Notes on using the **`PROJ`** software package for computing the mapping [red]#may# be found at link:$$https://proj.org/operations/projections/lcc.html$$[https://proj.org/operations/projections/lcc.html].
and
...
__Notes:__:: Notes on using the **`PROJ`** software packages for computing the mapping [red]#may# be found at
...
* **`north_pole_grid_longitude`** - This parameter is [red]#optional# (default is 0).
...
__Notes:__:: Formulas for the mapping and its inverse along with notes on using the **`PROJ`** software package for doing the calcuations [red]#may# be found at
link:$$https://proj.org/operations/projections/stere.html$$[https://proj.org/operations/projections/stere.html]
and
...
This [red]#should not# be confused with the PROJ geos projection.
...
| **`crs_wkt`** | S
| This [red]#optional# attribute [red]#may# be used to specify multiple coordinate system properties in well-known text (WKT) format.
The syntax [red]#must# conform to the WKT format as specified in reference <<OGC_WKT-CRS>>.
...
This attribute [red]#should# be specified for those projected coordinate reference systems in which the X-Y cartesian coordinates have been derived using a spherical Earth approximation.
If the cartesian coordinates were derived using an ellipsoid, this attribute [red]#should not# be defined.
...
In the case of a spherical Earth this attribute [red]#should# be omitted or set to zero.
...
If all three attributes are specified then the supplied values [red]#must# be consistent with the aforementioned formula.)
...
Used by perspective-type map projections, for example the Vertical Perspective Projection, which [red]#may# be used to simulate the view from a Meteosat satellite.
...
In the case of a spherical Earth approximation this attribute [red]#should# be omitted (the preferred option) or else set equal to the value of the semi_major_axis attribute.
See also the inverse_flattening attribute.
...
| __Deprecated__. Has the same meaning as **`longitude_of_projection_origin`**, which [red]#must# be used instead.
...
. The various **`*_name`** attributes are [red]#optional# but [red]#recommended# when known as they allow for a better description and interoperability with WKT definitions.
. **`reference_ellipsoid_name`**, **`prime_meridian_name`**, **`horizontal_datum_name`** and **`geographic_crs_name`** [red]#must# be all defined if any one is defined, and if **`projected_crs_name`** is defined then **`geographic_crs_name`** [red]#must# be also.
