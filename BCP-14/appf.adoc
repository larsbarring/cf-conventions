...
Each section contains: the valid name that is used with the **`grid_mapping_name`** attribute; a list of the specific attributes that [red aqua-background]#may# be used to assign values to the mapping's parameters; the standard names used to identify the coordinate variables that contain the mapping's independent variables; and references to the mapping's definition or other information that [red aqua-background]#may# help in using the mapping.
Since the attributes used to set a mapping's parameters [red aqua-background]#may# be shared among several mappings, their definitions are contained in a table in the final section.
The attributes which describe the ellipsoid and prime meridian [red aqua-background]#may# be included, when applicable, with any grid mapping.
These are:
...
__Map parameters:__::
* **`standard_parallel`** - There [red aqua-background]#may# be 1 or 2 values.
* **`longitude_of_central_meridian`**
* **`latitude_of_projection_origin`**
* **`false_easting`** - This parameter is [red aqua-background]#optional# (default is 0)
* **`false_northing`** - This parameter is [red aqua-background]#optional# (default is 0)
...
__Notes:__:: Notes on using the **`PROJ`** software package for computing the mapping [red aqua-background]#may# be found at
...
+
The algorithm for computing the mapping [red aqua-background]#may# be found at link:$$https://www.cgms-info.org/documents/pdf_cgms_03.pdf$$[https://www.cgms-info.org/documents/pdf_cgms_03.pdf].
This document assumes the point of observation is directly over the equator, and that the **`sweep_angle_axis`** is y.

+
Explanatory diagrams for the projection [red aqua-background]#may# be found link:$$https://proj.org/operations/projections/geos.html$$[on the PROJ website], as well as notes on using the PROJ software for computing the mapping.
...
Only one of those the attributes **`fixed_angle_axis`** or **`sweep_angle_axis`** is mandatory, as they [black aqua-background]#can# be used to infer each other.
Note also that the values "x" and "y" are not case-sensitive.

+
The use of `projection_x_coordinate` and `projection_y_coordinate` was [black aqua-background]#deprecated# in version 1.9 of the CF Conventions.
The initial definition of this projection used these standard names to identify the projection coordinates even though their canonical units (meters) do not match those [red aqua-background]#required# for this projection (radians).
...
__Notes:__:: Notes on using the **`PROJ`** software package for computing the mapping [red aqua-background]#may# be found at link:$$https://proj.org/operations/projections/lcc.html$$[https://proj.org/operations/projections/lcc.html].
and
...
__Notes:__:: Notes on using the **`PROJ`** software packages for computing the mapping [red aqua-background]#may# be found at
...
Detailed formulas [black aqua-background]#can# be found in <<bibliography.adoc#Snyder>> pages 76-85.
...
It is included so that the figure of the Earth [black aqua-background]#can# be described.
...
As PROJ requires the standard parallel, [Snyder] formula 21-7 [black aqua-background]#can# be used to compute it from the scale factor if needed.
...
* **`north_pole_grid_longitude`** - This parameter is [red aqua-background]#optional# (default is 0).
...
Detailed formulas [black aqua-background]#can# be found in <<Snyder>>, pages 243-248.
...
__Notes:__:: Formulas for the mapping and its inverse along with notes on using the **`PROJ`** software package for doing the calcuations [red aqua-background]#may# be found at
link:$$https://proj.org/operations/projections/stere.html$$[https://proj.org/operations/projections/stere.html]
and
...
This [red aqua-background]#should not# be confused with the PROJ geos projection.
...
| **`crs_wkt`** | S
| This [red aqua-background]#optional# attribute [red aqua-background]#may# be used to specify multiple coordinate system properties in well-known text (WKT) format.
The syntax [red aqua-background]#must# conform to the WKT format as specified in reference <<OGC_WKT-CRS>>.
...
This attribute [red aqua-background]#should# be specified for those projected coordinate reference systems in which the X-Y cartesian coordinates have been derived using a spherical Earth approximation.
If the cartesian coordinates were derived using an ellipsoid, this attribute [red aqua-background]#should not# be defined.
...
It corresponds to the inner-gimbal axis of the gimbal view model, whose axis of rotation moves about the outer-gimbal axis.
This value [black aqua-background]#can# adopt two values, "x" or "y", corresponding with the Earth's E-W or N-S axis, respectively.
The counterpart to this attribute is `sweep_angle_axis`.
If set to "x", `sweep_angle_axis` is "y", and vice versa.
If one of the attributes `fixed_angle_axis` or `sweep_angle_axis` is provided, the other is not mandatory, as they [black aqua-background]#can# be used to infer each other.
...
This is to ensure that named datums [black aqua-background]#can# be correctly identified for precise datum transformations  (see https://github.com/cf-convention/cf-conventions/wiki/OGC-WKT-Coordinate-System-Issues for more details).
...
In the case of a spherical Earth this attribute [red aqua-background]#should# be omitted or set to zero.
...
If all three attributes are specified then the supplied values [red aqua-background]#must# be consistent with the aforementioned formula.)
...
Used by perspective-type map projections, for example the Vertical Perspective Projection, which [red aqua-background]#may# be used to simulate the view from a Meteosat satellite.
...
In the case of a spherical Earth approximation this attribute [red aqua-background]#should# be omitted (the preferred option) or else set equal to the value of the semi_major_axis attribute.
See also the inverse_flattening attribute.
...
| **`towgs84`** | N
|   This indicates a list of up to 7 Bursa Wolf transformation parameters., which [black aqua-background]#can# be used to approximate a transformation from the horizontal datum to the WGS84 datum.
More precise datum transformations [black aqua-background]#can# be done with datum shift grids.
...
. The various **`*_name`** attributes are [red aqua-background]#optional# but [red aqua-background]#recommended# when known as they allow for a better description and interoperability with WKT definitions.
. **`reference_ellipsoid_name`**, **`prime_meridian_name`**, **`horizontal_datum_name`** and **`geographic_crs_name`** [red aqua-background]#must# be all defined if any one is defined, and if **`projected_crs_name`** is defined then **`geographic_crs_name`** [red aqua-background]#must# be also.
