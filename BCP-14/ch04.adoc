...
The commonest use of coordinate variables is to locate the data in space and time, but coordinates [red aqua-background]#may# be provided for any other continuous geophysical quantity (e.g. density, temperature, radiation wavelength, zenith angle of radiance, sea surface wave frequency) or discrete category (see <<discrete-axis>>, e.g. area type, model level number, ensemble member number) on which the data variable depends.
...
Because identification of a coordinate type by its units is complicated by requiring the use of an external package <<UDUNITS>>, we provide two [red aqua-background]#optional# methods that yield a direct identification.
The attribute **`axis`** [red aqua-background]#may# be attached to a coordinate variable and given one of the values **`X`**, **`Y`**, **`Z`** or **`T`** which stand for a longitude, latitude, vertical, or time axis respectively.
Alternatively the **`standard_name`** attribute [red aqua-background]#may# be used for direct identification.
But note that these [red aqua-background]#optional# attributes are in addition to the [red aqua-background]#required# COARDS metadata.

To identify generic spatial coordinates we [black aqua-background]#recommend# that the **`axis`** attribute be attached to these coordinates and given one of the values **`X`**, **`Y`** or **`Z`**.
The values **`X`** and **`Y`** for the axis attribute [red aqua-background]#should# be used to identify horizontal coordinate variables.
If both X- and Y-axis are identified, **`X-Y-up`** [red aqua-background]#should# define a right-handed coordinate system, i.e. rotation from the positive X direction to the positive Y direction is anticlockwise if viewed from above.
We strongly [black aqua-background]#recommend# that coordinate variables be used for all coordinate types whenever they are applicable.
...
Variables representing latitude [red aqua-background]#must# always explicitly include the **`units`** attribute; there is no default value.
The [red aqua-background]#recommended# value of the **`units`** attribute is the string **`degrees_north`**. Also accepted are **`degree_north`**, **`degree_N`**, **`degrees_N`**, **`degreeN`**, and **`degreesN`**.
...
Application writers [red aqua-background]#should# note that the UDUNITS package does not recognize the directionality implied by the "north" part of the unit specification.
It only recognizes its size, i.e., 1 degree is defined to be pi/180 radians.
Hence, determination that a coordinate is a latitude type [red aqua-background]#should# be done via a string match between the given unit and one of the acceptable forms of **`degrees_north`**.


Optionally, the latitude type [red aqua-background]#may# be indicated additionally by providing the **`standard_name`** attribute with the value **`latitude`**, and/or the **`axis`** attribute with the value **`Y`**.

Coordinates of latitude with respect to a rotated pole [red aqua-background]#should# be given units of **`degrees`**, not **`degrees_north`** or equivalents, because applications which use the units to identify axes would have no means of distinguishing such an axis from real latitude, and might draw incorrect coastlines, for instance.

[[longitude-coordinate, Section 4.2, "Longitude Coordinate"]]
=== Longitude Coordinate

Variables representing longitude [red aqua-background]#must# always explicitly include the **`units`** attribute; there is no default value.
The [red aqua-background]#recommended# value of the **`units`** attribute is the string **`degrees_east`**. Also accepted are **`degree_east`**, **`degree_E`**, **`degrees_E`**, **`degreeE`**, and **`degreesE`**.
...
Application writers [red aqua-background]#should# note that the UDUNITS package has limited recognition of the directionality implied by the "east" part of the unit specification.
It defines **`degrees_east`** to be pi/180 radians, and hence equivalent to **`degrees_north`**.
We [black aqua-background]#recommend# the determination that a coordinate is a longitude type [red aqua-background]#should# be done via a string match between the given unit and one of the acceptable forms of **`degrees_east`**.

Optionally, the longitude type [red aqua-background]#may# be indicated additionally by providing the **`standard_name`** attribute with the value **`longitude`**, and/or the **`axis`** attribute with the value **`X`**.

Coordinates of longitude with respect to a rotated pole [red aqua-background]#should# be given units of **`degrees`**, not **`degrees_east`** or equivalents, because applications which use the units to identify axes would have no means of distinguishing such an axis from real longitude, and might draw incorrect coastlines, for instance.

[[vertical-coordinate, Section 4.3, "Vertical Coordinate"]]
=== Vertical (Height or Depth) Coordinate

Variables representing dimensional height or depth axes [red aqua-background]#must# always explicitly include the **`units`** attribute; there is no default value.
...
For this reason the attribute **`positive`** as defined in the COARDS standard is [red aqua-background]#required# if the vertical axis units are not a valid unit of pressure (as determined by the UDUNITS package <<UDUNITS>>) -- otherwise its inclusion is optional.
The **`positive`** attribute [red aqua-background]#may# have the value **`up`** or **`down`** (case insensitive).
This attribute [red aqua-background]#may# be applied to either coordinate variables or auxiliary coordinate variables that contain vertical coordinate data.
...
Optionally, the vertical type [red aqua-background]#may# be indicated additionally by providing the **`standard_name`** attribute with an appropriate value, and/or the **`axis`** attribute with the value **`Z`**. 
If both **`positive`** and **`standard_name`** are provided, it is [red aqua-background]#recommended# that they [red aqua-background]#should# be consistent. 
...
If an application detects such an inconsistency, the user [red aqua-background]#should# be warned, and the **`positive`** attribute [red aqua-background]#should# be used to determine the sign convention.

Recommendations:  The **`positive`** attribute [red aqua-background]#should# be consistent with the sign convention implied by the definition of the **`standard_name`**, if both are provided.

==== Dimensional Vertical Coordinate


Variables representing dimensional vertical coordinates for or height [red aqua-background]#must# always explicitly include the  **`units`** attribute.
The acceptable units for a vertical (depth or height) coordinate variable [red aqua-background]#must# a UDUNITS <<UDUNITS>> representation of one of the following:
...
* other units that [red aqua-background]#may# under certain circumstances reference vertical position such as units of density or temperature.
...
The **`units`** attribute is not [red aqua-background]#required# for dimensionless coordinates.
...
These values are not recognized by the UDUNITS package, and are considered a [black aqua-background]#deprecated# feature in the CF standard.
...
The `standard_name` of the parametric (usually dimensionless) vertical coordinate variable [black aqua-background]#can# be used to find the definition of the associated computed (always dimensional) vertical coordinate in <<parametric-v-coord>>.
The definition provides a mapping between the parametric vertical coordinate values and computed values that [black aqua-background]#can# positively and uniquely indicate the location of the data.
The `formula_terms` attribute [black aqua-background]#can# be used to associate terms in the definitions with variables in a netCDF file, and the `computed_standard_name` attribute [black aqua-background]#can# be used to supply the `standard_name` of the computed vertical coordinate values computed according to the definition.
To maintain backwards compatibility with COARDS the use of these attributes is not required, but is strongly recommended.
Some of the definitions [red aqua-background]#may# be supplemented with information stored in the `grid_mapping` variable about the datum used as a vertical reference (e.g. geoid, other geopotential datum or reference ellipsoid; see <<grid-mappings-and-projections>> and <<appendix-grid-mappings>>).
...
Variables representing reference time [red aqua-background]#must# always explicitly include the **`units`** attribute; there is no default value.
The **`units`** attribute takes a string value that follows the formatting requirements of the <<UDUNITS>> package. These requirements [black aqua-background]#can# best be described by an example with explanatory comments:
...
The time zone specification [black aqua-background]#can# also be written without a colon using one or two digits (indicating hours) or three or four digits (indicating hours and minutes).
...
It [red aqua-background]#may# include date alone, or date and time, or date, time and time zone.
...
The CF standard follows UDUNITS in the definition of units, but we [black aqua-background]#recommend# that **`year`** and **`month`** [red aqua-background]#should not# be used, because of the potential for mistakes and confusion.
...
Optionally, the time coordinate [red aqua-background]#may# be indicated additionally by providing the **`standard_name`** attribute with an appropriate value, and/or the **`axis`** attribute with the value **`T`**.

[[calendar, Section 4.4.1, "Calendar"]]
==== Calendar

A date/time is the set of numbers which together identify an instant of time, namely its year, month, day, hour, minute and second, where the second [red aqua-background]#may# have a fraction but the others are all integer.
A time coordinate value represents a date/time.
In order to calculate a time coordinate value from a date/time, or the reverse, one [red aqua-background]#must# know the **`units`** attribute of the time coordinate variable (containing the time unit of the coordinate values and the reference date/time) and the calendar.
...
It is [red aqua-background]#recommended# that the calendar be specified by the **`calendar`** attribute of the time coordinate variable.
...
* A time coordinate value [red aqua-background]#must not# be interpreted as representing a date/time in the excluded range.
For instance, **`60 seconds after 23:59`** means **`00:00`** on the next day.
* A date/time in the excluded range [red aqua-background]#must not# be used as a reference date/time e.g. **`seconds since 2016-12-31 23:59:60`** is not a permitted value for **`units`**.
...
In the **`julian`** and the default **`standard`** mixed Gregorian/Julian calendar, dates in years before year 0 (i.e. before 0-1-1 0:0:0) are not allowed, and the year in the reference date/time of the units [red aqua-background]#must not# be negative.
...
This is the default.
A [black aqua-background]#deprecated# alternative name for this calendar is **`gregorian`**.
...
Therefore it is [red aqua-background]#recommended# that date/times in the range from (and including) 1582-10-5 0:0:0 until (but excluding) 1582-10-15 0:0:0 [red aqua-background]#should not# be used as reference in **`units`**, and that a time coordinate variable [red aqua-background]#should not# include any date/times in this range, because their interpretation is unclear.
It is also [red aqua-background]#recommended# that a reference date/time before the discontinuity [red aqua-background]#should not# be used for date/times after the discontinuity, and vice-versa.
...
The **`calendar`** attribute [red aqua-background]#may# be set to **`none`** in climate experiments that simulate a fixed time of year.
...
If none of the calendars defined above applies (e.g., calendars appropriate to a different paleoclimate era), a non-standard calendar [black aqua-background]#can# be defined.
...
If leap years are included, then two other attributes of the time axis [red aqua-background]#should# also be defined:
...
The **`calendar`** attribute is not [red aqua-background]#required# when a non-standard calendar is being used.
...
The spatiotemporal coordinates described in sections 4.1-4.4 are continuous variables, and other geophysical quantities [red aqua-background]#may# likewise serve as continuous coordinate variables, for instance density, temperature or radiation wavelength.
...
Consequently such an axis [red aqua-background]#may# be called {ldquo}discrete{rdquo}.
