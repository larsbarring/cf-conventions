...
To represent data at scattered locations and times with no implied relationship among of coordinate positions, both data and coordinates [aqua-background]#must# share the same (sample) instance dimension.  
...
The `obs` dimension [aqua-background]#may# optionally be the netCDF unlimited dimension of the netCDF file.
====

[[time-series-data, Section H.2, "Time Series Data"]]
=== Time Series Data

Data [aqua-background]#may# be taken over periods of time at a set of discrete point, spatial locations called stations (see also discussion in 9.1).
The set of elements at a particular station is referred to as a timeSeries feature and a data variable [aqua-background]#may# contain a collection of such features.
...
The station variables [aqua-background]#may# contain missing values, allowing one to reserve space for additional stations that [aqua-background]#may# be added at a later time, as discussed in section 9.6.
In addition,

* It is strongly [aqua-background]#recommended# that there [aqua-background]#should# be a station variable (which [aqua-background]#may# be of any type) with the attribute `cf_role={rdquo}timeseries_id{rdquo}`, whose values uniquely identify the stations.
* It is [aqua-background]#recommended# that there [aqua-background]#should# be station variables with standard_name attributes " `platform_name` ", " `surface_altitude` " and `platform_id` when applicable.

All the representations described in section 9.3 [lime-background]#can# be used for time series.
The global attribute `featureType={rdquo}timeSeries{rdquo}` (case-insensitive) [aqua-background]#must# be included.

==== Orthogonal multidimensional array representation of time series

If the time series instances have the same number of elements and the time values are identical for all instances, you [aqua-background]#may# use the orthogonal multidimensional array representation.
...
In the former case, listing the time variable in the **`coordinates`** attributes of the data variables is [aqua-background]#optional#.
...
Either the instance (`station`) or the element (`time`) dimension [aqua-background]#may# optionally be the netCDF unlimited dimension.
====

==== Incomplete multidimensional array representation of time series

Much of the simplicity of the orthogonal multidimensional representation [lime-background]#can# be preserved even in cases where individual time series have different time coordinate values. 
All time series [aqua-background]#must# be allocated the amount of storage needed by the longest, so the use of this representation will trade off simplicity against storage space in some cases.  
...
Either the instance (`station`) dimension or the element (`obs`) dimension [lime-background]#could# be the unlimited dimension of a netCDF file. 
Any unused elements of the data and auxiliary coordinate variables [aqua-background]#must# contain the missing data flag value(section 9.6).
====
...
The sample dimension [lime-background]#could# optionally be the netCDF unlimited dimension. 
The variable bearing the `sample_dimension` attribute [aqua-background]#must# have the instance dimension (`station` in this example) as its single dimension, and [aqua-background]#must# have an integer type. 
...
The variable bearing the `instance_dimension` attribute [aqua-background]#must# have the sample dimension (`obs` in this example) as its single dimension, and [aqua-background]#must# have an integer type. 
...
A data variable [aqua-background]#may# contain a collection of profile features.
...
It is strongly [aqua-background]#recommended# that there always be a profile variable (of any data type) with `cf_role` attribute " `profile_id` ", whose values uniquely identify the profiles.
The profile variables [aqua-background]#may# contain missing values.
This [lime-background]#allows# one to reserve space for additional profiles that [aqua-background]#may# be added at a later time, as discussed in section 9.6.
All the representations described in section 9.1.3 [lime-background]#can# be used for profiles.
The global attribute `featureType={rdquo}profile{rdquo}` (case-insensitive) [aqua-background]#should# be included if all data variables in the file contain profiles.

==== Orthogonal multidimensional array representation of profiles

If the profile instances have the same number of elements and the vertical coordinate values are identical for all instances, you [aqua-background]#may# use the orthogonal multidimensional array representation.
...
In the former case, listing the vertical coordinate variable in the **coordinates** attributes of the data variables is [aqua-background]#optional#.
...
Either the instance (`profile`) or the element (`z`) dimension [lime-background]#could# be the netCDF unlimited dimension.
====

==== Incomplete multidimensional array representation of profiles

If there are the same number of levels in each profile, but they do not have the same set of vertical coordinates, one [lime-background]#can# use the incomplete multidimensional array representation, which the vertical coordinate variable is two-dimensional e.g. replacing `z(z)` in <<example-h.8>> with `alt(profile,z)`.
This representation also [lime-background]#allows# one to have a variable number of elements in different profiles, at the cost of some wasted space.
In that case, any unused elements of the data and auxiliary coordinate variables [aqua-background]#must# contain missing data values (section 9.6).
...
The profile variables `time`, `lat` and `lon`, shown here as scalar, [lime-background]#could# alternatively be one-dimensional `time(profile)`, `lat(profile)`, `lon(profile)` if a size-one profile dimension were retained in the file.
====

==== Contiguous ragged array representation of profiles

When the number of vertical levels for each profile varies, and one [lime-background]#can# control the order of writing, one [lime-background]#can# use the contiguous ragged array representation.
...
The sample dimension (`obs`) [aqua-background]#may# be the unlimited dimension or not.
...
It [aqua-background]#must# have the profile dimension as its single dimension, and [aqua-background]#must# have an integer type. 
...
When the number of vertical levels for each profile varies, and one cannot write them contiguously, one [lime-background]#can# use the indexed ragged array representation.
The canonical use case is when writing real-time data streams that contain reports from many profiles, arriving randomly.
If the sample dimension is the unlimited dimension, this [lime-background]#allows# data to be appended to the file.
...
It [aqua-background]#must# have the sample dimension as its single dimension, and [aqua-background]#must# have an integer type. 
...
Data [aqua-background]#may# be taken along discrete paths through space, each path constituting a connected set of points called a trajectory, for example along a flight path, a ship path or the path of a parcel in a Lagrangian calculation.
A data variable [aqua-background]#may# contain a collection of trajectory features.
...
It is strongly [aqua-background]#recommended# that there always be a trajectory variable (of any data type) with the attribute `cf_role={rdquo}trajectory_id{rdquo}` attribute, whose values uniquely identify the trajectories.
The trajectory variables [aqua-background]#may# contain missing values.
This [lime-background]#allows# one to reserve space for additional trajectories that [aqua-background]#may# be added at a later time, as discussed in section 9.6.
All the representations described in section 9.3 [lime-background]#can# be used for trajectories.
The global attribute `featureType={rdquo}trajectory{rdquo}` (case-insensitive) [aqua-background]#should# be included if all data variables in the file contain trajectories.

==== Multidimensional array representation of trajectories

When storing multiple trajectories in the same file, and the number of elements in each trajectory is the same, one [lime-background]#can# use the multidimensional array representation.
This representation also [lime-background]#allows# one to have a variable number of elements in different trajectories, at the cost of some wasted space.
In that case, any unused elements of the data and auxiliary coordinate variables [aqua-background]#must# contain missing data values (section 9.6).  
...
Either the instance (trajectory) or the element (`obs`) dimension [lime-background]#could# be the netCDF unlimited dimension.
All variables that have trajectory as their only dimension are considered to be information about that trajectory.

If the trajectories all have the same set of times, the time auxiliary coordinate variable [lime-background]#could# be one-dimensional `time(obs)`, or replaced by a one-dimensional coordinate variable `time(time)`, where the size of the time dimension is now equal to the number of elements of each trajectory.
In the latter case, listing the time coordinate variable in the coordinates attribute is [aqua-background]#optional#.
====
...
The `time` dimension [aqua-background]#may# be unlimited or not.
...
When the number of elements for each trajectory varies, and one [lime-background]#can# control the order of writing, one [lime-background]#can# use the contiguous ragged array representation.
...
It [aqua-background]#must# have the trajectory dimension as its single dimension, and [aqua-background]#must# have an integer type. 
...
When the number of elements at each trajectory vary, and the elements cannot be written in order, one [lime-background]#can# use the indexed ragged array representation.
The canonical use case is when writing real-time data streams that contain reports from many trajectories.
The data [lime-background]#can# be written as it arrives; if the flatsample dimension is the unlimited dimension, this [lime-background]#allows# data to be appended to the file.
...
A data variable [aqua-background]#may# contain a collection of such timeSeriesProfile features, one feature per station.
The instance dimension in the case of a timeSeriesProfile is also referred to as the **station dimension**.
The instance variables, which have just this dimension, including latitude and longitude for example, are also referred to as **station variables** and are considered to contain information describing the stations.
The station variables [aqua-background]#may# contain missing values.
This [lime-background]#allows# one to reserve space for additional stations that [aqua-background]#may# be added at a later time, as discussed in section 9.6.
In addition,

* It is strongly [aqua-background]#recommended# that there [aqua-background]#should# be a station variable (which [aqua-background]#may# be of any type) with `cf_role` attribute `timeseries_id`, whose values uniquely identify the stations.
* It is [aqua-background]#recommended# that there [aqua-background]#should# be station variables with standard_name attributes `platform_name`, `surface_altitude` and `platform_id` when applicable.
...
It is strongly [aqua-background]#recommended# that there always be a variable (of any data type) with the profile dimension and the `cf_role` attribute `profile_id`, whose values uniquely identify the profiles.

==== Multidimensional array representations of time series profiles

When storing time series of profiles at multiple stations in the same data variable, if there are the same number of time points for all timeSeries, and the same number of vertical levels for every profile, one [lime-background]#can# use the multidimensional array representation:
...
Any of the three dimensions [lime-background]#could# be the netCDF unlimited dimension, if it [lime-background]#might# be useful to be able enlarge it.

If all of the profiles at any given station have the same set of vertical coordinates values, the vertical auxiliary coordinate variable [lime-background]#could# be dimensioned `alt(station, z)`.
If all the profiles have the same set of vertical coordinates, the vertical auxiliary coordinate variable [lime-background]#could# be one-dimensional `alt(z)`, or replaced by a one-dimensional coordinate variable `z(z)`, provided the values are in strict monotonic order.
In the latter case, listing the vertical coordinate variable in the coordinates attribute is [aqua-background]#optional#.

If the profiles are taken at all stations at the same set of times, the time auxiliary coordinate variable [lime-background]#could# be one-dimensional `time(profile)`, or replaced by a one-dimensional coordinate variable `time(time)`, where the size of the `time` dimension is now equal to the number of profiles at each station.
In the latter case, listing the time coordinate variable in the coordinates attribute is [aqua-background]#optional#.
...
At the cost of some wasted space, the multidimensional array representation also [lime-background]#allows# one to have a variable number of profiles for different stations, and varying numbers of levels for different profiles.
In these cases, any unused elements of the data and auxiliary coordinate variables [aqua-background]#must# contain missing data values (section 9.6).
...
When the number of profiles and levels for each station varies, one [lime-background]#can# use a ragged array representation.
Each of the two element dimensions (time and vertical) [lime-background]#could# in principle be stored either contiguous or indexed, but this convention supports only one of the four possible choices.
...
The index variable [aqua-background]#must# have the profile dimension as its sole dimension, and [aqua-background]#must# have an integer type. 
...
The count variable (`row_size`) contains the number of elements for each profile, which [aqua-background]#must# be written contiguously.
...
It [aqua-background]#must# have the profile dimension as its sole dimension, and [aqua-background]#must# have an integer type. 
...
A data variable [aqua-background]#may# contain a collection of such trajectoryProfile features, one feature per trajectory.
...
If there are the same number of profiles for all trajectories, and the same number of vertical levels for every profile, one [lime-background]#can# use the multidimensional representation:
...
If all of the profiles along any given trajectory have the same set of vertical coordinates values, the vertical auxiliary coordinate variable [lime-background]#could# be dimensioned `alt(trajectory, z)`.
If all the profiles have the same set of vertical coordinates, the vertical auxiliary coordinate variable [lime-background]#could# be one-dimensional `alt(z)`, or replaced by a one-dimensional coordinate variable `z(z)`, provided the values are in strict monotonic order.
In the latter case, listing the vertical coordinate variable in the coordinates attribute is [aqua-background]#optional#.

If the profiles are taken along all the trajectories at the same set of times, the time auxiliary coordinate variable [lime-background]#could# be one-dimensional `time(profile)`, or replaced by a one-dimensional coordinate variable `time(time)`, where the size of the time dimension is now equal to the number of profiles along each trajectory.
In the latter case, listing the time coordinate variable in the coordinates attribute is [aqua-background]#optional#.
====

At the cost of some wasted space, the multidimensional array representation also [lime-background]#allows# one to have a variable number of profiles for different trajectories, and varying numbers of levels for different profiles.
In these cases, any unused elements of the data and auxiliary coordinate variables [aqua-background]#must# contain missing data values (section 9.6).
...
When the number of profiles and levels for each trajectory varies, one [lime-background]#can# use a ragged array representation.
Each of the two element dimensions (along a trajectory, within a profile) [lime-background]#could# in principle be stored either contiguous or indexed, but this convention supports only one of the four possible choices.
