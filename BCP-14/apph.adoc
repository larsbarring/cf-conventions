...
To represent data at scattered locations and times with no implied relationship among of coordinate positions, both data and coordinates [red]#must# share the same (sample) instance dimension.  
...
The obs dimension [red]#may# optionally be the netCDF unlimited dimension of the netCDF file.
====
...
Data [red]#may# be taken over periods of time at a set of discrete point, spatial locations called stations (see also discussion in 9.1).
The set of elements at a particular station is referred to as a timeSeries feature and a data variable [red]#may# contain a collection of such features.
...
The station variables [red]#may# contain missing values, allowing one to reserve space for additional stations that [red]#may# be added at a later time, as discussed in section 9.6.
In addition,

* It is strongly [red]#recommended# that there [red]#should# be a station variable (which [red]#may# be of any type) with the attribute **`cf_role={rdquo}timeseries_id{rdquo}`**, whose values uniquely identify the stations.
* It is [red]#recommended# that there [red]#should# be station variables with standard_name attributes " **`platform_name`** ", " **`surface_altitude`** " and {ldquo} **`platform_id`** {rdquo} when applicable.

All the representations described in section 9.3 can be used for time series.
The global attribute **`featureType={rdquo}timeSeries{rdquo}`** (case-insensitive) [red]#must# be included.


==== Orthogonal multidimensional array representation of time series

If the time series instances have the same number of elements and the time values are identical for all instances, you [red]#may# use the orthogonal multidimensional array representation.
...
Either the instance (station) or the element (time) dimension [red]#may# optionally be the netCDF unlimited dimension.
====
...
All time series [red]#must# be allocated the amount of staorage needed by the longest, so the use of this representation will trade off simplicity against storage space in some cases.  
...
Any unused elements of the data and auxiliary coordinate variables [red]#must# contain the missing data flag value(section 9.6).
====
...
The variable bearing the `**sample_dimension**` attribute [red]#must# have the instance dimension (**`station`** in this example) as its single dimension, and [red]#must# have an integer type.
...
The variable bearing the `**instance_dimension**` attribute [red]#must# have the sample dimension (**`obs`** in this example) as its single dimension, and [red]#must# have an integer type.
...
A data variable [red]#may# contain a collection of profile features.
...
It is strongly [red]#recommended# that there always be a profile variable (of any data type) with **`cf_role`** attribute " **`profile_id`** ", whose values uniquely identify the profiles.
The profile variables [red]#may# contain missing values.
This allows one to reserve space for additional profiles that [red]#may# be added at a later time, as discussed in section 9.6.
All the representations described in section 9.1.3 can be used for profiles.
The global attribute **`featureType={rdquo}profile{rdquo}`** (case-insensitive) [red]#should# be included if all data variables in the file contain profiles.


==== Orthogonal multidimensional array representation of profiles

If the profile instances have the same number of elements and the vertical coordinate values are identical for all instances, you [red]#may# use the orthogonal multidimensional array representation.
...
In that case, any unused elements of the data and auxiliary coordinate variables [red]#must# contain missing data values (section 9.6).
...
The sample dimension (obs) [red]#may# be the unlimited dimension or not.
...
It [red]#must# have the profile dimension as its single dimension, and [red]#must# have an integer type.
...
It [red]#must# have the sample dimension as its single dimension, and [red]#must# have an integer type.
...
Data [red]#may# be taken along discrete paths through space, each path constituting a connected set of points called a trajectory, for example along a flight path, a ship path or the path of a parcel in a Lagrangian calculation.
A data variable [red]#may# contain a collection of trajectory features.
...
It is strongly [red]#recommended# that there always be a trajectory variable (of any data type) with the attribute **`cf_role={rdquo}trajectory_id{rdquo}`** attribute, whose values uniquely identify the trajectories.
The trajectory variables [red]#may# contain missing values.
This allows one to reserve space for additional trajectories that [red]#may# be added at a later time, as discussed in section 9.6.
All the representations described in section 9.3 can be used for trajectories.
The global attribute **`featureType={rdquo}trajectory{rdquo}`** (case-insensitive) [red]#should# be included if all data variables in the file contain trajectories.
...
The time dimension [red]#may# be unlimited or not.
...
It [red]#must# have the trajectory dimension as its single dimension, and [red]#must# have an integer type.
...
A data variable [red]#may# contain a collection of such timeSeriesProfile features, one feature per station.
The instance dimension in the case of a timeSeriesProfile is also referred to as the **station dimension**.
The instance variables, which have just this dimension, including latitude and longitude for example, are also referred to as **station variables** and are considered to contain information describing the stations.
The station variables [red]#may# contain missing values.
This allows one to reserve space for additional stations that [red]#may# be added at a later time, as discussed in section 9.6.
In addition,

* It is strongly [red]#recommended# that there [red]#should# be a station variable (which [red]#may# be of any type) with **`cf_role`** attribute "**`timeseries_id`**", whose values uniquely identify the stations.
* It is [red]#recommended# that there [red]#should# be station variables with standard_name attributes "**`platform_name`**", "**`surface_altitude`**" and {ldquo}**`platform_id`**{rdquo} when applicable.
...
It is strongly [red]#recommended# that there always be a variable (of any data type) with the profile dimension and the **`cf_role`** attribute " **`profile_id`** ", whose values uniquely identify the profiles.
...
In these cases, any unused elements of the data and auxiliary coordinate variables [red]#must# contain missing data values (section 9.6).
...
The index variable [red]#must# have the profile dimension as its sole dimension, and [red]#must# have an integer type.
...
The count variable (row_size) contains the number of elements for each profile, which [red]#must# be written contiguously.
...
It [red]#must# have the profile dimension as its sole dimension, and [red]#must# have an integer type.
...
A data variable [red]#may# contain a collection of such trajectoryProfile features, one feature per trajectory.
