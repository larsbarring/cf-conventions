...
2. The conventions are changed only as actually [red]#required# by common use-cases, and not for needs which cannot be anticipated with certainty.
...
4. The conventions [red]#should# be practicable for both producers and users of data.

5. The metadata [red]#should# be both easily readable by humans and easily parsable by programs.

6. To avoid potential inconsistency within the metadata, the conventions [red]#should# minimise redundancy.

7. The conventions [red]#should# minimise the possibility for mistakes by data-writers and data-readers.

8. Conventions are provided to allow data-producers to describe the data they wish to produce, rather than attempting to prescribe what data they [red]#should# produce; consequently most CF conventions are optional.

9. Because many datasets remain in use for a long time after production, it is desirable that metadata written according to previous versions of the convention [red]#should# also be compliant with and have the same interpretation under later versions.

10. Because all previous versions [red]#must# generally continue to be supported in software for the sake of archived datasets, and in order to limit the complexity of the conventions, there is a strong preference against introducing any new capability to the conventions when there is already some method that can adequately serve the same purpose (even if a different method would arguably be better than the existing one).
...
path:: Paths [red]#must# follow the UNIX style path convention and [red]#may# begin with either a '/', '..', or a word.

recommendation:: Recommendations in this convention are meant to provide advice that [red]#may# be helpful for reducing common mistakes.
In some cases we have [red]#recommended# rather than [red]#required# particular attributes in order to maintain backwards compatibility with COARDS.
An application [red]#must not# depend on a dataset's adherence to recommendations.
...
Depending on context, it [red]#may# be functionally equivalent either to a size-one coordinate variable (<<scalar-coordinate-variables>>) or to a size-one auxiliary coordinate variable (<<labels>> and <<collections-instances-elements>>).
...
Files using this version of the CF Conventions [red]#must# set the NUG defined attribute **`Conventions`** to contain the string value "**`CF-{current-version-as-attribute}`**" to identify datasets that conform to these conventions.

The general description of a file's contents [red]#should# be contained in the following attributes: **`title`**, **`history`**, **`institution`**, **`source`**, **`comment`** and **`references`** (<<description-of-file-contents>>).
For backwards compatibility with COARDS none of these attributes is required, but their use is [red]#recommended# to provide human readable documentation of the file contents.
...
The **`units`** attribute is [red]#required# for all variables that represent dimensional quantities (except for boundary variables defined in <<cell-boundaries>>.
...
Every variable [red]#must# have associated metadata that allows identification of each such coordinate that is relevant.
...
__The use of coordinate variables is [red]#required# for all dimensions that correspond to one dimensional space or time coordinates__.
...
Once the variables containing coordinate data are identified, further conventions are [red]#required# to determine the type of coordinate represented by each of these variables.
Latitude, longitude, and time coordinates are identified solely by the value of their **`units`** attribute.
Vertical coordinates with units of pressure [red]#may# also be identified by the **`units`** attribute.
Other vertical coordinates [red]#must# use the attribute **`positive`** which determines whether the direction of increasing coordinate value is up or down.
Because identification of a coordinate type by its units involves the use of an external package <<UDUNITS>>, we provide the [red]#optional# attribute **`axis`** for a direct identification of coordinates that correspond to latitude, longitude, vertical, or time axes.
...
In particular a model [red]#may# output data on the dimensionless vertical coordinate used in its mathematical formulation.
...
In addition to standardizing the metadata [red]#required# to identify each of these axis types COARDS restricts the axis (equivalently dimension) ordering to be longitude, latitude, vertical, and time (with longitude being the most rapidly varying dimension).
Because of I/O performance considerations it [red]#may# not be possible for models to output their data in conformance with the COARDS requirement.
...
The UGRID conventions have their own conformance document, which [red]#should# be used in conjunction with the CF conformance document when checking the validity of datasets.