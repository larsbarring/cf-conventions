...
The netCDF interface enables but does [lime-background]#not require# the creation of __self-describing__ datasets.
The purpose of the CF conventions is to [lime-background]#require# conforming datasets to contain sufficient metadata that they are self-describing in the sense that each variable in the file has an associated description of what it represents, including physical units if appropriate, and that each value [lime-background]#can# be located in space (relative to earth-based coordinates) and time.
...
The purpose in restricting how the metadata is represented is to make it practical to write software that [lime-background]#allows# a machine to parse that metadata and to automatically associate each data value with its location in time and space.
...
It is recognised that there are limits to what a set of conventions [lime-background]#can# practically cover; these conventions are restricted to issues that are considered to be of common and frequent concern in the design of climate and forecast metadata.
...
The metadata objects [lime-background]#could# be contained in file formats other than netCDF.
...
Extensions to COARDS are implemented in a manner such that the content that doesn't depend on the extensions is still accessible to applications that adhere to the COARDS conventions.
...
A self-describing dataset is one which [lime-background]#can# be interpreted without need for reference to resources outside itself, and the CF principle is to minimise that need.
...
2. The conventions are changed only as actually [aqua-background]#required# by common use-cases, and not for needs which cannot be anticipated with certainty.
...
4. The conventions [aqua-background]#should# be practicable for both producers and users of data.

5. The metadata [aqua-background]#should# be both easily readable by humans and easily parsable by programs.

6. To avoid potential inconsistency within the metadata, the conventions [aqua-background]#should# minimise redundancy.

7. The conventions [aqua-background]#should# minimise the possibility for mistakes by data-writers and data-readers.

8. Conventions are provided to [lime-background]#allow# data-producers to describe the data they wish to produce, rather than attempting to prescribe what data they [aqua-background]#should# produce; consequently most CF conventions are [aqua-background]#optional#.

9. Because many datasets remain in use for a long time after production, it is desirable that metadata written according to previous versions of the conventions [aqua-background]#should# also be compliant with and have the same interpretation under later versions.

10. Because all previous versions [aqua-background]#must# generally continue to be supported in software for the sake of archived datasets, and in order to limit the complexity of the conventions, there is a strong preference against introducing any new capability to the conventions when there is already some method that [lime-background]#can# adequately serve the same purpose (even if a different method would arguably be better than the existing one).
...
auxiliary coordinate variable:: Any netCDF variable that contains coordinate data, but is not a coordinate variable (in the sense of that term defined by the <<NUG>> and used by these conventions - see below).
...
cell:: A region in one or more dimensions whose boundary [lime-background]#can# be described by a set of vertices recorded in boundary variables.
...
In CF, a coordinate variable [aqua-background]#must# be of a numeric data type (note that
...
The coordinate values [aqua-background]#must# be in strict monotonic order (all values are different, and they are arranged in either consistently increasing or consistently decreasing order).
Missing values are [lime-background]#not allowed# in coordinate variables.
To avoid confusion with coordinate variables, CF does not [lime-background]#permit# a one-dimensional string-valued variable to have the same name as its dimension.
...
datetime:: The set of numbers which together identify an instant of time, namely its year, month, day, hour, minute and second, where the second [aqua-background]#may# have a fraction but the others are all integer. See <<time-coordinate-units>>.
...
nearest item:: The item (variable or group) that [lime-background]#can# be reached via the shortest traversal of the file from the referring group following the rules set forth in the <<groups>>.

out-of-group reference:: A reference to a variable or dimension that is not contained in the referring group.

path:: Paths [aqua-background]#must# follow the UNIX style path convention and [aqua-background]#may# begin with either a '/', '..', or a word.
...
recommendation:: Recommendations in these conventions are meant to provide advice that [aqua-background]#may# be helpful for reducing common mistakes.
In some cases the use of particular attributes is [aqua-background]#recommended# rather than [aqua-background]#required# in order to maintain backwards compatibility with COARDS.
An application [aqua-background]#must not# depend on a dataset's adherence to recommendations.
...
Depending on context, it [aqua-background]#may# be functionally equivalent either to a size-one coordinate variable (<<scalar-coordinate-variables>>) or to a size-one auxiliary coordinate variable (<<labels>> and <<collections-instances-elements>>).
...
Variable or dimension names [lime-background]#can# either be a single variable name or a path to a variable.
The overview provided in this section will be followed with more complete descriptions in following sections.
<<attribute-appendix>> contains a summary of all the attributes used in these conventions.

Files using this version of the CF Conventions [aqua-background]#must# set the <<NUG>> defined attribute **`Conventions`** to contain the string value "**`CF-{current-version-as-attribute}`**" to identify datasets that conform to these conventions.

The general description of a file's contents [aqua-background]#should# be contained in the following attributes: **`title`**, **`history`**, **`institution`**, **`source`**, **`comment`** and **`references`** (<<description-of-file-contents>>).
For backwards compatibility with COARDS none of these attributes is [aqua-background]#required#, but their use is [aqua-background]#recommended# to provide human readable documentation of the file contents.
...
The **`units`** attribute is [aqua-background]#required# for all variables that represent dimensional quantities (except for boundary variables defined in <<cell-boundaries>>).
The values of the **`units`** attributes are character strings that are recognized by UNIDATA's UDUNITS package <<UDUNITS>> (with exceptions [lime-background]#allowed# as discussed in <<units>>).

The **`long_name`** and **`standard_name`** attributes are used to describe the content of each variable.
For backwards compatibility with COARDS neither is [aqua-background]#required#, but use of at least one of them is strongly [aqua-background]#recommended#.
...
Every variable [aqua-background]#must# have associated metadata that [lime-background]#allows# identification of each such coordinate that is relevant.
Two independent parts of the conventions [lime-background]#allow# this to be done.
...
__The use of coordinate variables is [aqua-background]#required# for all dimensions that correspond to one dimensional space or time coordinates__.
...
Once the variables containing coordinate data are identified, further conventions are [aqua-background]#required# to determine the type of coordinate represented by each of these variables.
Latitude, longitude, and time coordinates are identified solely by the value of their **`units`** attribute.
Vertical coordinates with units of pressure [aqua-background]#may# also be identified by the **`units`** attribute.
Other vertical coordinates [aqua-background]#must# use the attribute **`positive`** which determines whether the direction of increasing coordinate value is up or down.
Because identification of a coordinate type by its units involves the use of an external package <<UDUNITS>>, the [aqua-background]#optional# attribute **`axis`** is provided for a direct identification of coordinates that correspond to latitude, longitude, vertical, or time axes.
...
In particular a model [aqua-background]#may# output data on the parametric (usually dimensionless) vertical coordinate used in its mathematical formulation.
To achieve the goal of being able to spatially locate all data values, these conventions provide a mapping, via the **`standard_name`** and **`formula_terms`** attributes of a parametric vertical coordinate variable, between its values and dimensional vertical coordinate values that [lime-background]#can# be uniquely located with respect to a point on the earth's surface (<<parametric-vertical-coordinate>>; <<parametric-v-coord>>).
...
This assumption does not hold in CF. If bounds are not provided, the location of the gridpoint within the cell is undefined, and nothing [lime-background]#can# be assumed about the location and extent of the cell.
...
When data that is representative of cells [lime-background]#can# be described by simple statistical methods (for instance, mean or maximum), those methods [lime-background]#can# be indicated using the **`cell_methods`** attribute.
...
The conventions also strive to maximize __conformance__ to the COARDS conventions so that datasets that only [lime-background]#require# the metadata that was available under COARDS will still be able to be processed by COARDS conforming applications.
...
The features of these conventions that [lime-background]#allow# writing netCDF files that are not COARDS conforming are summarized below.

COARDS standardizes the description of grids composed of independent latitude, longitude, vertical, and time axes.
In addition to standardizing the metadata [aqua-background]#required# to identify each of these axis types, COARDS [lime-background]#requires# (_time_, _vertical_, _latitude_, _longitude_) as the CDL order for the dimensions of a variable, with longitude being the most rapidly varying dimension (the last dimension in CDL order).
Because of I/O performance considerations it [aqua-background]#may# not be possible for models to output their data in conformance with the COARDS requirement.
The CF conventions place no rigid restrictions on the order of dimensions, however data producers are encouraged to make the extra effort to stay within the COARDS conventions order.
The use of non-COARDS axis ordering will render files inaccessible to some applications and limit interoperability.
Often a buffering operation [lime-background]#can# be used to miminize performance penalties when axis ordering in model code does not match the axis ordering of a COARDS file.

COARDS addresses the issue of identifying dimensionless vertical coordinates, but does not provide any mechanism for mapping the dimensionless values to dimensional ones that [lime-background]#can# be located with respect to the earth's surface.
For backwards compatibility the **`units`** attribute of dimensionless vertical coordinates to take the values "level", "layer", or "sigma_level" continues to be [lime-background]#allowed# (but not required).
But it is [aqua-background]#recommended# that the **`standard_name`** and **`formula_terms`** attributes be used to identify the appropriate definition of the dimensionless vertical coordinate (see <<parametric-vertical-coordinate>>).
...
Briefly the new attributes [lime-background]#allow#:
...
Only version 1.0 of the UGRID conventions is [lime-background]#allowed#.
...
A summary indicating how UGRID relates to other parts of the CF conventions, and which features of UGRID are excluded from CF, [lime-background]#can# be found in <<mesh-topology-variables>>.
...
The UGRID conventions have their own conformance document, which [aqua-background]#should# be used in conjunction with the CF conformance document when checking the validity of datasets.
