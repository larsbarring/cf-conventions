...
A data model is necessary because it imposes the rules, constraints, and relationships connecting metadata to the data that are needed to imagine how the quantities included in the dataset [red]#should# be combined and processed scientifically.
...
The data model was transcribed from the GMD paper into the CF conventions at version 1.9, also incorporating the modifications [red]#required# to represent new features introduced at versions 1.7, 1.8 and 1.9.

[[data-model-design-criteria]]
=== Design criteria of the CF data model

The primary requirement of the data model is that it [red]#should# be able to describe all existing and conceivable CF-compliant datasets.

The data model [red]#should# comprise a minimal set of elements that are sufficient for accommodating all aspects of the CF conventions.
...
The CF data model [red]#should# also be independent of the encoding.
This means that it [red]#should not# be constrained by the parts of the CF conventions which describe explicitly how to store (i.e. encode) metadata in a netCDF file.
The virtue of this is that [red]#should# netCDF ever fail to meet the community needs, the groundwork for applying CF to other file formats will already exist.
...
The constructs contained by the field and domain constructs cannot exist independently, with the exception of the domain construct itself that [red]#may# be part of a field construct or exist on its own, as is indicated by the nature of the class associations shown in <<figure-field>>.
...
All of the constructs contained by the field construct are [red]#optional# (as indicated by "0.." in <<figure-field>>).
...
The **`standard_name`** property constrains the **`units`** property (i.e. only certain units are consistent with each standard name) and in some cases also the dimensions that a data variable [red]#must# have.
These constraints, however, do not supply any further information--they are just for self consistency.
Similarly the **`featureType`** property imposes some requirements on the axes the domain [red]#must# have.
...
The domain construct is the only metadata construct that [red]#may# also exist independently of a field construct.
...
All of the constructs contained by the domain construct are [red]#optional# (as indicated by "0.." in <<figure-field>>).
...
* explicitly in a mesh topology variable, in which case the domain [red]#may# be one of multiple domains defined by the same variable (for instance, a single mesh topology variable could contain different domains defined respectively at node, edge, and face mesh elements);
...
The field construct's data array spans the domain axis constructs of the domain, except that the size-one axes [red]#may# optionally be omitted, because their presence makes no difference to the order of the elements.
Hence, the data array [red]#may# be zero-dimensional (i.e. scalar) if there are no domain axis constructs of size greater than one.
...
In practice, the unpacked data array [red]#may# be dominated by missing values (as could occur, for example, if all features in a collection of time series had no common time coordinates), in which case it [red]#may# be preferable to view the collection as if each DSG feature were a separate variable, each one corresponding to a different field construct.
...
A coordinate construct consists of an [red]#optional# data array of the coordinate values spanning the subset of the domain axis constructs, properties to describe the coordinates (in the same sense as for the field construct), an [red]#optional# data array of cell bounds recording the extents of each cell, and any extra arrays needed to interpret the cell bounds values.
...
All parts of all the cells [red]#must# be of the same one of these three kinds, which are called "geometry types".
...
If cell bounds are provided for a dimension coordinate construct then each cell [red]#must# have exactly two vertices forming a line geometry.
...
The cell method constructs indicate how the multiple time segments [red]#should# be inferred from these climatological bounds.

If a polygonal cell is composed of multiple parts it [red]#may# have holes, i.e. polygon regions that are to be omitted from, as opposed to included in, the cell extent.
When such holes are present an "interior ring" array is [red]#required# that records whether each polygon is to be included or excluded from the cell, and is supplied by an interior ring variable in CF-netCDF.
...
In such cases cells [red]#may# be described with one-dimensional auxiliary coordinate constructs for which, provided that there is a cell bounds array to describe the cell extents, the coordinate array is optional, since coordinates are not always well defined for such cells.
...
In the case of climatological time coordinates, the two points of the cell bounds, in conjunction with the cell methods, imply the existence of multiple line parts, different subsets of which are associated with the different cell methods [red]#required# to define the climatology.
...
The domain [red]#may# contain various coordinate systems, each of which is constructed from a subset of the dimension and auxiliary coordinate constructs.
For example, the domain of a four-dimensional field construct [red]#may# contain horizontal (__y__-__x__), vertical (_z_), and temporal (_t_) coordinate systems.
There [red]#may# be more than one of each of these, if there is more than one coordinate construct applying to a particular spatiotemporal dimension (for example, there could be both latitude-longitude and __y__-__x__ projection coordinate systems).

A coordinate system [red]#may# be constructed _implicitly_ from any subset of the coordinate constructs, yet a coordinate construct does not need to be explicitly or exclusively associated with any coordinate system.
...
The datum [red]#may# be explicitly indicated via properties, or it [red]#may# be implied by the metadata of the contained dimension and auxiliary coordinate constructs.
...
Note that the datum [red]#may# contain the definition of a geophysical surface which corresponds to the zero of a vertical coordinate construct, and this [red]#may# be [red]#required# for both horizontal and vertical coordinate systems.
* A coordinate conversion, which defines a formula for converting coordinate values taken from the dimension or auxiliary coordinate constructs to a different coordinate system. A term of the conversion formula can be a scalar or vector parameter which does not depend on any domain axis constructs, [red]#may# have units (such as a reference pressure value), or [red]#may# be a descriptive string (such as the projection name "mercator"), or it can be a domain ancillary construct (such as one containing spatially varying orography data).
...
The coordinate conversion is not [red]#required# if no other coordinate systems are described.

Some parts of the coordinate reference construct [red]#may# not be relevant to a given coordinate construct which it contains.
...
It also contains an [red]#optional# array of cell bounds recording the extents of each cell (only applicable if the array contains coordinate data) and properties to describe the data (in the same sense as for the field construct).
...
These CF-netCDF variables [red]#may# be coordinate, scalar coordinate, or auxiliary coordinate variables, or they [red]#may# be data variables.
...
The properties [red]#must# contain a "measure" property, which indicates which metric of the space it supplies, e.g. cell horizontal areas, and a units property consistent with the measure property, e.g. m2.
...
A domain topology construct defines the geospatial topology of cells arranged in two or three dimensions in real space but indexed by a single (discrete) domain axis construct, and at most one domain topology construct [red]#may# be associated with any such domain axis.
...
Each node has a unique arbitrary identity that is independent of its spatial location, and different nodes [red]#may# be spatially co-located.

The topology [red]#may# only describe cells that have a common spatial dimensionality, one of:
...
For example, the mesh depicted in <<figure-mesh-example>> [red]#may# be used with any of three domain topology constructs for domains comprising two face cells (one triangle and one quadrilateral), six edge cells, and five point cells respectively.

A domain topology construct contains an array defining the mesh, and properties to describe it.
There [red]#must# be a property indicating the spatial dimensionality of the cells.
The array values comprise the node identities, and all array elements that refer to the same node [red]#must# contain the same value, which [red]#must# differ from any other value in the array.
...
When a domain topology construct is present it is considered to be definitive and [red]#must# be used in preference to the topology implied by inspection of any other constructs, which is not guaranteed to be the same.
...
The topology for node cells [red]#may# be provided by any of these three UGRID variables.
The integer indices contained in the UGRID variable [red]#may# be used as the mesh node identities, although the CF data model attaches no significance to the values other than the fact that some values are the same as others.
...
Whether or not two domains have the same mesh [red]#may# be reliably deternined by inspection, thereby allowing the creation of netCDF datasets containing UGRID mesh topology variables.

The restrictions on the type of mesh that [red]#may# be used with a given cell spatial dimensionality excludes some meshes which can be described by an undirected graph, but is consistent with UGRID encoding within CF-netCDF.
...
There [red]#must# be a property indicating the condition by which the connectivity is derived from the domain topology.
...
In the direct case, the integer indices contained in the UGRID variable [red]#may# be used as the cell identities, although the CF data model attaches no significance to the values other than the fact that some values are the same as others.
...
The field construct [red]#may# contain an ordered sequence of cell method constructs describing multiple processes which have been applied to the data, e.g. a temporal maximum of the areal mean has two components--a mean and a maximum, each acting over different sets of axes.
...
The strings which identify such axes are well defined in that they [red]#must# be standard names (e.g. time, longitude) or the special string **`area`**, indicating a combination of horizontal axes.
