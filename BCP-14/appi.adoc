...
A data model is necessary because it imposes the rules, constraints, and relationships connecting metadata to the data that are needed to imagine how the quantities included in the dataset [aqua-background]#should# be combined and processed scientifically.
...
The data model was transcribed from the GMD paper into the CF conventions at version 1.9, also incorporating the modifications [aqua-background]#required# to represent new features introduced at versions 1.7, 1.8 and 1.9.

[[data-model-design-criteria]]
=== Design criteria of the CF data model

The primary requirement of the data model is that it [aqua-background]#should# be able to describe all existing and conceivable CF-compliant datasets.

The data model [aqua-background]#should# comprise a minimal set of elements that are sufficient for accommodating all aspects of the CF conventions.
The elements of the data model are restricted to those that are explicitly mentioned in CF, but there do not [lime-background]#have to# be as many elements in the data model as there are elements described by CF, because a single data model element [lime-background]#can# incorporate more than one CF entity.
...
The CF data model [aqua-background]#should# also be independent of the encoding.
This means that it [aqua-background]#should not# be constrained by the parts of the CF conventions which describe explicitly how to store (i.e. encode) metadata in a netCDF file.
The virtue of this is that [aqua-background]#should# netCDF ever fail to meet the community needs, the groundwork for applying CF to other file formats will already exist.
...
[.text-center]
.The relationships between CF-netCDF elements and their corresponding netCDF variables, dimensions and attributes (identified here with the "NC" prefix). It is useful to define an abstract generic coordinate variable that [lime-background]#can# be used to refer to coordinates when the their type (coordinate, auxiliary or scalar coordinate variable) is not an issue.
...
The constructs contained by the field and domain constructs cannot exist independently, with the exception of the domain construct itself that [aqua-background]#may# be part of a field construct or exist on its own, as is indicated by the nature of the class associations shown in <<figure-field>>.
All CF-netCDF elements are mapped to field constructs, domain constructs or their components; and the field and domain constructs completely contain all the data and metadata which [lime-background]#can# be extracted from the file using the CF conventions.

[[figure-field]]
[caption="Figure {doc-part}.{counter:figure}. ", reftext=Figure {doc-part}.{figure}]
[.text-center]
.The constructs of the CF data model. The field and domain constructs correspond to CF-netCDF data and domain variables respectively (defined in <<figure-cf-concepts>> and identified here with the "CN" prefix). Relationships between the other constructs and CF-netCDF are given in <<figure-dim-aux>> and <<figure-coordinate-reference>>. It is useful to define an abstract generic coordinate construct that [lime-background]#can# be used to refer to coordinates when the their type (dimension or auxiliary coordinate construct) is not an issue.
...
All of the constructs contained by the field construct are [aqua-background]#optional# (as indicated by "0.." in <<figure-field>>).
...
Hence, metadata stored in attributes of the group as a whole [lime-background]#have to# be transferred to the field construct.
...
The **`standard_name`** property constrains the **`units`** property (i.e. only certain units are consistent with each standard name) and in some cases also the dimensions that a data variable [aqua-background]#must# have.
These constraints, however, do not supply any further information--they are just for self consistency.
Similarly the **`featureType`** property imposes some requirements on the axes the domain [aqua-background]#must# have.
...
The domain construct is the only metadata construct that [aqua-background]#may# also exist independently of a field construct.
...
All of the constructs contained by the domain construct are [aqua-background]#optional# (as indicated by "0.." in <<figure-field>>).
...
* explicitly in a mesh topology variable, in which case the domain [aqua-background]#may# be one of multiple domains defined by the same variable (for instance, a single mesh topology variable [lime-background]#could# contain different domains defined respectively at node, edge, and face mesh elements);
...
The field construct's data array spans the domain axis constructs of the domain, except that the size-one axes [aqua-background]#may# optionally be omitted, because their presence makes no difference to the order of the elements.
Hence, the data array [aqua-background]#may# be zero-dimensional (i.e. scalar) if there are no domain axis constructs of size greater than one.
...
In practice, the unpacked data array [aqua-background]#may# be dominated by missing values (as [lime-background]#could# occur, for example, if all features in a collection of time series had no common time coordinates), in which case it [aqua-background]#may# be preferable to view the collection as if each DSG feature were a separate variable, each one corresponding to a different field construct.
...
A coordinate construct consists of an [aqua-background]#optional# data array of the coordinate values spanning the subset of the domain axis constructs, properties to describe the coordinates (in the same sense as for the field construct), an [aqua-background]#optional# data array of cell bounds recording the extents of each cell, and any extra arrays needed to interpret the cell bounds values.
The data array of the coordinate values is [aqua-background]#required#, execpt for the special cases described below.
...
Auxiliary coordinate constructs [lime-background]#have to# be used, instead of dimension coordinate constructs, when a single domain axis [lime-background]#requires# more than one set of coordinate values, when coordinate values are not numeric, strictly monotonic, or contain missing values, or when they vary along more than one domain axis construct simultaneously.
...
All parts of all the cells [aqua-background]#must# be of the same one of these three kinds, which are called "geometry types".
...
If cell bounds are provided for a dimension coordinate construct then each cell [aqua-background]#must# have exactly two vertices forming a line geometry.
For climatological time coordinates the actual cell extent comprises multiple time segments equivalent to multiple line geometry parts, but the bounds [lime-background]#require# just two points to define each cell, namely the earliest and latest times of the sequence.
The cell method constructs indicate how the multiple time segments [aqua-background]#should# be inferred from these climatological bounds.

If a polygonal cell is composed of multiple parts it [aqua-background]#may# have holes, i.e. polygon regions that are to be omitted from, as opposed to included in, the cell extent.
When such holes are present an "interior ring" array is [aqua-background]#required# that records whether each polygon is to be included or excluded from the cell, and is supplied by an interior ring variable in CF-netCDF.
...
For example, a cell describing the land area surrounding a lake would [lime-background]#require# two polygon parts: one defines the outer boundary of the land area; the other, recorded as an interior ring, is the lake boundary, defining the inner boundary of the land area.
...
In such cases cells [aqua-background]#may# be described with one-dimensional auxiliary coordinate constructs for which, provided that there is a cell bounds array to describe the cell extents, the coordinate array is [aqua-background]#optional#, since coordinates are not always well defined for such cells.
...
In the case of climatological time coordinates, the two points of the cell bounds, in conjunction with the cell methods, imply the existence of multiple line parts, different subsets of which are associated with the different cell methods [aqua-background]#required# to define the climatology.
...
The domain [aqua-background]#may# contain various coordinate systems, each of which is constructed from a subset of the dimension and auxiliary coordinate constructs.
For example, the domain of a four-dimensional field construct [aqua-background]#may# contain horizontal (__y__-__x__), vertical (_z_), and temporal (_t_) coordinate systems.
There [aqua-background]#may# be more than one of each of these, if there is more than one coordinate construct applying to a particular spatiotemporal dimension (for example, there [lime-background]#could# be both latitude-longitude and __y__-__x__ projection coordinate systems).

A coordinate system [aqua-background]#may# be constructed _implicitly_ from any subset of the coordinate constructs, yet a coordinate construct does not need to be explicitly or exclusively associated with any coordinate system.
A coordinate system of the field construct [lime-background]#can# be _explicitly_ defined by a coordinate reference construct (<<figure-coordinate-reference>>) which relates the coordinate values of the coordinate system to locations in a planetary reference frame and consists of the following:
...
The datum [aqua-background]#may# be explicitly indicated via properties, or it [aqua-background]#may# be implied by the metadata of the contained dimension and auxiliary coordinate constructs.
...
Note that the datum [aqua-background]#may# contain the definition of a geophysical surface which corresponds to the zero of a vertical coordinate construct, and this [aqua-background]#may# be [aqua-background]#required# for both horizontal and vertical coordinate systems.
* A coordinate conversion, which defines a formula for converting coordinate values taken from the dimension or auxiliary coordinate constructs to a different coordinate system. A term of the conversion formula [lime-background]#can# be a scalar or vector parameter which does not depend on any domain axis constructs, [aqua-background]#may# have units (such as a reference pressure value), or [aqua-background]#may# be a descriptive string (such as the projection name "mercator"), or it [lime-background]#can# be a domain ancillary construct (such as one containing spatially varying orography data).
...
The coordinate conversion is not [aqua-background]#required# if no other coordinate systems are described.

Some parts of the coordinate reference construct [aqua-background]#may# not be relevant to a given coordinate construct which it contains.
...
It also contains an [aqua-background]#optional# array of cell bounds recording the extents of each cell (only applicable if the array contains coordinate data) and properties to describe the data (in the same sense as for the field construct).
...
These CF-netCDF variables [aqua-background]#may# be coordinate, scalar coordinate, or auxiliary coordinate variables, or they [aqua-background]#may# be data variables.
...
Cell measure constructs [lime-background]#have to# be used when the size or shape of the cells cannot be deduced from the dimension or auxiliary coordinate constructs without special knowledge that a generic application cannot be expected to have.
...
The properties [aqua-background]#must# contain a "measure" property, which indicates which metric of the space it supplies, e.g. cell horizontal areas, and a units property consistent with the measure property, e.g. m2.
...
A domain topology construct defines the geospatial topology of cells arranged in two or three dimensions in real space but indexed by a single (discrete) domain axis construct, and at most one domain topology construct [aqua-background]#may# be associated with any such domain axis.
...
Each node has a unique arbitrary identity that is independent of its spatial location, and different nodes [aqua-background]#may# be spatially co-located.

The topology [aqua-background]#may# only describe cells that have a common spatial dimensionality, one of:
...
Each type of cell implies a restricted topology for which only some kinds of mesh are [lime-background]#allowed#.
...
For edge and face cells, every node corresponds to a boundary vertex of a cell; the same node [lime-background]#can# represent vertices in multiple cells; every link in the mesh connects two cell boundary vertices; and two cells have a topological relationship if and only if they share at least one node.
...
For example, the mesh depicted in <<figure-mesh-example>> [aqua-background]#may# be used with any of three domain topology constructs for domains comprising two face cells (one triangle and one quadrilateral), six edge cells, and five point cells respectively.

A domain topology construct contains an array defining the mesh, and properties to describe it.
There [aqua-background]#must# be a property indicating the spatial dimensionality of the cells.
The array values comprise the node identities, and all array elements that refer to the same node [aqua-background]#must# contain the same value, which [aqua-background]#must# differ from any other value in the array.
...
When a domain topology construct is present it is considered to be definitive and [aqua-background]#must# be used in preference to the topology implied by inspection of any other constructs, which is not guaranteed to be the same.

In CF-netCDF a domain topology construct [lime-background]#can# only be provided for a UGRID mesh topology variable.
...
The topology for node cells [aqua-background]#may# be provided by any of these three UGRID variables.
The integer indices contained in the UGRID variable [aqua-background]#may# be used as the mesh node identities, although the CF data model attaches no significance to the values other than the fact that some values are the same as others.
...
Whether or not two domains have the same mesh [aqua-background]#may# be reliably deternined by inspection, thereby allowing the creation of netCDF datasets containing UGRID mesh topology variables.

The restrictions on the type of mesh that [aqua-background]#may# be used with a given cell spatial dimensionality excludes some meshes which [lime-background]#can# be described by an undirected graph, but is consistent with UGRID encoding within CF-netCDF.
...
Connectivity [lime-background]#can# only be provided when the domain axis construct also has a domain topology construct, and two cells [lime-background]#can# only be connected if they also have a topological relationship.
For instance, the connectivity of two-dimensional face cells [lime-background]#could# be characterised by whether or not they have shared edges, where the edges are defined by connected nodes of the domain topology construct.

The cell connectivity construct consists of an array recording the connectivity, and properties to describe the data.
There [aqua-background]#must# be a property indicating the condition by which the connectivity is derived from the domain topology.
...
Exactly the same connectivity information [lime-background]#could# be derived from the domain topology construct.
Connectivity information inferred from inspection of any other constructs is not guaranteed to be the same.

In CF-netCDF a cell topology construct [lime-background]#can# only be provided by a UGRID mesh topology variable.
...
In the direct case, the integer indices contained in the UGRID variable [aqua-background]#may# be used as the cell identities, although the CF data model attaches no significance to the values other than the fact that some values are the same as others.
...
For example, if a data variable holds a variable retrieved from a satellite instrument, a related ancillary data variable [lime-background]#might# provide the uncertainty estimates for those retrievals (varying over the same spatiotemporal domain).
...
It [aqua-background]#may# have properties to describe the data (in the same sense as for the field construct).
...
The field ancillary construct [aqua-background]#may# have cell methods constructs (<<data-model-cell-method>>), which are supplementary information for a single one of the cell method constructs of the field, but do not contribute to the definition of the domain.
...
A single cell method construct consists of a set of axes (see below), a "method" property which describes how a value of the field construct's data array describes the variation of the quantity within a cell over those axes (e.g. a value [lime-background]#might# represent the cell area average), and properties serving to indicate more precisely how the method was applied (e.g. recording the spacing of the original data, or the fact the method was applied only over El Niño years).

The field construct [aqua-background]#may# contain an ordered sequence of cell method constructs describing multiple processes which have been applied to the data, e.g. a temporal maximum of the areal mean has two components--a mean and a maximum, each acting over different sets of axes.
...
For example, a climatological time mean [lime-background]#might# be based on data which are not available over the same time periods at every horizontal location--the useful information that the data have been temporally averaged [lime-background]#can# be recorded without specifying the range of times.
The strings which identify such axes are well defined in that they [aqua-background]#must# be standard names (e.g. time, longitude) or the special string **`area`**, indicating a combination of horizontal axes.
