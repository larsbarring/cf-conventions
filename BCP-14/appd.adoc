...
Also, the definitions are given in general forms that [red]#may# be simplified by omitting certain terms.
A term that is omitted from the **`formula_terms`** attribute [red]#should# be assumed to be zero.

The variables containing the terms [red]#may# optionally have `standard_name` attributes, with values as indicated in this Appendix.
The `standard_name` of the dimensional coordinate values which are computed by the formula [red]#may# optionally be specified by the `computed_standard_name` attribute of the vertical coordinate variable, as indicated in this Appendix.
A `computed_standard_name` is uniquely implied by the formula in some cases, while in others it depends on the `standard_name` of one or more of the terms, with which it [red]#must# be consistent.
...
The `standard_name` of `orog` [red]#may# be `surface_altitude` (i.e. above the geoid) or `surface_height_above_geopotential_datum`.
...
The `standard_name` of `ztop` [red]#may# be `altitude_at_top_of_atmosphere_model` (i.e.  above the geoid) or `height_above_geopotential_datum_at_top_of_atmosphere_model`.
The `computed_standard_name` is `altitude` or `height_above_geopotential_datum` in these cases respectively.
...
The pass:q[`standard_name`]s for `eta` and `depth` and the `computed_standard_name` [red]#must# be one of the consistent sets shown in <<table-computed-standard-names, Table D.1>>.
...
The constants `a` and `b` are dimensionless, and `depth_c` [red]#must# have units of length.
...
**The description of this type of parametric vertical coordinate is defective in version 1.8 and earlier versions of the standard, in that it does not state what values the vertical coordinate variable [red]#should# contain.
...
Both `sigma` and `zlev` [red]#must# have this dimension.
For any `k`, whichever of `sigma(k)` or `zlev(k)` is undefined [red]#must# contain missing data, while the other [red]#must# not.
...
The pass:q[`standard_name`]s for `eta`, `depth`, `zlev` and the `computed_standard_name` [red]#must# be one of the consistent sets shown in <<table-computed-standard-names, Table D.1>>.
The `standard_name` for `sigma` is `ocean_sigma_coordinate`.
No `standard_name` has been defined for `depth_c` or `nsigma`.
The `nsigma` parameter is deprecated and [red]#optional# in `formula_terms`; if supplied, it [red]#must# equal the number of elements of `zlev` which contain missing data.

The `standard_name` for the vertical coordinate variable is `ocean_sigma_z_coordinate`.
This variable [red]#should# contain `sigma(k)*depth_c` for the layers where `sigma` is defined and `zlev(k)` for the other layers, with units of length.
The layers [red]#must# be arranged so that the vertical coordinate variable contains a strictly monotonic set of indicative values for the heights of the levels relative to the datum, either increasing or decreasing, and the direction [red]#must# be indicated by the `positive` attribute, in the usual way for a vertical coordinate variable.
...
The `standard_name` for `depth` and the `computed_standard_name` [red]#must# be one of the consistent sets shown in <<table-computed-standard-names, Table D.1>>.
