...
The definitions given here [lime-background]#allow# an application to compute dimensional coordinate values from the parametric vertical coordinate values (usually dimensionless) and associated variables.
...
Also, the definitions are given in general forms that [aqua-background]#may# be simplified by omitting certain terms.
A term that is omitted from the **`formula_terms`** attribute [aqua-background]#should# be assumed to be zero.

The variables containing the terms [aqua-background]#may# optionally have `standard_name` attributes, with values as indicated in this Appendix.
The `standard_name` of the dimensional coordinate values which are computed by the formula [aqua-background]#may# optionally be specified by the `computed_standard_name` attribute of the vertical coordinate variable, as indicated in this Appendix.
A `computed_standard_name` is uniquely implied by the formula in some cases, while in others it depends on the `standard_name` of one or more of the terms, with which it [aqua-background]#must# be consistent.
...
===  Atmosphere hybrid sigma log-pressure coordinate
...
+
----
eta(k) = p(n,k,j,i)/p0 * ( ps(n,j,i)/p0 )**(-b(k))
----
...
+
----
ln( p(n,k,j,i)/p0 ) = ln( eta(k) ) + b(k)*ln( ps(n,j,i)/p0 )
p(n,k,j,i) = p0 * eta(k) * ( ps(n,j,i)/p0 )**b(k)
----

where `p(n,k,j,i)` is the pressure at gridpoint `(n,k,j,i)`, `p0` is a reference pressure, and `ps(n,j,i)` is the surface pressure at horizontal gridpoint `(j,i)` and time `(n)`. 
`eta(k)` is the hybrid level coordinate value at level `k`. 
`eta(k)` is dimensionless and takes values in the range [0,1]. 
`b(k)` is dimensionless and is a function of `eta(k)` and two dimensionless parameters `rmin` and `rmax` according to:

----
lambda(k) = ( zeta(k) - zeta_U ) / ( zeta_S - zeta_U )
r(k) = rmax - (rmax - rmin)*lambda(k)
b(k) = lambda(k)**r(k)
----

where `zeta_S = ln(p0)`, `zeta(k) = zeta_S + ln(eta(k))` and `zeta_U = min(zeta(k))`. `rmin` and `rmax` [lime-background]#can# be adjusted to control the rectification of levels over high terrain as described by Girard et al. 2014 (https://doi.org/10.1175/MWR-D-13-00255.1).

The format for the formula_terms attribute is
----
formula_terms = "eta: var1 b: var2 ps: var3 p0: var4"
----

The `standard_name` of `eta` is `atmosphere_hybrid_sigma_ln_pressure_coordinate`. 
The `standard_name` of `p0` is `reference_air_pressure_for_atmosphere_vertical_coordinate`, and of `ps` is `surface_air_pressure`.  
The air pressure `p` is the computed vertical coordinate. 
To indicate the `standard_name` of this quantity, the coordinate variable [aqua-background]#may# be given the attribute `computed_standard_name="air_pressure"`. 
...
The `standard_name` of `orog` [aqua-background]#may# be `surface_altitude` (i.e. above the geoid) or `surface_height_above_geopotential_datum`.
...
The `standard_name` of `ztop` [aqua-background]#may# be `altitude_at_top_of_atmosphere_model` (i.e. above the geoid) or `height_above_geopotential_datum_at_top_of_atmosphere_model`. 
...
The pass:q[`standard_name`]s for `eta` and `depth` and the `computed_standard_name` [aqua-background]#must# be one of the consistent sets shown in <<table-computed-standard-names, Table D.1>>.
...
The constants `a` and `b` are dimensionless, and `depth_c` [aqua-background]#must# have units of length.
...
**The description of this type of parametric vertical coordinate is defective in version 1.8 and earlier versions of these conventions, in that it does not state what values the vertical coordinate variable [aqua-background]#should# contain.
...
Both `sigma` and `zlev` [aqua-background]#must# have this dimension.
For any `k`, whichever of `sigma(k)` or `zlev(k)` is undefined [aqua-background]#must# contain missing data, while the other [aqua-background]#must not#.
...
The pass:q[`standard_name`]s for `eta`, `depth`, `zlev` and the `computed_standard_name` [aqua-background]#must# be one of the consistent sets shown in <<table-computed-standard-names, Table D.1>>.
The `standard_name` for `sigma` is `ocean_sigma_coordinate`.
No `standard_name` has been defined for `depth_c` or `nsigma`.
The `nsigma` parameter is deprecated and [aqua-background]#optional# in `formula_terms`; if supplied, it [aqua-background]#must# equal the number of elements of `zlev` which contain missing data.

The `standard_name` for the vertical coordinate variable is `ocean_sigma_z_coordinate`.
This variable [aqua-background]#should# contain `sigma(k)*depth_c` for the layers where `sigma` is defined and `zlev(k)` for the other layers, with units of length.
The layers [aqua-background]#must# be arranged so that the vertical coordinate variable contains a strictly monotonic set of indicative values for the heights of the levels relative to the datum, either increasing or decreasing, and the direction [aqua-background]#must# be indicated by the `positive` attribute, in the usual way for a vertical coordinate variable.
...
The `standard_name` for `depth` and the `computed_standard_name` [aqua-background]#must# be one of the consistent sets shown in <<table-computed-standard-names, Table D.1>>.
