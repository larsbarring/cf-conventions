...
Therefore, in accordance with the rules, all versions of the standard before 1.9 are [lime-background]#deprecated# for datasets that use the "ocean sigma over z" coordinate.**
...
The `nsigma` parameter is [lime-background]#deprecated# and [aqua-background]#optional# in `formula_terms`; if supplied, it [aqua-background]#must# equal the number of elements of `zlev` which contain missing data.
