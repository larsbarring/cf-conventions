...
Each attribute [aqua-background]#may# be used in any of the ways shown in its "Use" entry.
**G** indicates it [lime-background]#can# appear as a global attribute, and **Gr** as a group attribute.
...
In cases where there is a strong constraint on dataset size, it is [lime-background]#allowed# to pack the coordinate variables (using add_offset and/or scale_factor), but this is not [aqua-background]#recommended# in general.
...
Allowed for auxiliary coordinate variables but [lime-background]#not allowed# for coordinate variables.
