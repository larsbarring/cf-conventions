...
We extend COARDS by adding the [red]#optional# **`standard_name`** attribute which is used to provide unique identifiers for variables.
...
But since it is an [red]#optional# attribute, applications that implement these standards [red]#must# continue to be able to identify coordinate types based on the COARDS conventions.


[[units, Section 3.1, "Units"]]
=== Units

The **`units`** attribute is [red]#required# for all variables that represent dimensional quantities (except for boundary variables defined in <<cell-boundaries>> and climatology variables defined in <<climatological-statistics>>).
...
The COARDS convention prohibits the unit `degrees` altogether, but this unit is not forbidden by the CF convention because it [red]#may# in fact be appropriate for a variable containing, say, solar zenith angle.
...
In this case the coordinate values are not true latitudes and longitudes which [red]#must# always be identified using the more specific forms of `degrees` as described in <<latitude-coordinate>> and <<longitude-coordinate>>.

Units are not [red]#required# for dimensionless quantities.
A variable with no **`units`** attribute is assumed to be dimensionless.
However, a **`units`** attribute specifying a dimensionless unit [red]#may# optionally be included.
The canonical unit (see also <<standard-name>>) for dimensionless quantities that represent fractions, or parts of a whole, is `1`.
When a dimensionless quantity is a ratio of dimensional quantities, CF suggests that it [red]#may# be informative to users of data if the **`units`** are given as ratio of dimensional units, for instance `mg kg-1` for a mass ratio of 1e-6, or `microlitre litre-1` for a volume ratio of 1e-6.
...
"area fraction" or "probability") does not belong in the **`units`** attribute, but [red]#should# be given in the **`long_name`** or **`standard_name`** attributes (see <<long-name>> and <<standard-name>>), in the same way as for physical quantities with dimensional units.
...
The application of any scale factors or offsets to data [red]#should# be indicated by the **`scale_factor`** and **`add_offset`** attributes.
...
But it is highly [red]#recommended# that either this or the **`standard_name`** attribute defined in the next section be provided to make the file self-describing.
If a variable has no **`long_name`** attribute then an application [red]#may# use, as a default, the **`standard_name`** if it exists, or the variable name itself.
...
For this reason each variable [red]#may# optionally be given a "standard name", whose meaning is defined by this convention.
There [red]#may# be several variables in a dataset with any given standard name, and these [red]#may# be distinguished by other metadata, such as coordinates (<<coordinate-types>>) and **`cell_methods`** (<<cell-methods>>).
...
We don't attempt to provide precise definitions of fundumental physical quantities (e.g., temperature) which [red]#may# be found in the literature.
The description [red]#may# define rules on the variable type, attributes and coordinates which [red]#must# be complied with by any variable carrying that standard name (such as in example 3.4).
...
and this table [red]#may# be consulted in order to find the standard name that [red]#should# be assigned to a variable.
...
For example, a variable [red]#may# contain data to which spatial or temporal operations have been applied.
Or the data [red]#may# represent an uncertainty in the measurement of a quantity.
...
Other types of quantity modifiers are expressed using the [red]#optional# modifier part of the **`standard_name`** attribute.
...
When one data variable provides metadata about the individual values of another data variable it [red]#may# be desirable to express this association by providing a link between the variables.
For example, instrument data [red]#may# have associated measures of uncertainty.
...
The nature of the relationship between variables associated via **`ancillary_variables`** [red]#must# be determined by other attributes.
...
The dimensions of an ancillary variable [red]#must# be the same as or a subset of the dimensions of the variable to which it is related, but their order is not restricted, and with one exception:
If an ancillary variable of a data variable that has been compressed by gathering (<<compression-by-gathering>>) does not span the compressed dimension, then its dimensions [red]#may# be any subset of the data variable's uncompressed dimensions, i.e. any of the dimensions of the data variable except the compressed dimension, and any of the dimensions listed by the **`compress`** attribute of the compressed coordinate variable.
...
Status codes and Boolean (binary) condition flags [red]#may# be expressed with different combinations of **`flag_values`** and **`flag_masks`** attribute definitions.
...
Each word or phrase [red]#should# consist of characters from the alphanumeric set and the following five: '_', '-', '.', '+', '@'.
If multi-word phrases are used to describe the flag values, then the words within a phrase [red]#should# be connected with underscores.
...
Thus, any or all of the flagged conditions [red]#may# be true, depending on the variable bit settings.
...
A variable with standard name of `region`, `area_type` or any other standard name which requires string-valued values from a defined list [red]#may# use flags together with `flag_values` and `flag_meanings` attributes to record the translation to the string values.
...
Each **`flag_values`** and **`flag_masks`** value [red]#must# coincide with a **`flag_meanings`** value.
...
The "12b" flag mask is repeated in the **`sensor_status_qc`** **`flag_masks`** definition to explicitly declare the [red]#recommended# bit field masks to repeatedly AND with the variable value while searching for matching enumerated values.
