...
The COARDS conventions are extended by adding the [aqua-background]#optional# **`standard_name`** attribute which is used to provide unique identifiers for variables.
...
The **`standard_name`** attribute [lime-background]#can# be used to identify variables that contain coordinate data.
But since it is an [aqua-background]#optional# attribute, applications that implement these conventions [aqua-background]#must# continue to be able to identify coordinate types based on the COARDS conventions.

[[units, Section 3.1, "Units"]]
=== Units

The **`units`** attribute is [aqua-background]#required# for all variables that represent dimensional quantities (except for boundary variables defined in <<cell-boundaries>> and climatology boundary variables defined in <<climatological-statistics>>).
The **`units`** attribute is [lime-background]#permitted# but not [aqua-background]#required# for dimensionless quantities (see <<dimensionless-units>>).
If multiplication by a dimensionless constant and addition of a dimensionless constant are the only operations [aqua-background]#required# for the value of a dimensional quantity expressed in one unit to be converted to the value expressed in another unit, the two units are considered __physically equivalent__.

The value of the **`units`** attribute is a string that [lime-background]#can# be recognized by the UDUNITS package <<UDUNITS>>, with the exceptions that are given in <<dimensionless-units>> and <<units-multiples>>.
...
CF does not assume or [lime-background]#require# that the UDUNITS software will be used for **`units`** conversion.
In most **`units`** conversions, the sole operation on the data is multiplication by a scale factor.
Special treatment is [aqua-background]#required# in converting the **`units`** of variables that involve temperature (<<temperature-units>>) and the **`units`** of time coordinate variables (<<time-coordinate>>).

The COARDS conventions prohibit the unit `degrees` altogether, but this unit is not [lime-background]#forbidden# by the CF conventions because it [aqua-background]#may# in fact be appropriate for a variable containing, say, solar zenith angle.
The unit `degrees` is also [lime-background]#allowed# on coordinate variables such as the latitude and longitude coordinates of a transformed grid.
In this case the coordinate values are not true latitudes and longitudes, which [aqua-background]#must# always be identified using the more specific forms of `degrees` as described in <<latitude-coordinate>> and <<longitude-coordinate>>.
...
However, a **`units`** attribute specifying a dimensionless unit [aqua-background]#may# optionally be included.
...
These units are [lime-background]#allowed# in the **`units`** attribute by CF only if the data variable has no **`standard_name`**.
These units are [lime-background]#prohibited# by CF if there is a **`standard_name`**, because the **`standard_name`** defines whether the quantity is a volume ratio, so the **`units`** are needed only to indicate a dimensionless number.

Information describing a dimensionless physical quantity itself (e.g.
"area fraction" or "probability") does not belong in the **`units`** attribute, but [aqua-background]#should# be given in the **`long_name`** or **`standard_name`** attributes (see <<long-name>> and <<standard-name>>), in the same way as for physical quantities with dimensional units.
As an exception, to maintain backwards compatibility with COARDS, the text strings `level`, `layer`, and `sigma_level` are [lime-background]#allowed# in the **`units`** attribute, in order to indicate dimensionless vertical coordinates.
...
When **`units`** of on-scale temperature are converted, the data [aqua-background]#may# [lime-background]#require# the addition of an offset as well as multiplication by a scale factor, because the physical meaning of a numerical value of zero for an on-scale temperature depends on the unit of measurement.
...
Therefore to convert the **`units`** of a temperature difference [lime-background]#requires# only multiplication by a scale factor, without the addition of an offset.
...
A **`standard_name`** (<<standard-name>>) or **`standard_name`** modifier (<<standard-name-modifiers>>) [aqua-background]#may# clarify the intention, but they are [aqua-background]#optional#.
Some statistical operations described by the **`cell_methods`** attribute (<<cell-methods>>; <<appendix-cell-methods>>) imply that temperature [aqua-background]#must# be interpreted as temperature difference, but this attribute is [aqua-background]#optional# too.

In order to convert the **`units`** correctly, it is essential to know whether a temperature is on-scale or a difference.
Therefore these conventions strongly [lime-background]#recommend# that any variable whose **`units`** involve a temperature unit [aqua-background]#should# also have a **`units_metadata`** attribute to make the distinction.
This attribute [aqua-background]#must# have one of the following three values: `temperature: on_scale`, `temperature: difference`, `temperature: unknown`.
The **`units_metadata`** attribute, **`standard_name`** modifier (<<standard-name-modifiers>>) and **`cell_methods`** attribute (<<appendix-cell-methods>>) [aqua-background]#must# be consistent if present.
...
If the **`units_metadata`** attribute is not present, the data-reader [aqua-background]#should# assume `temperature: unknown`.
The **`units_metadata`** attribute was introduced in CF 1.11.
In data written according to versions before 1.11, `temperature: unknown` [aqua-background]#should# be assumed for all **`units`** involving temperature, if it cannot be deduced from other metadata.
...
With `temperature: on_scale`, correct conversion [lime-background]#can# be guaranteed only for pure temperature **`units`**.
...
For instance, when temperature is on-scale, a value in `kg degree_C m-2` [lime-background]#can# be converted to a value in `kg K m-2` only if the values in `degree_C` and `kg m-2` of which it is the product are separately known.


[[units-multiples, Section 3.1.3, "Scale factors and offsets"]]
==== Scale factors and offsets

UDUNITS recognises the <<SI>> prefixes shown in <<table-supported-units, Table 3.1>> for decimal multiples and submultiples of units, and [lime-background]#allows# them to be applied to non-SI units as well.
...
The UDUNITS syntax [lime-background]#allows# dimensionless numbers to appear in **`units`**, as multiplicative factors before the name of the unit and as additive offsets to the named unit (following `@` or various keywords).
This UDUNITS syntax for arbitrary transformation of **`units`** [aqua-background]#must not# be used with the CF conventions, except for offsets following **`since`** for specifying reference time (<<time-coordinate>>).

The application of any scale factors or offsets to the data values, without altering the **`units`**, [aqua-background]#should# be indicated by the **`scale_factor`** and **`add_offset`** attributes.
...
The **`long_name`** attribute is defined by the <<NUG>> to contain a long descriptive name which [aqua-background]#may#, for example, be used for labeling plots.
For backwards compatibility with COARDS this attribute is [aqua-background]#optional#.
But it is highly [aqua-background]#recommended# that either this or the **`standard_name`** attribute defined in the next section be provided for all data variables and variables containing coordinate data, in order to make the file self-describing.
If a variable has no **`long_name`** attribute then an application [aqua-background]#may# use, as a default, the **`standard_name`** if it exists, or the variable name itself.
...
For many applications it is desirable to have a more definitive description of the quantity, which [lime-background]#allows# users of data from different sources (some of which [lime-background]#might# be models and others observational) to determine whether quantities are in fact comparable.
For this reason each variable [aqua-background]#may# optionally be given a "standard name", whose meaning is defined by these conventions.
There [aqua-background]#may# be several variables in a dataset with any given standard name, and these [aqua-background]#may# be distinguished by other metadata, such as coordinates (<<coordinate-types>>) and **`cell_methods`** (<<cell-methods>>).
...
The reference datetime in the canonical unit (the beginning of the day i.e. midnight on 1st January 1972 at 0 `degrees_east`) is not restrictive; the time coordinate variable's own **`units`** [aqua-background]#may# contain any reference datetime (after **`since`**) that is valid in its calendar.
...
In both kinds of time **`units`** attribute (with or without **`since`**), any unit for measuring time [lime-background]#can# be used i.e. any unit which is physically equivalent to the SI base unit of time, namely the second.
...
No attempt is made to provide precise definitions of fundumental physical quantities (e.g., temperature) which [aqua-background]#may# be found in the literature.
The description [aqua-background]#may# define rules on the variable type, attributes and coordinates which [aqua-background]#must# be complied with by any variable carrying that standard name (such as in Example 3.5).
...
and this table [aqua-background]#may# be consulted in order to find the standard name that [aqua-background]#should# be assigned to a variable.
Some standard names (e.g. **`region`**, <<geographic-regions>>, and **`area_type`**, <<statistics-applying-portions>>) are used to indicate quantities which are [lime-background]#permitted# to take only certain standard values.
This is indicated in the definition of the quantity in the standard name table, accompanied by a list or a link to a list of the [lime-background]#permitted# values.

Standard names by themselves are not always sufficient to describe a quantity.
For example, a variable [aqua-background]#may# contain data to which spatial or temporal operations have been applied.
Or the data [aqua-background]#may# represent an uncertainty in the measurement of a quantity.
...
Other types of quantity modifiers are expressed using the [aqua-background]#optional# modifier part of the **`standard_name`** attribute.
...
When one data variable provides metadata about the individual values of another data variable it [aqua-background]#may# be desirable to express this association by providing a link between the variables.
For example, instrument data [aqua-background]#may# have associated measures of uncertainty.
...
The nature of the relationship between a data variable and its ancillary variables (those named by its **`ancillary_variables`** attribute) [aqua-background]#must# be determined by other attributes.
...
The norm data and metadata (ancillary) variables [aqua-background]#must# have **`cell_methods`** attributes of their own.

An ancillary variable [aqua-background]#must# have all or a subset of the dimensions of the variable to which it is related, with one exception:
If an ancillary variable of a data variable that has been compressed by gathering (<<compression-by-gathering>>) does not span the compressed dimension, then its dimensions [aqua-background]#may# be any subset of the data variable's uncompressed dimensions, i.e. it [aqua-background]#may# have any of the dimensions listed by the **`compress`** attribute of the compressed coordinate variable, as well as any of the other dimensions of the data variable (those apart from the compressed dimension).
The order of dimensions of an ancillary variable is not restricted; it does not [lime-background]#have to# be the same as for the parent data variable.
An ancillary variable [aqua-background]#may# have auxiliary coordinate variables, named by a **`coordinates`** attribute, that are auxiliary coordinate variables of its parent data variable (<<coordinate-system>>), provided their dimensions are also dimensions of the ancillary variable.
An ancillary variable [aqua-background]#may# have any scalar coordinate variable (<<scalar-coordinate-variables>>) that is also a scalar coordinate variable of the parent data variable.
...
Status codes and Boolean (binary) condition flags [aqua-background]#may# be expressed with different combinations of **`flag_values`** and **`flag_masks`** attribute definitions.
...
Each word or phrase [aqua-background]#should# consist of characters from the alphanumeric set and the following five: '_', '-', '.', '+', '@'.
If multi-word phrases are used to describe the flag values, then the words within a phrase [aqua-background]#should# be connected with underscores.
...
Thus, any or all of the flagged conditions [aqua-background]#may# be true, depending on the variable bit settings.
...
A variable with standard name of `region`, `area_type` or any other standard name which [lime-background]#requires# string-valued values from a defined list [aqua-background]#may# use flags together with `flag_values` and `flag_meanings` attributes to record the translation to the string values.
...
Each **`flag_values`** and **`flag_masks`** value [aqua-background]#must# coincide with a **`flag_meanings`** value.
...
The "12b" flag mask is repeated in the **`sensor_status_qc`** **`flag_masks`** definition to explicitly declare the [aqua-background]#recommended# bit field masks to repeatedly AND with the variable value while searching for matching enumerated values.
