...
We extend COARDS by adding the [aqua-background]#optional# **`standard_name`** attribute which is used to provide unique identifiers for variables.
...
But since it is an [aqua-background]#optional# attribute, applications that implement these standards [aqua-background]#must# continue to be able to identify coordinate types based on the COARDS conventions.

[[units, Section 3.1, "Units"]]
=== Units

The **`units`** attribute is [aqua-background]#required# for all variables that represent dimensional quantities (except for boundary variables defined in <<cell-boundaries>> and climatology variables defined in <<climatological-statistics>>).
...
The COARDS convention prohibits the unit `degrees` altogether, but this unit is not [lime-background]#forbidden# by the CF convention because it [aqua-background]#may# in fact be appropriate for a variable containing, say, solar zenith angle.
...
The UDUNITS syntax that [lime-background]#allows# scale factors and offsets to be applied to a unit is not supported by this standard, except for case of specifying reference time, see section <<time-coordinate>>.
The application of any scale factors or offsets to data [aqua-background]#should# be indicated by the **`scale_factor`** and **`add_offset`** attributes.
...
Therefore this standard strongly [lime-background]#recommends# that any variable whose **`units`** involve a temperature unit [aqua-background]#should# also have a **`units_metadata`** attribute to make the distinction.
...
For instance, when temperature is on-scale, a value in `kg degree_C m-2` [lime-background]#can# be converted to a value in `kg K m-2` only if we know separately the values in `degree_C` and `kg m-2` of which it is the product.


[[units-multiples, Section 3.1.3, "Scale factors and offsets"]]
==== Scale factors and offsets

UDUNITS recognises the SI prefixes shown in <<table-supported-units>> for decimal multiples and submultiples of units, and [lime-background]#allows# them to be applied to non-SI units as well.
...
For this reason each variable [aqua-background]#may# optionally be given a "standard name", whose meaning is defined by this convention.
...
We don't attempt to provide precise definitions of fundumental physical quantities (e.g., temperature) which [aqua-background]#may# be found in the literature.
...
Some standard names (e.g. **`region`** and **`area_type`**) are used to indicate quantities which are [lime-background]#permitted# to take only certain standard values.
...
The nature of the relationship between variables associated via **`ancillary_variables`** [aqua-background]#must# be determined by other attributes.
...
The dimensions of an ancillary variable [aqua-background]#must# be the same as or a subset of the dimensions of the variable to which it is related, but their order is not restricted, and with one exception:
