...
We extend COARDS by adding the [red aqua-background]#optional# **`standard_name`** attribute which is used to provide unique identifiers for variables.
...
The **`standard_name`** attribute [black aqua-background]#can# be used to identify variables that contain coordinate data.
But since it is an [red aqua-background]#optional# attribute, applications that implement these standards [red aqua-background]#must# continue to be able to identify coordinate types based on the COARDS conventions.

[[units, Section 3.1, "Units"]]
=== Units

The **`units`** attribute is [red aqua-background]#required# for all variables that represent dimensional quantities (except for boundary variables defined in <<cell-boundaries>> and climatology variables defined in <<climatological-statistics>>).
The **`units`** attribute is permitted but not [red aqua-background]#required# for dimensionless quantities (see <<dimensionless-units>>).

The value of the **`units`** attribute is a string that [black aqua-background]#can# be recognized by the UDUNITS package <<UDUNITS>>, with the exceptions that are given in <<dimensionless-units>> and <<units-multiples>>.
...
Special treatment is [red aqua-background]#required# in converting the **`units`** of variables that involve temperature (<<temperature-units>>) and the **`units`** of time coordinate variables (<<time-coordinate>>).

The COARDS convention prohibits the unit `degrees` altogether, but this unit is not forbidden by the CF convention because it [red aqua-background]#may# in fact be appropriate for a variable containing, say, solar zenith angle.
...
In this case the coordinate values are not true latitudes and longitudes, which [red aqua-background]#must# always be identified using the more specific forms of `degrees` as described in <<latitude-coordinate>> and <<longitude-coordinate>>.
...
However, a **`units`** attribute specifying a dimensionless unit [red aqua-background]#may# optionally be included.
...
"area fraction" or "probability") does not belong in the **`units`** attribute, but [red aqua-background]#should# be given in the **`long_name`** or **`standard_name`** attributes (see <<long-name>> and <<standard-name>>), in the same way as for physical quantities with dimensional units.
...
This use of **`units`** is not compatible with UDUNITS, and is [black aqua-background]#deprecated# by this standard because conventions for more precisely identifying dimensionless vertical coordinates are available (see <<dimensionless-vertical-coordinate>>).
...
The application of any scale factors or offsets to data [red aqua-background]#should# be indicated by the **`scale_factor`** and **`add_offset`** attributes.
...
When **`units`** of on-scale temperature are converted, the data [red aqua-background]#may# require the addition of an offset as well as multiplication by a scale factor, because the physical meaning of a numerical value of zero for an on-scale temperature depends on the unit of measurement.
...
A **`standard_name`** (<<standard-name>>) or **`standard_name`** modifier (<<standard-name-modifiers>>) [red aqua-background]#may# clarify the intention, but they are optional.
Some statistical operations described by the **`cell_methods`** attribute (<<cell-methods>>; <<appendix-cell-methods>>) imply that temperature [red aqua-background]#must# be interpreted as temperature difference, but this attribute is [red aqua-background]#optional# too.

In order to convert the **`units`** correctly, it is essential to know whether a temperature is on-scale or a difference.
Therefore this standard strongly [black aqua-background]#recommends# that any variable whose **`units`** involve a temperature unit [red aqua-background]#should# also have a **`units_metadata`** attribute to make the distinction.
This attribute [red aqua-background]#must# have one of the following three values: `temperature: on_scale`, `temperature: difference`, `temperature: unknown`.
The **`units_metadata`** attribute, **`standard_name`** modifier (<<standard-name-modifiers>>) and **`cell_methods`** attribute (<<appendix-cell-methods>>) [red aqua-background]#must# be consistent if present.
...
If the **`units_metadata`** attribute is not present, the data-reader [red aqua-background]#should# assume `temperature: unknown`.
The **`units_metadata`** attribute was introduced in CF 1.11.
In data written according to versions before 1.11, `temperature: unknown` [red aqua-background]#should# be assumed for all **`units`** involving temperature, if it cannot be deduced from other metadata.
...
With `temperature: on_scale`, correct conversion [black aqua-background]#can# be guaranteed only for pure temperature **`units`**.
...
For instance, when temperature is on-scale, a value in `kg degree_C m-2` [black aqua-background]#can# be converted to a value in `kg K m-2` only if we know separately the values in `degree_C` and `kg m-2` of which it is the product.
...
But it is highly [red aqua-background]#recommended# that either this or the **`standard_name`** attribute defined in the next section be provided to make the file self-describing.
If a variable has no **`long_name`** attribute then an application [red aqua-background]#may# use, as a default, the **`standard_name`** if it exists, or the variable name itself.
...
For this reason each variable [red aqua-background]#may# optionally be given a "standard name", whose meaning is defined by this convention.
There [red aqua-background]#may# be several variables in a dataset with any given standard name, and these [red aqua-background]#may# be distinguished by other metadata, such as coordinates (<<coordinate-types>>) and **`cell_methods`** (<<cell-methods>>).
...
We don't attempt to provide precise definitions of fundumental physical quantities (e.g., temperature) which [red aqua-background]#may# be found in the literature.
The description [red aqua-background]#may# define rules on the variable type, attributes and coordinates which [red aqua-background]#must# be complied with by any variable carrying that standard name (such as in Example 3.5).
...
and this table [red aqua-background]#may# be consulted in order to find the standard name that [red aqua-background]#should# be assigned to a variable.
...
For example, a variable [red aqua-background]#may# contain data to which spatial or temporal operations have been applied.
Or the data [red aqua-background]#may# represent an uncertainty in the measurement of a quantity.
...
Other types of quantity modifiers are expressed using the [red aqua-background]#optional# modifier part of the **`standard_name`** attribute.
...
When one data variable provides metadata about the individual values of another data variable it [red aqua-background]#may# be desirable to express this association by providing a link between the variables.
For example, instrument data [red aqua-background]#may# have associated measures of uncertainty.
...
The nature of the relationship between variables associated via **`ancillary_variables`** [red aqua-background]#must# be determined by other attributes.
...
The dimensions of an ancillary variable [red aqua-background]#must# be the same as or a subset of the dimensions of the variable to which it is related, but their order is not restricted, and with one exception:
If an ancillary variable of a data variable that has been compressed by gathering (<<compression-by-gathering>>) does not span the compressed dimension, then its dimensions [red aqua-background]#may# be any subset of the data variable's uncompressed dimensions, i.e. any of the dimensions of the data variable except the compressed dimension, and any of the dimensions listed by the **`compress`** attribute of the compressed coordinate variable.
...
Status codes and Boolean (binary) condition flags [red aqua-background]#may# be expressed with different combinations of **`flag_values`** and **`flag_masks`** attribute definitions.
...
Each word or phrase [red aqua-background]#should# consist of characters from the alphanumeric set and the following five: '_', '-', '.', '+', '@'.
If multi-word phrases are used to describe the flag values, then the words within a phrase [red aqua-background]#should# be connected with underscores.
...
Thus, any or all of the flagged conditions [red aqua-background]#may# be true, depending on the variable bit settings.
...
A variable with standard name of `region`, `area_type` or any other standard name which requires string-valued values from a defined list [red aqua-background]#may# use flags together with `flag_values` and `flag_meanings` attributes to record the translation to the string values.
...
Each **`flag_values`** and **`flag_masks`** value [red aqua-background]#must# coincide with a **`flag_meanings`** value.
...
The "12b" flag mask is repeated in the **`sensor_status_qc`** **`flag_masks`** definition to explicitly declare the [red aqua-background]#recommended# bit field masks to repeatedly AND with the variable value while searching for matching enumerated values.
