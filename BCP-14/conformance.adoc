...
* Any text stored in a CF attribute or variable [aqua-background]#must# be represented in Unicode Normalization Form C and encoded in UTF-8.

* Any attribute of variable-length string type [aqua-background]#must# be a scalar (not an array).
...
* Each aggregated dimension [aqua-background]#must# name a dimension in the file.

* An aggregation variable [aqua-background]#must# be a scalar.

* An aggregation variable [aqua-background]#must# have an **`aggregated_data`** attribute whose string value comprises blank-separated elements of the form __feature: variable__.
  Each __variable__ [aqua-background]#must# be the name of a variable in the file.
  The feature keywords [aqua-background]#must# comprise either all three of `map`, `uris`, and `identifiers`; or else both of `map` and `unique_values`.

* A `uris` variable [aqua-background]#must# have a string data type.

  ** The `uris` variable [aqua-background]#must# have the same number of dimensions as there are aggregated dimensions.
  ** The size of each dimension [aqua-background]#must# be the equal to number of valid values in the corresponding row of the `map` variable.
  ** The variable [aqua-background]#must# have no missing values.
  ** A data value of a `uris` variable [aqua-background]#must# be either an absolute URI or else a relative-path URI reference.

* An `identifiers` variable [aqua-background]#must# be either a scalar, or else have the same dimensions in the same order as the `uris` variable.
  ** The variable [aqua-background]#must# have no missing values.

* The `unique_values` variable [aqua-background]#must# have the same number of dimensions as there are aggregated dimensions.
  ** The size of each dimension [aqua-background]#must# be equal to the number of valid values in the corresponding row of the `map` variable.

* The `map` variable [aqua-background]#must# have an integer data type.
  ** If there are zero aggregated dimensions then the `map` variable [aqua-background]#must# a be scalar and contain the value `1`.
  ** If there are one or more aggregated dimensions then the `map` variable [aqua-background]#must# be two-dimensional.
  *** The size of the slower-varying dimension (i.e. the first dimension in CDL order, representing rows) [aqua-background]#must# be the number of aggregated dimensions.
  *** The sum of the valid values in a row [aqua-background]#must# equal the size of the corresponding aggregated dimension.

*Recommendations:*

* The following kinds of variable [aqua-background]#should not# be aggregation variables: grid mapping variables, domain variables, mesh topology variables, geometry container variables, interpolation variables, and quantization variables.
...
* The **`units`** string [aqua-background]#must not# contain literal numbers (integer or real) as factors or offsets to a named unit of measure.
An equivalent statement, using the syntax and terms of the link:$$https://docs.unidata.ucar.edu/udunits/current/udunits2lib.html#Grammar$$[UDUNITS Unit Grammar], is that **`INT`** or **`REAL`** are [lime-background]#not allowed# in a **`units`** string that contains **`ID`**, except for the specification of a reference time in the units of time coordinates.
...
* If present, the **`units_metadata`** attribute [aqua-background]#must# have one of these values: `temperature: on_scale`, `temperature: difference`, `temperature: unknown`, `leap_seconds: none`, `leap_seconds: utc`, or `leap_seconds: unknown`.
...
* If a variable has a **`units`** attribute that involves a temperature unit, and has a **`cell_methods`** attribute includes any entry with any of the methods `range`, `standard_deviation` or `variance`, then the **`units_metadata`** attribute, if present, [aqua-background]#must# have the value `temperature: difference`.
* A variable [aqua-background]#must not# have a **`units_metadata`** attribute if it has no **`units`** attribute, or if its **`units`** do not involve a temperature unit or a reference time unit.
...
* The time **`units`** of a time coordinate variable [aqua-background]#must# contain a reference datetime.
* A non-zero time zone offset [aqua-background]#must not# appear in the reference datetime in the **`utc`** and **`tai`** calendars.
* The reference datetime [aqua-background]#must# be written in the correct format.
In particular, it [aqua-background]#must# include the date, and the time zone offset [aqua-background]#must not# be included unless the time is included.
...
* Units of **`year`** and **`month`** [aqua-background]#should not# be used in any calendar.
* Units of **`day`**, **`hour`** and **`minute`** [aqua-background]#should not# be used in the **`utc`** calendar. In that calendar, the only [aqua-background]#recommended# unit is **`second`**.
* The decimal prefixes of Table 3.1 [aqua-background]#should not# be used with any unit of time apart from **`second`**.
...
* A non-zero time zone offset [aqua-background]#should not# be specified in the reference datetime.
* In datetime strings, leading zeroes [aqua-background]#should# be inserted where needed such that the year has at least four digits, and that any other components included have two digits (the month, day, hour, minute, the integer part of second, and the hour and minute in the time zone offset).
...
* The **`calendar`** attribute [aqua-background]#may# only be attached to time coordinate variables.
* If present, the value of the **`calendar`** attribute [aqua-background]#must# be one of the standardized values (case insensitive) detailed in this section, unless the **`month_length`** attribute is present, in which case the **`calendar`** attribute [aqua-background]#must# _not_ take one of the standardized values.
...
Datetimes which are invalid in the specified calendar [aqua-background]#must not# be encoded as time coordinates.
* The reference datetime of a time coordinate variable [aqua-background]#must# be a valid datetime in the specified calendar.
...
* In the **`standard`** calendar, a reference datetime before 1582-10-05 00:00:00 [aqua-background]#should not# be used for a time coordinate representing 1582-10-15 00:00:00 or a later datetime, and a reference datetime of 1582-10-15 00:00:00 or later [aqua-background]#should not# be used for a time coordinate representing a datetime earlier than 1582-10-05 00:00:00.
* A datetime during a leap second (i.e. with a value for seconds that equals or exceeds 60) [aqua-background]#should not# be used as the reference datetime of a time coordinate variable if the **`calendar`** is **`utc`**.
(In other calendars, leap seconds [aqua-background]#must not# be used because they are not valid datetimes.)

[[explicitly-defined-calendar]]
=== 4.4.4 Explicitly Defined Calendar

* The **`month_lengths`**, **`leap_year`**, and **`leap_month`** attributes [aqua-background]#may# only be attached to time coordinate variables.
* If the **`calendar`** attribute of a time coordinate variable is given a non-standard value, then the attribute **`month_lengths`** is [aqua-background]#required#, along with **`leap_year`** and **`leap_month`** as appropriate.
...
* The data types of the attributes of the grid mapping variable [aqua-background]#must# be as specified in Table F.1 of Appendix F. +
...
* If the value of the **`indexing_scheme`** attribute is **`nested`** or **`ring`**, then the **`refinement_level`** attribute [aqua-background]#must# also be present.
* If the value of the **`indexing_scheme`** attribute is **`nuniq`** or **`zuniq`**, then the **`refinement_level`** attribute [aqua-background]#must not# be present.
* The value of the **`refinement_level`** attribute [aqua-background]#must# be an integer in the range 0-29.
* A variable with standard name **`healpix_index`** [aqua-background]#must# have an integer data type and all values [aqua-background]#must# be non-negative.
* A variable with standard name **`reduced_gaussian_index`** [aqua-background]#must# have an integer data type.
Its values [aqua-background]#must# be positive, strictly monotonic, and less than the total number of points in the global reduced Gaussian grid.
* The attribute **`grid_subtype`** of the **`reduced_gaussian`** grid mapping [aqua-background]#must# be **`normal`** or **`octahedral`**.
* The value of the attribute **`latitude_dimension`** of the **`reduced_gaussian`** grid mapping [aqua-background]#must# be an even number.
* A one-dimensional variable [aqua-background]#must# exist whose name, and the name of its single dimension, [aqua-background]#must# both equal the **`latitude_dimension`** attribute of the **`reduced_gaussian`** grid mapping.
* A one-dimensional variable [aqua-background]#must# exist whose name, and the name of its single dimension, [aqua-background]#must# both equal the **`latitude_dimension`** attribute of the **`reduced_gaussian`** grid mapping.
...
* An auxiliary coordinate variable with standard name **`healpix_index`**, of size greater than one, all of whose values are distinct, [aqua-background]#should# be a coordinate variable instead (with values ordered monotonically).
...
* If the associated coordinate variable is one-dimensional and of size greater than one, the bounds of each cell [aqua-background]#must# be ordered in the same sense as the coordinates (increasing or decreasing).
...
The specified variable [aqua-background]#must# exist in the file.
* A climatology boundary variable [aqua-background]#must# have the same dimension as its associated time coordinate variable, and have a trailing dimension (CDL order) of size 2.
* A climatology boundary variable [aqua-background]#must# be a numeric data type.
* If a climatology boundary variable has **`units`**, **`standard_name`**, or **`calendar`** attributes, they [aqua-background]#must# agree with those of its associated variable.
* A climatology boundary variable [aqua-background]#must not# have **`_FillValue`** or **`missing_value`** attributes.

=== 7.5 Anomaly data

*Requirements:*

* In a **`cell_methods`** entry of the form __name__**`:`** [__name__**`:`** ...] **`anomaly_wrt`** __norm__, the word __norm__ [aqua-background]#must# be the name of a variable that is present in the file, and is named by the **`ancillary_variables`** attribute of the data variable (hence we hereafter refer to it as the "norm ancillary variable").
* Each of the coordinate variables and scalar coordinate variables identified by the __name__s in the **`anomaly_wrt`** entry of **`cell_methods`** (we refer to these variables hereafter as "anomaly coordinate variables") [aqua-background]#must# have a **`standard_name`** attribute.
* If either the data variable or the norm ancillary variable has a **`standard_name`** attribute, its value [aqua-background]#must not# end in **`_anomaly`**, and if they both have **`standard_name`** attributes, they [aqua-background]#must# have the same value.
* The anomaly coordinate variables [aqua-background]#must not# be coordinate variables or scalar coordinate variables of the norm ancillary variable.
* The coordinate variables and scalar coordinate variables of the norm ancillary variable [aqua-background]#must# satisfy one of the following conditions:
...
* In cases (1) and (2), for each of the anomaly coordinate variables, the norm ancillary variable [aqua-background]#must# have either a coordinate variable with a size-one dimension, or a scalar coordinate variable, with the same **`standard_name`** attribute as the anomaly coordinate variable, and each of these variables [aqua-background]#must# have either a **`bounds`** or a **`climatology`** attribute. (We hereafter refer to these variables as "norm coordinate variables.)
* In cases (1) and (2), the norm ancillary variable [aqua-background]#must# have a **`cell_methods`** attribute with an entry that refers to all the norm coordinate variables, or more than one entry if they contain **`within`** and **`over`** keywords.
* In case (3), the norm ancillary variable [aqua-background]#must# have a **`cell_methods`** attribute comprising entries containing **`within`** and **`over`** keywords and referring to its sole dimension.
* In cases (2) and (3), **`cell_methods**` [aqua-background]#must# have no entries other that the ones described in the previous two points.
* In case (3), the norm ancillary variable [aqua-background]#must# have an auxiliary coordinate variable of integer type with a **`select`** attribute naming a time coordinate variable that has a **`climatology`** attribute. The auxiliary coordinate values [aqua-background]#must# be non-negative and less than the dimension named by the **`select`** attribute.
