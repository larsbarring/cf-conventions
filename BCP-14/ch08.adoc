...
However a simple packing [red aqua-background]#may# be achieved through the use of the [red aqua-background]#optional# NUG defined attributes **`scale_factor`** and **`add_offset`**.
...
When scaled data are written, the application [red aqua-background]#should# first subtract the offset and then divide by the scale factor.
The units of a variable [red aqua-background]#should# be representative of the unpacked data.
...
When packed data is written, the **`scale_factor`** and **`add_offset`** attributes [red aqua-background]#must# be of the same type as the unpacked data, which [red aqua-background]#must# be either **`float`** or **`double`**. Data of type **`float`** [red aqua-background]#must# be packed into one of these types: **`byte`**, **`unsigned byte`**, **`short`**, **`unsigned short`**. Data of type **`double`** [red aqua-background]#must# be packed into one of these types: **`byte`**, **`unsigned byte`**, **`short`**, **`unsigned short`**, **`int`**, **`unsigned int`**.

When packed data is read, it [red aqua-background]#should# be unpacked to the type of the **`scale_factor`** and **`add_offset`** attributes, which [red aqua-background]#must# have the same type if both are present. For guidance only, we suggest that packed data which does not conform to the rules of this section regarding the types of the data variable and attributes [red aqua-background]#should# be unpacked to **`double`** type, in order to minimise the risk of loss of precision.

When data to be packed contains missing values the attributes that indicate missing values (**`_FillValue`**, **`valid_min`**, **`valid_max`**, **`valid_range`**) [red aqua-background]#must# be of the same data type as the packed data.
See <<missing-data>> for a discussion of how applications [red aqua-background]#should# treat variables that have attributes indicating both missing values and transformations defined by a scale and/or offset.

[[compression-by-gathering, Section 8.2, "Lossless Compression by Gathering"]]
=== Lossless Compression by Gathering

To save space in the netCDF file, it [red aqua-background]#may# be desirable to eliminate points from data arrays that are invariably missing.
Such a compression [black aqua-background]#can# operate over one or more adjacent axes, and is accomplished with reference to a list of the points to be stored.
...
The list is the set of indices in this one-dimensional mask of the [red aqua-background]#required# points.
...
The uncompressed variables [black aqua-background]#can# be reconstituted exactly as they were using this information.
The list variable [red aqua-background]#must not# have an associated boundary variable.
...
variables:
  ...
  double time(time);
----
This information implies that the salinity field [red aqua-background]#should# be uncompressed to an array with dimensions `(depth,lat,lon)`.
====
...
For some applications the coordinates of a data variable [black aqua-background]#can# require considerably more storage than the data itself.
Space [red aqua-background]#may# be saved in the netCDF file by storing a subsample of the coordinates that describe the data.
The uncompressed coordinate and auxiliary coordinate variables [black aqua-background]#can# be reconstituted by interpolation, from the subsampled coordinate values to the domain of the data (i.e. the target domain).
...
The creator of the compressed dataset [black aqua-background]#can# control the accuracy of the reconstituted coordinates through the degree of subsampling and the choice of interpolation method, see <<appendix-coordinate-subsampling>>.
...
As an interpolation method relies on the regularity and continuity of the coordinate values within each interpolation subarea, special attention [red aqua-background]#must# be given to the case when uncompressed coordinates contain discontinuities.
A discontinuity [black aqua-background]#could# be an overlap or a gap in the coordinates' coverage, or a change in cell size or cell alignment.
As an example, such discontinuities are common in remote sensing data and [red aqua-background]#may# be caused by combinations of the instrument scan motion, the motion of the sensor platform and changes in the instrument scan mode.
...
Such a shared tie point coordinate [black aqua-background]#can# only be located in one of a pair of adjacent interpolation subareas, which is always the first of the pair in index space.
...
When applied for a given interpolation subarea, interpolation methods (such as those described in <<appendix-coordinate-subsampling>>) [red aqua-background]#must# ensure that reconstituted coordinate points are only generated inside the interpolation subarea being processed, even if some of the tie point coordinates lie outside of that interpolation subarea.
...
Tie point coordinate variables for both coordinate and auxiliary coordinate variables [red aqua-background]#must# be defined as numeric data types and are not allowed to have missing values.
...
To indicate that coordinate interpolation is required, a **`coordinate_interpolation`** attribute [red aqua-background]#must# be defined for a data variable.
...
For example, to specify that the tie point coordinate variables `lat` and `lon` are to be interpolated according to the interpolation variable `bi_linear` [black aqua-background]#could# be indicated with `lat: lon: bi_linear`.

[[compression-by-coordinate-subsampling-interpolation-variable, Section 8.3.3, "Interpolation Variable"]]
==== Interpolation Variable

The method used to uncompress the tie point coordinate variables is described by an interpolation variable that acts as a container for the attributes that define the interpolation technique and the parameters that [red aqua-background]#should# be used.
The variable [red aqua-background]#should# be a scalar (i.e. it has no dimensions) of arbitrary type, and the value of its single element is immaterial.

The interpolation method [red aqua-background]#must# be identified in one of two ways.
...
These attributes [red aqua-background]#must not# be both set.

The valid values of **`interpolation_name`** are given in <<appendix-coordinate-subsampling>>.
This appendix describes the interpolation technique for each method, and [red aqua-background]#optional# interpolation variable attributes for configuring the interpolation process.

If a standardized interpolation name is not given, the interpolation variable [red aqua-background]#must# have an **`interpolation_description`** attribute defined instead, containing a description of the non-standardised interpolation (in a similar manner to a long name being used instead of a standard name).
This description is free text that [black aqua-background]#can# take any form (including fully qualified URLs, for example).
Whilst it is [red aqua-background]#recommended# that a standardised interpolation is provided, the alternative is provided to promote interoperability in cases where a well defined user community needs to use sophisticated interpolation techniques that [red aqua-background]#may# also be under development.

The definition of the interpolation method, however it is specified, [red aqua-background]#may# include instructions to treat groups of physically related coordinates simultaneously, if such tie points are present.
For example, there are cases where longitudes cannot be interpolated without considering the corresponding latitudes.
It is up to the interpolation description to describe how such coordinates are to be identified (e.g. it [red aqua-background]#may# be that such tie point coordinate variables require particular units or standard names).

Note that the interpolation method is always applied on a per interpolation subarea basis, for which the construction of the uncompressed coordinates [red aqua-background]#may# only access those tie points that define the extent of the of the interpolation subarea.
...
For each interpolation variable identified in the **`coordinate_interpolation`** attribute, all of the associated tie point coordinate variables [red aqua-background]#must# share the same set of one or more dimensions.
This set of dimensions [red aqua-background]#must# correspond to the set of dimensions of the uncompressed coordinate or auxiliary coordinate variables, such that each of these dimensions [red aqua-background]#must# be either the uncompressed dimension itself, or a dimension that is to be interpolated to the uncompressed dimension.
...
The dimensions of a tie point coordinate variable [red aqua-background]#must# contain at least one  subsampled dimension, for each of which the corresponding interpolated dimension cannot be included.

The size of a subsampled dimension will be less than the size of the corresponding interpolated dimension.
For example, if the interpolated dimensions are `xc = 30` and `yc = 10`, interpolation [black aqua-background]#could# be applied in both of these dimensions, based on tie point variables of the dimensions `tp_xc = 4` and `tp_yc = 2`.
...
The presence of non-interpolated dimensions in the tie point coordinate variable impacts the interpolation process in that there [red aqua-background]#must# be a separate application of the interpolation method for each combination of indices of the non-interpolated dimensions.
For example, if `xc = 30` is an interpolated dimension and `yc = 10` is a non-interpolated dimension, interpolation [black aqua-background]#could# be applied in the `xc` dimension only, based on tie point variables that have the subsampled dimension `tp_xc = 4` and the non-interpolated dimension `yc = 10`.
...
It is only [red aqua-background]#required# to associate an interpolated dimension  to an interpolation subarea dimension in the case that the interpolation subarea dimension is spanned by an interpolation parameter variable, as described in <<compression-by-coordinate-subsampling-interpolation-parameters>>.
If an interpolation subarea dimension is provided, then it [red aqua-background]#must# be the second of the two named dimensions following the tie point index variable.
...
A tie point index variable is a one-dimensional integer variable that [red aqua-background]#must# span the subsampled dimension.
...
The tie point index values [red aqua-background]#must# be strictly monotonically increasing.
...
As a consequence, any pair of tie point index values that defines an extent of an interpolation subarea [red aqua-background]#must# differ by two or more, i.e. in general, an interpolation subarea spans at least two points in each of its interpolated dimensions.
...
These interpolation subareas contain tie points at both of the subarea boundaries with respect to those subsampled dimensions and so [red aqua-background]#must# span at least three points in the corresponding interpolated dimensions (see <<interpolation_subarea_generation>>).
...
The interpolation variable attribute **`interpolation_parameters`** [red aqua-background]#may# be used to provide extra information to the interpolation process.
...
Any numerical term that is specified as [red aqua-background]#optional# in <<appendix-coordinate-subsampling>> and is omitted from the **`interpolation_parameters`** attribute [red aqua-background]#should# be assumed to be zero.

The **`interpolation_parameters`** attribute [red aqua-background]#may# only be provided if allowed by the definition of the interpolation method.
Interpolation parameters [red aqua-background]#may# always be provided to non-standardized interpolation methods.

The interpolation parameters are not permitted to contain absolute coordinate information, such as additional tie points, but [red aqua-background]#may# contain relative coordinate information, for example an offset with respect to a tie point or with respect to a combination of tie points.
This is to ensure that interpolation methods are equally applicable to both coordinate and bounds interpolation.

The interpolation parameter variable dimensions [red aqua-background]#must# include, for all of the interpolated dimensions, either the associated subsampled dimension or the associated interpolation subarea dimension.
...
[caption="Figure {doc-part}.{counter:figure}. ", reftext=Figure {doc-part}.{figure}]
[.text-center]
.Through combination of dimensions, interpolation parameter variables [red aqua-background]#may# provide values for a) interpolation subareas sharing a tie point, b) each interpolation subarea,  c) and d) interpolation subareas sharing a boundary.
...
Coordinates [red aqua-background]#may# have cell bounds.
For the case that the reconstituted cells are contiguous and have exactly two cell bounds along each interpolated dimension, cell bounds of interpolated dimensions [black aqua-background]#can# be stored as __bounds tie points__ and reconstituted through interpolation.
...
Since the cell bounds are contiguous, bounds points of adjacent cells will coincide and so the full set of bounds points [red aqua-background]#may# be represented as a grid, comparable to the coordinate points grid.
In the middle part of <<figure_interpolation_of_cell_boundaries>>, both the reconstituted bounds points grid and the reconstituted coordinate points grid are shown for a continuous area, where each bounds point [red aqua-background]#may# be shared by up to four cells.
...
This dimension is used for solely descriptive purposes, and is not [red aqua-background]#required# in a compressed dataset. 
...
Both the process of compressing bounds and the process of uncompressing bounds requires the steps to be carried out for a full continuous area, however, individual continuous areas [black aqua-background]#can# be processed independently.
...
A **`bounds_tie_points`** attribute [red aqua-background]#must# be defined for each tie point coordinate variable corresponding to reconstituted coordinates with cell boundaries.
...
In a first step, which [red aqua-background]#must# be carried out for a full continuous area at a time, each bound point is reconstituted by interpolation between the bounds tie points within each interpolation subarea, using the same interpolation method as defined for the ordinary tie points.
...
The interpolation method specifications provided in <<appendix-coordinate-subsampling>> are complete in their description of steps and formulas [red aqua-background]#required# for compressing and uncompressing coordinate data. Formulas are structured in a way that encourages an efficient implementation of the interpolation method in a high-level programming language.
For instance, expressions that are constant within a computational loop [red aqua-background]#should# be externalised from that loop.

**Computational Precision Attribute** +
The data creator [red aqua-background]#shall# specify the floating-point arithmetic precision used during the preparation and validation of the compressed coordinates by setting the interpolation variable’s **`computational_precision**` attribute to one of the following values:
...
For instance, a **`computational_precision**` value of **`"64"**` would specify that, using the same implementation and hardware as the creator of the compressed dataset, sufficient accuracy [black aqua-background]#could not# be reached when using a floating-point precision lower than 64-bit floating-point arithmetic in the interpolation computations [red aqua-background]#required# to reconstitute the coordinates.
