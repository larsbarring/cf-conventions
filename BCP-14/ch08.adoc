...
====
In a longitude-latitude-depth array of soil temperatures all sea points at all depths [lime-background]#can# be eliminated.
...
----
Since `landpoint(0)=363`, for instance, it [lime-background]#can# be inferred that `landsoilt(*,0)` maps on to point 363 of the original data with dimensions `(lat,lon)`.
...
====
Points below the sea floor [lime-background]#can# be eliminated to compress a longitude-latitude-depth field of ocean salinity.
...
(Quantization of integer types, although theoretically [lime-background]#allowed#, is not covered by these conventions.)
