...
| `anomaly_wrt` | __u__ | Difference from a statistical norm (<<anomalies>>).
...
*The definition of this method implies that if _u_ is a either a unit of temperature, or a unit of temperature multiplied by some other unit, the temperature in _u_ [aqua-background]#must# be interpreted as a temperature difference.
Therefore the **`units_metadata`** attribute, if present, [aqua-background]#must# have the value `temperature: difference`.
